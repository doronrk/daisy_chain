		...
		app.search = {
			...
			bindClickToPaginationLinks : function() {
				jQuery("#search div.pagination a").click(function(e) {
					...
					if(jQuery(this).parents("li").hasClass("unselectable")) return false;
			...
			bindChangeToSortBy : function() {
				jQuery("div.sortby select").change(function(e) {
					...
					var winSearch = url.split("?")[1];
					var idx = url.indexOf(winSearch);
					
					if (idx >= 0)
					{
						window.location.hash.replace("#", "");
						window.location.hash = url.substring(url.split("?")[0].length);												
					}
			...
			updateGrid : function() {
				if (window.location.hash && app.search.result != null)
				{
					...
					var url;
					if(cururl.indexOf("frombreadcrumb")>0){						
			...
			SearchResult : function(keywordSearch, categoryID)
			{
				...
				{		
					...
					if(this.isRegistered(dwRefinement, pos) && this.initialized)
					{
						...
						var ul_menu = jQuery("#refinement-region").find(".refineattributes").find("ul li");
						
						jQuery.each(ul_menu,function(i,e){
							var li = jQuery(this);							
							if(li .hasClass("selected") && li .hasClass("unselectable")){li .removeClass("selected");}							
						})
						
						
						
						if(dwRefinement.id=='refinement-region'){							
							var reg_li =jQuery("#refinement-region").find(".refineattributes").find("ul")
							reg_li.unbind("hover");
							var reg_sel = reg_li.find(".selected");
							
							
							var clearmap = function(){
								jQuery("#map-afr-dot").stop(true, true).hide("fast");
								jQuery("#map-asi-dot").stop(true, true).hide("fast");
								jQuery("#map-amr-dot").stop(true, true).hide("fast");
							
								
								jQuery.each(reg_sel,function(i,e){									
									if(jQuery(this).hasClass("selected")){
										var t = jQuery(this).find("a").text();									
										if(t.indexOf('Africa')>-1){
											jQuery("#map-afr-dot").stop(true, true).show("fast");
										}else if(t.indexOf('Asia')>-1){
											jQuery("#map-asi-dot").stop(true, true).show("fast");
										}else if(t.indexOf('America')>-1){
											jQuery("#map-amr-dot").stop(true, true).show("fast");
										}else{
											jQuery("#map-afr-dot").stop(true, true).show("fast");
											jQuery("#map-asi-dot").stop(true, true).show("fast");
											jQuery("#map-amr-dot").stop(true, true).show("fast");
							...
							reg_li.find("li").hover(function(){
								//alert(jQuery(this).hasClass("unselectable"));
				...
				this.renderRefinement = function(dwRefinement, pos)
				{
					...
					//rounds <a> links in IE -asmith					
					jQuery('.leftcontainer ul li a').corner("3px");  
				};
			
				// binds the toggling event //disabled by Rodolfo D. Robasto Jr 2010-06-02
				/*
				...
				this.updateValues = function(dwRefinement)
				{
					...
					//preventing refresh on regmap and intensity
					
					if (jQuery("#refinementIntensityWrapper").length > 0 ||jQuery("#refineRegMapWrapper").length >0){
						return false;						
					}
					...
					if(dwRefinement.type == "attribute" && !dwRefinement.swatchBased && dwRefinement.cutoffThreshold != null)
					{
						...
						{
							jQuery("#"+dwRefinement.id+" div.refineattributes").children().addClass("scrollable-remove");
						}
						else
						{
					...
					//alert(dwRefinement.id);
					
					// add new values to the list
					
					jQuery("#"+dwRefinement.id + " ul li").addClass("unselectable");
					for(var i=0; i<dwRefinement.values.length; i++)
					{
						...
						var _items = jQuery("#"+dwRefinement.id + " ul li a");
						var aexist = jQuery(_items).filter(function() {
							//bug fix -ASMITH 1/4/2013:
							var decoded = jQuery('<div/>').html(value.displayValue).text(); //decodes html special characters. ex: &eacute; becomes
							if(jQuery(this).attr('id')){
								return jQuery(this).attr('id').split("value_")[1].split(" ")[0] == decoded;
							} else {
								return false;
							}
							//old code:
							//return jQuery(this).text() == value.displayValue.replace(/&reg;/g,"®");
						});
						
						if(aexist.length>0){
							aexist.parent().removeClass("unselectable");
							aexist.attr("href", aUrl.replace(/&amp;/g, "&"));
						}				
						jQuery('.leftcontainer ul li a').corner("3px");
				...
				this.updateClear = function(dwRefinement)
				{
					...
					{
						//alert(this.getParamValue('listview'));
						var islist = this.getParamValue('listview');						
						jQuery("#"+dwRefinement.id+" div.refinedclear").empty();
						if(islist){
							jQuery("#"+dwRefinement.id+" div.refinedclear").append("(<a href=\"" + dwRefinement.clearUrl + "&listview=true" + "\">Clear<\/a>)");
				...
				this.toggleRefine = function(objRef)
				{
					...
					if(jQuery(objRef).parent().hasClass("unselectable")) return false;			
					...
					//check state of gridResult:if true,show results in listview 					
					if(this.getParamValue("listview")){url +="&listview=true";}
				...
				this.updateGrid = function(url)
				{
					...
					var shopNowOptions = {
						buttonSelector: ".shopnowbutton",
						imageSelector: "#content div.product div.image",
						buttonLinkSelector: ".shopnowbutton a",
						productNameLinkSelector: "#content div.product div.name a"
					};
...
jQuery(document).ready(function() {
	...
	// init refinement toggling //disabled by Rodolfo D. Robasto Jr 2010-06-02
	/*
	jQuery("#searchrefinements div.navgroup h3").click(function(e) {
		jQuery(this).toggleClass("collapsed"); 
		jQuery(this).nextAll("div.refineattributes").toggle();
		jQuery(this).nextAll("div.refinementIntensityWrapper").toggle();
		jQuery(this).nextAll("#refineRegMapWrapper").toggle();
	});*/
	
	
	app.quickView.quickViewOptions = {
		container: "#content, #hitlist",
		buttonSelector: "div.quickviewbutton",
		imageSelector: "div.product div.image",
		buttonLinkSelector: "div.quickviewbutton a",
		productNameLinkSelector: "div.product div.name a"
	};

	app.shopNow.shopNowOptions = {
		container: "#content, #hitlist",
		buttonSelector: ".shopnowbutton",
		imageSelector: "div.product div.image",
		buttonLinkSelector: ".shopnowbutton a",
	...
	app.quickView.bindEvents(app.quickView.quickViewOptions);
	app.shopNow.bindEvents(app.shopNow.shopNowOptions);
