// jQuery form validation plugin fix for the 'trim' issue in IE7 & IE8
...
//CIS100069910 - Global variable to track the paymentMethodForm submission
var isPaymentRequestSubmitted=false;
...
$(document).ready(function(){
	...
	// US or Canadian postal codes
	$.validator.addMethod("countryspecificpostalcodePassCountrySelector", function( value, element, params ) {
		if (params.selector){
			countrySelector = params.selector
		}else{
			countrySelector = '#addressFormModalCountry';
		}
		if ($(countrySelector).val() == 'CA'){
			return this.optional(element) || /^[ABCEGHJKLMNPRSTVXYabceghjklmnprstvxy]{1}\d{1}[A-Za-z]{1} \d{1}[A-Za-z]{1}\d{1}$/.test($.trim(value));
		}else{
			return this.optional(element) || /^(\d{5}(-\d{4})?$)$/.test($.trim(value));
		}
		...
		switch(e.id) {
			...
			case 'NoResultsSearchForm' :
				$(e).validate({
					...
					invalidHandler: function(event, validator) {
					    ...
					    if (errors) {
					    	$('label[for="search-input"]').empty();
					    	$('label[for="search-input"]').append("The search field was empty. Please enter a search term");
					    	$('label[for="search-input"]').css('color','red');
					    } else {
					    	$('label[for="search-input"]').empty();
					    }
					  }
			...
			case 'PaymentMethodForm' : 
				$(e).validate({
					...
					//CIS100069910 - Included the submit handler to prevent the form submission multiple times
					submitHandler: function(form) {
							 if(!isPaymentRequestSubmitted)
					   		 {
					   		 	isPaymentRequestSubmitted=true;
					   		 	form.submit();
					   		 }
							 else
								return false;
...
//	CHANGE PASSWORD FORM
	...
	rules: {
		...
		logonPassword: 'required',
		logonPasswordVerify: {required:true, equalTo: '#ChangePasswordForm input[name="logonPassword"]'}
	   },
	messages: {
	   }
	});
...
//  CHANGE FORGOT PASSWORD FORM
	...
	$('#ChangeForgotPasswordForm').validate({
		rules: {
			logonPassword: 'required',
			logonPasswordVerify: {required:true, equalTo: '#ChangeForgotPasswordForm input[name="logonPassword"]'}
		   },
		messages: {
		   }
		});
...
//	$('#RegisterForm input[name=logonPassword]').keyup(function(e){checkPasswordStrength(e.target.value)});	
	...
	$('#EmailSignupForm').validate({
		...
		submitHandler: function(form) {
			$("#invalid_email_error").text("");
			emailSubmit(form);
		},
		invalidHandler: function(event, validator) {
			$("#invalid_email_error").text("Invalid");
		}
	});
...
function validateGiftMessageForm(form) {
	...
	form.validate({
		...
		messages: {
			firstName: messages.CHECKOUT_DELIVERY_RecipientFirstNameRequired,
			lastName: messages.CHECKOUT_DELIVERY_ReciptentLastNameRequred,
			fromName: messages.CHECKOUT_DELIVERY_YourNameRequired,
			giftMessage: messages.CHECKOUT_DELIVERY_GiftMessageRequired
		}
	});
