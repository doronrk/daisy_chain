...
CheckoutHelperJS={
	...
	shipRestrictError: "false",

	suppressShipError: false,
	...
	initializeShipmentPage:function(shipmentTypeId){
		...
		// TODO : MAZEEM , remove when multi shipping is supported 
	...
	deleteItemsFromCart:function(orderItemIds, partNumber, forWishlist) { 
		...
		{
				eval("params.orderItemId" + "_" + i + " = orderItemIds[i]");
			}
		}
		else
		{
			params.orderItemId = orderItemIds;
		}
	 ...
	 */
	deleteFromCart:function(orderItemId,partNumber,forWishlist){ 
		//Google Analytics Begin
		_addEventMap.action='Delete';
		_addEventMap.opt_name=partNumber;
	...
	removeBloomEventTacking: function(productId,partnumber,prodName,prodColor)
	{
		...
		if(typeof BrTrk != undefined)
			...
			try
			  {
			BrTrk.getTracker().logEvent('Cart', 'remove', {'acct_id':accId,'prod_id': productId, 'sku' : partnumber, 'prod_name': prodName, 'prod_color':prodColor});
			  }
			catch(err)
			{
		...
		/*****************************************************************
		 *         international Shippping Changes UC-7 
		 * 
		 * calculatePreshippingOrderTotal function is called to calculate
		 * preshipping order total for international shipping only
 		 *****************************************************************/
		/*if(CheckoutHelperJS.checkForInternationalShipCookie()){
		CheckoutHelperJS.calculatePreshippingOrderTotal();
		}*/
	},
	 ...
	 * @param {Boolean} forWishlist If the value is true, then the item is added to the wish list.
	 */
	deleteFromCartShipment:function(orderItemId,partNumber,forWishlist){ 
		//Google Analytics Begin
		_addEventMap.action='Delete';
		_addEventMap.opt_name=partNumber;
		//Google Analytics End
		...
		if(y == 1){
			...
			if(forWishlist){
				...
				if(this.shoppingCartPage || this.pendingOrderDetailsPage){
					...
					wc.service.invoke("AjaxDeleteOrderItemForShippingBillingPage", params);
					dojo.query(".giftBoxItemListID_" +orderItemId).forEach(dojo.destroy);
				}
			}
		}
	},
	 ...
	 * It's only called for shipping address creation or update scenarios. 
	 * If updating/creating a billing an address, orderitem addressid should not be updated (this is the shipping address)
	 *
	...
	saveOrderItemsList:function(orderItemId,addressId){
		...
		// update/create shipping address for single shipment.
	...
	/**
	 * (BOPIS)This Method is used to update Address Details of the Guest against the shipping address of the order
	 * when the order is submitted with a Designate guest pickup
	 *
	 * @param {Integer} addressId A valid address ID.
	 */
	removeStoreAddressIdForOrderItem:function(addressId){
		if(addressId == null || addressId.length == 0){
			return true;
		}	
		var params = [];
		...
		wc.service.invoke("RemoveStoreAddressForOrder", params);    
	},
	/**
	 *UC2(BOPIS) When products needs to be shipd instead of pickup there is an store address ID in an order, this function delete address ID by invoking the <code>AjaxPersonChangeServiceAddressDelete</code> service.
	 *
	 * @param {Integer} addressId A valid storeaddress ID.
	 */
	removeStoreAddressForOrderItem:function(addressId){
		//alert(addressId);
		if(addressId == null || addressId.length == 0){
			return true;
		}
		...
		params.check="*n";				
		this.updateParamObject(params,"addressId",addressId,false);
		wc.service.invoke("RemoveStoreAddOfOrderItems", params);    
	},
	 ...
	 * @param {Integer} addressId A valid address ID.
	 */
	updateGuestPickUpAddressIdForOrderItem:function(addressId){
		if(addressId == null || addressId.length == 0){
			return true;
		}	
		var params = [];
	...
	/**
	 *BOPIS Added for persisting store address during change store operation in cart page
	 */
	updateAddressIdForOrderItemOfStoreAddress:function(addressId){
		if(addressId == null || addressId.length == 0){
			return true;
		}
		var params = [];
		...
		this.updateParamObject(params,"addressId",addressId,false);
		wc.service.invoke("AjaxSetAddressIdOfOrderItemsForStoreAddress", params);    
	},
	

	
	/**
	 * This is used to update the selected store address corresponding to the order from Address page.
	 * After sucessfully updating the address the user will be redirected to DOM payment page
	 *
	 * @param {Integer} addressId A valid address ID.
	 */
	updateStoreAddressIdForOrderItem:function(addressId){
		if(addressId == null || addressId.length == 0){
			return true;
		}
		...
		wc.service.invoke("OrderItemStoreAddressShipMethodUpdate", params);    
	},
	/**
	...
	applyPromotionCode:function(formName,returnView) {
		...
		var errorLabel = dojo.byId("promoCodeErrMsg");
		
		// FIX: 20140910 -- addressing issue of staking promos
		// Added check for allowing multiple promos for LT only
		if((document.getElementById('promotion_1')) && this.langId != '-1'){
			if(this.langId == '-25'){
				errorLabel.innerHTML = 'Un seul code promotionnel par commande';
			} else {
				errorLabel.innerHTML = 'Only one promo code can be used per order';
			}
			
			document.getElementById('promoCode').value = '';
			UtilitiesJS.toggleErrorMsg(errorLabel, "block");
			//setTimeout("UtilitiesJS.toggleErrorMsg(errorLabel,'none')",5000);
			setTimeout("UtilitiesJS.toggleErrorMsg(dojo.byId('promoCodeErrMsg'),'none')",5000);
			return;
		}
	...
	createAddress:function(orderItemId,addressType){		
		// Defect 1175: when creating a billing address, do not save order items list 
		//  billing address should not be saved against order line items, only shipping addresses.
	...
	createShippingAddress:function(orderItemId,addressType){		
		this.saveOrderItemsList(orderItemId,"-1");

		cursor_wait();

		wc.render.updateContext('editShippingAddressContext', {'shippingAddress':'-1','addressType':addressType});
		var url = HBC.billingurl+"&isShippingAddress=true&createAddr=Y";
		HBC.loadShippingAddressStaticDialog(url);
	},
	...
	// Defect 1175: If addresses are same, need to create a new address (instead of editing) so then pass no addressId	
	areShippingAndBillingAddrSame:function(){
		var isSame = false;
		var pf = document.forms["PaymentForm1"];
		var ssa = dojo.byId("singleShipmentAddress");
		
		if (pf != undefined && pf != null &&
			pf.billing_address_id != undefined && pf.billing_address_id != null &&
			ssa != undefined && ssa != null &&
			pf.billing_address_id.value == ssa.value) {
			isSame = true;
		}
	...
	editAddress:function(addressSelectBoxName,orderItemId,profileshipping,profilebilling){		
		...
		var url = HBC.billingurl+"&isShippingAddress=true";

		if (this.areShippingAndBillingAddrSame()) {
			url = url + "&createNewAddr=true";
		}
	...
	editBillingAddress:function(orderItemId,paymentArea,profileshipping,profilebilling){		
		...
		//alert("editBillingAddress..."+form);
		var addressBox = form.billing_address_id;
		//alert("addressBox..."+addressBox);
		...
		cursor_wait();
		// Defect 1175: If addresses are same, need to create a new address (instead of editing) so then pass no addressId
		wc.render.updateContext('editShippingAddressContext', {'shippingAddress':addressBox.value, 'addressType':'ShippingAndBilling'} );

		var url = HBC.billingurl+"&isBillingAddress=true";

		if (this.areShippingAndBillingAddrSame()) {
			url = url + "&createNewAddr=true";
		}
		
		HBC.loadStaticDialog(url);
		//Hide the mainContents (contains shipping/billing details, shop cart details, promotion details, orderDetails)
		//this.showHideDivs('editAddressContents','mainContents');		
	},

	createBillingAddress:function(orderItemId,addressType){
		// Defect 1175: when creating a billing address, do not save order items list 
		//  billing address should not be saved against order line items, only shipping addresses.
		//this.saveOrderItemsList(orderItemId,"-1");
		this.orderItemIds = [];
		...
		cursor_wait();
		wc.render.updateContext('editShippingAddressContext', {'shippingAddress':'-1','addressType':addressType});
		var url = HBC.billingurl+"&isBillingAddress=true&createAddr=Y";
		HBC.loadStaticDialog(url);
	},
	 ...
	 */
	updateCartWait:function(quantityBox, orderItemId,partNumber,event) {
		...
		setTimeout(dojo.hitch(this,"updateCart",quantityBox,orderItemId,partNumber, this.keyPressCount[orderItemId]),this.updateWaitTimeOut);
	 ...
	 */
	updateCart:function(quantityBox, orderItemId, partNumber, keyPressCountValue){
		//Google Analytics Begin
		_addEventMap.action='Update';
		_addEventMap.opt_name=partNumber;
		_addEventMap.opt_value=quantityBox.value;
		//Google Analytics End
	...
	/**
	* replaceItemAjaxHelper This function is used to replace a gift card in the cart, with a new gift card amount. 
	*
	* @param {String} catEntryId The catalog entry of the item to replace to the cart.
	* @param {String} removeOrderItemId The order item ID of the catalog entry to remove from the cart.
	*
	**/
	updateGiftCardCart:function(catEntryId,removeOrderItemId){
		var params = [];
		params.storeId = this.storeId;
		params.catalogId = this.catalogId;
		params.langId = this.langId;
		...
		params2.catEntryId	= catEntryId.value;
		params2.quantity = document.getElementById("itemQty").value;
	...
	/**
	 * This function is used to update the estimated shipping mode of all order items on the shopping bag page only.
	 */
	updateShoppingBagOrderShipMode:function(shipmentSelectBox, controllerURL){
		
		// commented out these lines since it's giving me _obj undefined error when I click on
		// Apply at cart and changes ship method at payment page.  
		// TODO: revisit and advise.!!
		//if(!HBC.formVilidate("applyPostalCode","estShipCharge")){
		//	return false;
		//}

		if(!this.isAjaxCheckOut())return;
		var shipModeId = shipmentSelectBox;
		if(shipModeId == -1){
			return;
		}

		var params = '&estShipModeId=' + shipModeId;
			
	    var zipCode = dijit.byId('postalCode');
	    
	    if(!isValidZipCode(zipCode.value)) {
			zipCode.invalidMessage = MessageHelper.messages["SHOPPINGBAG_PAGE_ZIP_CODE_ERROR_MSG"];
			var originalValidator = zipCode.validator;
			var originalInvalidMsg = zipCode.get("invalidMessage");

			zipCode.validator = function(value, constraints){
		        return false;
		    };		    
		    zipCode.focus();
		    zipCode.validator = originalValidator;
		    zipCode.set("invalidMessage", originalInvalidMsg);
		    //Added for defect 2475
		    dojo.byId("estShipCharge").focus();
		    //END - Added for defect 2475
			return;
		}
	    
	    if(zipCode && zipCode != 'undefined' && zipCode.value.length > 0) {
			params = '&zipCode=' + zipCode.value;
			setCookie('zipCode', zipCode.value);
		}
	
		setCookie('estShipModeId', shipModeId);
		...
		CommonControllersDeclarationJS.setControllerURL('estChargeDisplayAreaController', controllerURL + params);
		wc.render.updateContext('shippingChargeContext', null);
	},
	
	/**
	 * This function is used to update the estimated shipping mode of all order items for shipping and payment page.
	 */
	updateOrderShipMode:function(shipmentSelectBox, controllerURL){
	...
	displayAddressDetails:function(addressSelectBoxValue,addressType){
		if(addressSelectBoxValue != null && addressSelectBoxValue != 'undefined' && addressSelectBoxValue != ""){
			var errorLabel = dojo.byId("label-shippingdropDown");
			UtilitiesJS.toggleErrorMsg(errorLabel, "none");
		}
		if(!this.isAjaxCheckOut()){
	...
	showHideEditAddressLink:function(addressSelectBox,orderItemId){
		//Fix for Edit address -GR- Guru -Start
		var registrantAddressId = $("#registrantAddressId").val();
		//Fix for Edit address -GR- Guru -End
		if(addressSelectBox.value == '-3'){			
			...
			//Fix for Edit address -GR- Guru -Start & Updated by Susanna for GR#163
			if(registrantAddressId == addressSelectBox.value)
				{
				//$(".edit-address-gr").css("pointer-events","none");				
				$('#editAddressLinkHidden_'+orderItemId).css("display","none");			
				$('#editAddressLabel_'+orderItemId).css("display","block");
				}
				
			else
				{
				//$(".edit-address-gr").css("pointer-events","auto");				
				$('#editAddressLinkHidden_'+orderItemId).css("display","block");	
				$('#editAddressLabel_'+orderItemId).css("display","none");
				
				}
			//Fix for Edit address -GR- Guru -End & Updated by Susanna for GR#163
	},
	...
	checkForErrMsgTimeout: function() {		
		if(document.getElementById("ErrorMessage_General") != null && document.getElementById("errorMessageSection") != null){
			document.getElementById("ErrorMessage_General").style.display = "none";
		    document.getElementById("errorMessageSection").style.display = "none";
		}
	},
	
	/************************************************************
	 * Refresh error messages on the Shipping and Payment page
	 ************************************************************/
	refreshErrorMessages:function(){
		var taxErrorSpan = document.getElementById('ErrorMessage_TaxSystem');
		var shipErrorSpan = document.getElementById('ErrorMessage_ShipSystem');
		var taxAddrInvalidSpan = document.getElementById('ErrorMessage_TaxAddrInvalid');
		var errorMsgDiv = document.getElementById('ErrorMessage_General');
		
		taxErrorSpan.style.display="none";
		shipErrorSpan.style.display="none";
		taxAddrInvalidSpan.style.display="none";
		errorMsgDiv.style.display="none";
		
		isTaxError = false;
		isShipError = false;
		isTaxAddrInvalid = false;
		
		if(errorMsgDiv.innerHTML!="" && errorMsgDiv.innerHTML.trim().length > 0) {
			errorMsgDiv.style.display="block";
		} else if ("true" == document.getElementById('shipEstError').innerHTML) {
			isShipError = true;
			shipErrorSpan.style.display="block";				
		} else if ("true" == document.getElementById('taxEstError').innerHTML) {
			isTaxError = true;
			taxErrorSpan.style.display="block";
		} else if ("true" == document.getElementById('taxEstAddrInvalid').innerHTML) {
			isTaxAddrInvalid = true;
			taxAddrInvalidSpan.style.display="block";	
		}
		
		updateErrorMsg('');
	},
	
	updateShopCartHdr:function(hideCheckoutBtn){

		if(hideCheckoutBtn==true){
			dojo.query("div.checkoutbtn").style("display","none");
		}

		var shopbagsize = 0;
		//normal items and standard gift cards
		dojo.query(".quantityType").forEach(function(node, index, nodelist){
			shopbagsize = shopbagsize + parseInt(node.value,10);
		});
		dojo.query(".shopbagqty").forEach(function(node, index, nodelist){
			shopbagsize = shopbagsize + parseInt(node.innerHTML,10);
		});
		//virtual gift cards
		dojo.query(".virtualGiftCardSection").forEach(function(node, index, nodelist){
			shopbagsize = shopbagsize + 1;
		});
		
		var shopbagdisplay = document.getElementById("outerCartContainer");
		if(storeId == 10151){
			shopbagdisplay.innerHTML='<span id="bagItems"><strong>Shopping Bag</strong> ('+shopbagsize+' item'+(shopbagsize>1?"s":"")+')</span>';
		}else if(storeId == 10701){
			shopbagdisplay.innerHTML='<span id="bagItems"> '+shopbagsize+' ITEM'+(shopbagsize>1?"s":"")+'</span>';
		}

	},
	calculatePreshippingOrderTotal:function(){
		var grandTotal=0.00;
		  dojo.query(".item-total").forEach(function(node, index){
			var itemPriceWithCurr=node.innerHTML;
			if(itemPriceWithCurr.indexOf("$") != -1){
			var itemPriceArr=itemPriceWithCurr.split('$');
			}
			else{
			 var cookieValue=dojo.cookie("INTL");
			 var splitCookieValues=cookieValue.split("|");
			 var currCurrency=splitCookieValues[2];
			 var itemPriceArr=itemPriceWithCurr.split(currCurrency);
			}
			grandTotal+=parseFloat(itemPriceArr[1].replace(/\,/g,''));
		 });
		 if(dojo.byId('intlPreshippingOrderTotal')!=null){
			 var grndTotal="$"+parseFloat(grandTotal).toFixed(2);
			 grndTotal=grndTotal.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
			 dojo.byId('intlPreshippingOrderTotal').innerHTML=grndTotal;
		 }
	},
	checkForInternationalShipCookie:function(){
	   var intlCookie = dojo.cookie("INTL");
	    if(window.internationalFlowFlag && typeof intlCookie != "undefined" && intlCookie!=null){
		return	!(intlCookie.indexOf("|US|") != -1);
	    }else{
		return false;
	    }
	},
	checkForPickUpCookie: function(){
		var pickUpCookie = dojo.cookie("WC_pickUpStore");
		if(pickUpCookie != null && typeof pickUpCookie != "undefined"){
			return true;
		}else{
			return false;
		}
...
function setCookie(cookieName, cookieValue) {
	...
	document.cookie = cookieName + '=' + cookieValue;// + ',expireDate=' + theDate + ';expires=' + theDate;
}

function updateEstTotalCharge(total) {
	// getting the total redisplayed after ajax call.
	var estTotalHTML = document.getElementById('estimateTotalCharges');
	estTotalHTML.innerHTML = total;

}

function updateErrorMsg(msg) {
	// Do not want to override following 3 errors - tax system error, tax system address invalid error, ship system error
	var msgSection = document.getElementById('errorMessageSection');
	//Hassane - Part of RTC 3163 fix
	var errorMsgDiv = dojo.byId('ErrorMessage_General');
	//if (isTaxError || isShipError || isTaxAddrInvalid || (msg && msg.length > 0)) {
	if (isTaxError || isShipError || isTaxAddrInvalid || (msg && msg.length > 0) || (errorMsgDiv.innerHTML!="" && dojo.trim(errorMsgDiv.innerHTML).length > 0)) {
	//END - Hassane - Part of RTC 3163 fix
	...
	if (msg.length > 0) {
		...
		*/
		...
		dojo.style(errorMsgDiv, "display", "block");
		...
		dojo.fx.chain([fadeInAnim]).play();
		
		errorMsgDiv.style.display="block";
		//dojo.fx.chain([fadeInAnim, fadeOutAnim]).play();	
		dojo.fx.chain([fadeInAnim]).play();
		
		if (dojo.byId('errorMessageSection').style.display=="block")
		{
			dojo.byId('errorMessageSection').focus();	
		}
		
	}else if(msg.length > 0){
		 	if(document.getElementById('ErrorMessage_TaxSystem') != null && document.getElementById('ErrorMessage_ShipSystem') != null && document.getElementById('ErrorMessage_TaxAddrInvalid') != null){
				document.getElementById('ErrorMessage_TaxSystem').style.display="none";
				document.getElementById('ErrorMessage_ShipSystem').style.display="none";
				document.getElementById('ErrorMessage_TaxAddrInvalid').style.display="none";
			}
			if(document.getElementById('ErrorMessage_General') != null && document.getElementById('errorMessageSection') != null){
				document.getElementById('ErrorMessage_General').innerHTML = '';
				document.getElementById('ErrorMessage_General').innerHTML = msg;
				document.getElementById('errorMessageSection').style.display="block";
				document.getElementById('errorMessageSection').focus();
				document.getElementById('ErrorMessage_General').style.display="block";
			}
			setTimeout('CheckoutHelperJS.checkForErrMsgTimeout();', 30000);
	}
	//END - Hassane - Part of RTC 3163 fix
	//Defect 1547 fix. Focus on top of the page if tax of general error  
	...
	//END - Defect 1547 fix. Focus on top of the page if tax of general error 
}
