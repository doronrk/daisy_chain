...
__sco.sender.send = function (obj) {
    ...
    function iframepost() {
        function tearDown() {
            if (document.getElementById("sc_if_post") != null) {
                document.getElementsByTagName("body")[0].removeChild(document.getElementById("sc_if_post"));
            }
        }
        ...
        try {
            ...
            var isIE = navigator.userAgent.match(/msie(\s+)[5-7]/i) != null;
            ...
            form.setAttribute("action", endpoint + (method == "GET" ? data : ""));
    ...
    var method = obj.args[0], endpoint = obj.args[1], data = obj.args[2], callback = obj.args.length > 3 ? obj.args[3] : null, headers = obj.args.length > 4 ? obj.args[4] : null, ticket = obj.ticket;

    /** Check if we have cors (XMLHttpRequest with credentials), if not fall back to iframe solution **/
    'XMLHttpRequest' in window ? xmlpost() : iframepost();
}
...
function ready() {
    ...
    if (tmpstore.local || tmpstore.cookies)
        message = "sc_ready";
