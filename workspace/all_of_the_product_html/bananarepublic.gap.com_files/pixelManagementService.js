 ...
 * @date: 04-07-2010
 * @rev: 1.0
 * @desc: Pixel Management Service - this is the principle JS Controller
 *        framework file for pixel management which is exposed for WCD to manage
 *        pixels through Pixel Definitions within content at runtime.
 * 
 * Functional Notes: 1) STEP 1: Initialization: Read the
 * pixelPartnerPersistedRegistry variable from the cookie and populate the
 * cookie string data in the model as pixelPartnerPersistedRegistryFromCookie.
 * 
 * 2) STEP 1: Initialization: Convert the
 * pixelPartnerPersistedRegistryFromCookie to an array of pixelPartner objects,
 * and instantiate new pixelPartner objects which would only have the following
 * properties populated: 1. prefix, 2. startDate 3. endDate.
 * 
 * 3) STEP 1: Initialization complete - this happens before any pixelPartners in
 * the pixelPartnerRegistry are processed.
 * 
 * 4) STEP 2: Process pixelPartnerRegistry: When we iterate through the
 * pixelPartnerRegistry, we need to detect if the pixelPartner from the registry
 * is already in the pixelPartnerPersistedRegistry - this can be done quickly by
 * doing an indexOf on the pixelPartnerPersistedRegistryFromCookie string.
 * 
 * NOTE: IN STEP 2 ITEM 4.a AND 4.b ARE NOT NEEDED BECAUSE THE FINAL
 * SERIALIZATION OF THE PERSISTED REGISTRY OCCURS AT STEP 5.B. a. If we find a
 * match in the pixelPartnerPersistedRegistryFromCookie string, then we should
 * update that string with the latest startDate and endDate from the
 * pixelPartner objects that is in the pixelPartnerRegistry. b. When this is
 * updated we should persist the newly updated
 * pixelPartnerPersistedRegistryFromCookie to the cookie variable
 * "pixelPartnerPersistedRegistry".
 * 
 * 5) STEP 3: Final Cleanup after the pixelPartnerRegistry has been scanned, we
 * should then perform a final loop over the pixelPartnerPersistedRegistry to
 * detect any outdated pixelPartners.
 * 
 * a. If an outdated partner is found, do a cleanup on all variables in the
 * cookie related to that partner, and remove the pixelPartner from the
 * pixelPartnerPersistedRegistryFromCookie, and from the
 * pixelPartnerPersistedRegistry. To do the registry update - copy the
 * pixelPartner currently in the pixelPartnerPersistedRegistry to the
 * pixelPartnerPersistedValidRegistry with push. When you complete iteration
 * over the pixelPartnerPersistedRegistry then clear the
 * pixelPartnerPersistedRegistry.clear() - and then change the reference of the
 * pixelPartnerPersistedRegistry to the pixelPartnerPersistedValidRegistry.
 * b.Finally, write the now updated and cleaned up
 * pixelPartnerPersistedRegistryFromCookie variable to the cookie. This should
 * be based on a final loop over the new completely validated
 * pixelPartnerPersistedRegistry.
 * 
 * 1) When we write to the cookie any variable we would also write the
 * pixelPartner to the pixelPartnerPersistedRegistry.
 * 
 * 2) We would also update the pixelPartnerPersistedRegistry variable. This
 * variable contains a list of all pixel partner prefixes.
 * 
 * The pixelPartnerPersistedRegistry variable does not have a prefix because it
 * is global to the service. example format: mpt_<startDate>-<endDate>,abc_<startDate>-<endDate>,myTestPrefix_<startDate>-<endDate>,aaa_<startDate>-<endDate>,bbb_<startDate>-<endDate>,cccc_<startDate>-<endDate>
 * 
 * At runtime compare the pixelPartnerPersistedRegistry variable with the actual
 * registry and if the prefix is in the pixelPartnerPersistedRegistry but NOT in
 * the registry, then check the start and end dates for the pixelPartnerPrefix
 * that was detected in the pixelPartnerPersistedRegistry startDate and endDate
 * variables. If the end date is already passed, then execute a cleanup
 * operation on that partner's cookie variables.
 * 
...
PixelManagementService.prototype = {
	...
	constants : {
		...
		DOTOMI_TOKEN_COOKIE_NAME : "dtm_token"
	},
	constructors : {
	...
	controller : {
		init : {
			main : function() {
				if (this.isActive) {
					var personalizationDataReadyHandler = this.controller.eventHandlers.personalizationDataReadyHandler
							.bind(this);
					Event.observe(document, "personalizationData:ready",
							personalizationDataReadyHandler);
					var domLoadedHandler = this.controller.eventHandlers.domLoadedHandler
							.bind(this);
					...
					} else if (/shopping_bag.do$/.test(document.location.pathname)) {
						Event.observe(document, 'shoppingBag:ready', domLoadedHandler);
						
					} else {
						// Standard event
						processingService.api.addApplicationMethodToRegistry(
								domLoadedHandler, "pixel management service");
					}
				}
			}
		},
		eventHandlers : {
			domLoadedHandler : function() {
				...
				var pixelManagementServiceReadyHandler = this.controller.eventHandlers.pixelManagementServiceReadyHandler
						.bind(this);
			...
			pixelManagementServiceReadyHandler : function() {
				...
				if (!isInitializationComplete) {
					var setMetricsLibraryBound = this.controller.metricsLibraryManager.setMetricsLibrary
							.bind(this);
					setMetricsLibraryBound();

					// STEP 1: Initialization: Read the
					// pixelPartnerPersistedRegistry
					var inspectPixelPartnerPersistedRegistryBound = this.controller.pixelPartnerManager.inspectPixelPartnerPersistedRegistry
							.bind(this);
					inspectPixelPartnerPersistedRegistryBound();

					var inspectPixelPartnerRegistryBound = this.controller.pixelPartnerManager.inspectPixelPartnerRegistry
							.bind(this);
		...
		setPixelPartnerToPixelPartnerRegistry : function(pixelPartner) {
			if (pixelManagementService.isActive) {
				pixelManagementService.controller.pixelPartnerManager
						.setPixelPartnerToPixelPartnerRegistry(pixelPartner);
			}
		}
	}
}
...
Object.extend(pixelManagementService.model.metricsLibrary, {
	common : {
		pageType : null,
		pageId : null,
		pageUrl : null,
				...
				pixelManagementService.constructors,
				{
					...
					AbstractPixelPartner : function() {
						...
						this.controller = {
							...
							setCookieVar : function(cookieVarKey,
									cookieVarValue) {
								var pixelPartnerManager = pixelManagementService.controller.pixelPartnerManager;
								...
								var pixelPartnerCookieVarPrefix = pixelPartnerManagerHelper
										.getPixelPartnerCookieVarPrefix(this);
								var cookieKey = pixelPartnerCookieVarPrefix
										+ cookieVarKey;
								gidLib.setCookieVar(
										PIXEL_MANAGEMENT_PERSIST_COOKIE_NAME,
										cookieKey, cookieVarValue);
							},
							getCookieVar : function(cookieVarKey) {
								...
								var pixelPartnerCookieVarPrefix = pixelPartnerManagerHelper
										.getPixelPartnerCookieVarPrefix(this);
								var cookieValue = gidLib.getCookieVar(
										PIXEL_MANAGEMENT_PERSIST_COOKIE_NAME,
										pixelPartnerCookieVarPrefix
												+ cookieVarKey);
								return cookieValue;
							},
							removeCookieVar : function(cookieVarKey) {
								...
								var pixelPartnerCookieVarPrefix = pixelPartnerManagerHelper
										.getPixelPartnerCookieVarPrefix(this);
								gidLib.removeCookieVar(
										PIXEL_MANAGEMENT_PERSIST_COOKIE_NAME,
										pixelPartnerCookieVarPrefix
												+ cookieVarKey);
							}
						};
						var setCookieVarBound = this.controller.setCookieVar
								.bind(this);
						this.controller.setCookieVar = setCookieVarBound;
						var getCookieVarBound = this.controller.getCookieVar
								.bind(this);
						this.controller.getCookieVar = getCookieVarBound;
						var removeCookieVarBound = this.controller.removeCookieVar
								.bind(this);
		...
		.extend(
				pixelManagementService.controller.metricsLibraryManager,
				{
					setMetricsLibrary : function() {
						var setCommonMetricsBound = this.controller.metricsLibraryManager.setCommonMetrics
								.bind(this);
						setCommonMetricsBound();
						var setShoppingBagMetricsBound = this.controller.metricsLibraryManager.setShoppingBagMetrics
								.bind(this);
						setShoppingBagMetricsBound();
						var setOrderMetricsBound = this.controller.metricsLibraryManager.setOrderMetrics
								.bind(this);
					...
					setCommonMetrics : function() {
						...
						var marketingPartner = commonMetricsHelper
								.getMarketingPartner();
						var ap = commonMetricsHelper.getAp(marketingPartner);
						var tid = commonMetricsHelper.getTid(marketingPartner);
						var enteringBrandCode = commonMetricsHelper
								.getEnteringBrandCode(marketingPartner);
						var isNewCustomerBound = commonMetricsHelper.isNewCustomer
								.bind(this);
						var isNewCustomer = isNewCustomerBound();
						var isRecognizedVisitorBound = commonMetricsHelper.isRecognizedVisitor
								.bind(this);
						...
						var parentStyleId = null;
						
						if (typeof(productService) !== 'undefined' && productService !== null) {
							parentStyleId = productService.productId;
						}
						...
						metricsLibrary.common.pageProductId = parentStyleId;
						...
						metricsLibrary.common.dotomiToken = commonMetricsHelper.getDotomiToken();
						if (null != personalizationService.model.personalizationData.personalizationInfoV1.userContext.userEmailEncrypt) {
							metricsLibrary.common.userEmailEncrypt = personalizationService.model.personalizationData.personalizationInfoV1.userContext.userEmailEncrypt;
							metricsLibrary.common.cipher = personalizationService.model.personalizationData.personalizationInfoV1.userContext.clickStream.cipher;
							metricsLibrary.common.vector = personalizationService.model.personalizationData.personalizationInfoV1.userContext.clickStream.vector;
						}
					},
					commonMetricsHelper : {
						...
						getMarketingPartner : function() {
							...
							if (partnerCookieValue != null
								...
								var partnerCookieValueArray = partnerCookieValue
										.split("|||");
						...
						isNewCustomer : function() {
							...
							var isNewCustomerCookieValue = getCookieVar(
									"omniSession", "isNewCustomer");
						...
						getDotomiToken : function() {
							var dotomiCookieValue = null;
                            dotomiCookieValue = gidLib.getCookie(pixelManagementService.constants.DOTOMI_TOKEN_COOKIE_NAME);
							if (dotomiCookieValue == '0'|| dotomiCookieValue == undefined ||dotomiCookieValue == ""){
								dotomiCookieValue=null;
                             }
	                      return dotomiCookieValue;
						}
					},
					setShoppingBagMetrics : function() {
						var commonViewManagerModel = reportingService.controller.viewManagers.commonViewManager.model;
						var metricsLibrary = this.model.metricsLibrary;
						var getProductListBound = this.controller.metricsLibraryManager.shoppingBagMetricsHelper.getProductList
								.bind(this);
						var productsFromCookie = commonViewManagerModel
								.getProductsDecoded(getCookieVar(
										"omniSessionProducts",
										"strProductsInBag"));
						...
						getProductList : function(productData) {
							...
							if (productsArray.length > 0) {
								...
								for ( var i = 0; i < productsArray.length; i++) {
									...
									var productAttributesArray = productData
											.split("^,^");
									...
									shoppingBagProduct.colorDescription = (productAttributesArray[5] ? commonViewManagerModel
											.parseVars(
													productAttributesArray[5],
													",")
											: "");
									shoppingBagProduct.skuDescription = (productAttributesArray[6] ? commonViewManagerModel
											.parseVars(
													productAttributesArray[6],
													",")
											: "");
					...
					setOrderMetrics : function() {
						...
						var isOrderConfirmationPage = orderMetricsHelper
								.isOrderConfirmationPage();
						var isInternationalOrderConfirmationPage = orderMetricsHelper
								.isInternationalOrderConfirmationPage();
						if (isOrderPage) {
							var setMetricsForOrderPageBound = orderMetricsHelper.setMetricsForOrderPage
									.bind(this);
							setMetricsForOrderPageBound();
						} else if (isOrderConfirmationPage) {
							var setMetricsForOrderConfirmPageBound = orderMetricsHelper.setMetricsForOrderConfirmPage
									.bind(this);
							setMetricsForOrderConfirmPageBound();
						} else if (isInternationalOrderConfirmationPage) {
							var setMetricsForInternationalOrderConfirmPageBound = orderMetricsHelper.setMetricsForInternationalOrderConfirmPage
									.bind(this);
					...
					orderMetricsHelper : {
						...
						setMetricsForOrderConfirmPage : function() {
							...
							var getProductListBound = this.controller.metricsLibraryManager.shoppingBagMetricsHelper.getProductList
									.bind(this);
							var orderMetricsHelper = this.controller.metricsLibraryManager.orderMetricsHelper;
							var productAndOrderDataListIteratorBound = orderMetricsHelper.productAndOrderDataListIterator
									.bind(this);
							...
							if (productAndOrderDataList != null
									&& productAndOrderDataList.length > 0) {
								productAndOrderDataList
										.each(productAndOrderDataListIteratorBound);
							}
							...
							if (null != personalizationService.model.personalizationData.personalizationInfoV1.userContext.billingAddressEncrypt) {
								metricsLibrary.order.billingAddressEncrypt = personalizationService.model.personalizationData.personalizationInfoV1.userContext.billingAddressEncrypt;
							}
						...
						setMetricsForInternationalOrderConfirmPage : function() {
							...
							var productAndOrderDataListIteratorBound = orderMetricsHelper.productAndOrderDataListIterator
									.bind(this);
							// var productAndOrderData =
							// internationalPlaceOrderViewManagerModel.order.products;
							...
							// metricsLibrary.order.productList =
							// metricsLibrary.shoppingBag.productList;
						...
						productAndOrderDataListIterator : function(
								shoppingBagProduct, index) {
							var metricsLibrary = this.model.metricsLibrary;
							var orderMetricsHelper = this.controller.metricsLibraryManager.orderMetricsHelper;
							/*
							 ...
							 */
							if (shoppingBagProduct.productBusinessId
									.indexOf("gross") != -1) {
								var price = orderMetricsHelper
										.getPriceValueFromProductData(shoppingBagProduct.productName);
...
Object
		.extend(
				pixelManagementService.controller.pixelPartnerManager,
				{
					...
					setPixelPartnerToPixelPartnerRegistry : function(
							pixelPartner) {
						var pixelPartnerRegistry = pixelManagementService.model.pixelPartnerRegistry;
						if (pixelPartnerRegistry == null) {
							pixelManagementService.model.pixelPartnerRegistry = [];
							pixelPartnerRegistry = pixelManagementService.model.pixelPartnerRegistry;
						}
						pixelPartnerRegistry.push(pixelPartner);
					},
					/**
					 * This method checks the persisted partners from the
					 * pixelManagementPersist cookie and set the partners to the
					 * pixelPartnerPersistedRegistry in the model. Format of
					 * partner in the cookie: pixelPartner_YYYY/MM/DD-YYYY/MM/DD
					 */
					inspectPixelPartnerPersistedRegistry : function() {
						...
						pixelManagementService.model.pixelPartnerPersistedRegistryFromCookie = gidLib
								.getCookieVar(
										PIXEL_MANAGEMENT_PERSIST_COOKIE_NAME,
										PIXEL_MANAGEMENT_PERSIST_PARTNER_LIST);
						var partnerArray = pixelManagementService.model.pixelPartnerPersistedRegistryFromCookie
								.split(",");
						if (partnerArray != null && partnerArray != undefined
								&& partnerArray != ""
								&& partnerArray.length > 0) {
							// set persistedPartnerRegistry from
							// persistedPartnerCookie
							var length = partnerArray.length;
							for ( var i = 0; i < length; i++) {

								var indexUnderscore = partnerArray[i]
										.lastIndexOf("_");
								var indexDash = partnerArray[i]
										.lastIndexOf("-");
								var cookieVarPrefixMapPixelPartner = null;
								var pixelPartnerCookieVarPrefix = partnerArray[i]
								...
								if (!cookieVarPrefixMapPixelPartner) {
									...
									persistedPartner.pixelPartnerStartDate = partnerArray[i]
											.substring(indexUnderscore + 1,
													indexDash);
									persistedPartner.pixelPartnerEndDate = partnerArray[i]
											.substring(indexDash + 1);
									pixelPartnerPersistedRegistry
											.push(persistedPartner);
					...
					/**
					 * This method copies all pixelPartners from the
					 * pixelPartnerWaitingRegistry to the pixelPartnerRegistry.
					 * It clears the current wait registry and then will
					 * re-inspect the pixelPartnerRegistry. If there are still
					 * wait reports this reports will be re-added to the
					 * pixelPartnerWaitingRegistry in the next run.
					 * 
					 * @author Andrew Southwick
					 */
					inspectPixelPartnerWaitingRegistry : function() {
						if (!window['pixelManagementService']) {
							return;
						}
						var pixelPartnerManager = pixelManagementService.controller.pixelPartnerManager;
						...
						if (pixelPartnerWaitingRegistry != null
								&& pixelPartnerWaitingRegistry.length > 0) {
							pixelPartnerWaitingRegistry
									.each(pixelPartnerWaitingRegistryIterator);
							/*
							 * Clear the current pixelPartnerWaitingRegistry as
							 * this will be rebuilt if their are still reports
							 * waiting when the inspectPixelPartnerRegistry
							 * method is called.
							 */
					...
					inspectPixelPartnerRegistry : function() {
						...
						/*
						 * Clear the pixelPartnerRegistry as all pixelPartners
						 * have been processed. Any partners that were not ready
						 * for transmission are what reports, and are in the
						 * pixelPartnersWaitingRegistry.
						 */
						pixelPartnerRegistry.clear();

						/* STEP 3 injection point */
						pixelPartnerManager
								.cleanUpExpiredParnterFromPersistedRegistry();
						...
						if (pixelPartnerManager.pixelPartnerManagerHasPixelPartnersToClean == true) {
							...
							pixelPartnerToCleanRegistry
									.each(pixelPartnerToCleanRegistryIterator);
							pixelPartnerToCleanRegistry.clear();
						}
					},
					pixelPartnerRegistryIterator : function(pixelPartner,
							registryIndex) {
						...
						if (pixelPartner.isReadyForTransmission == false) {
							if ((pixelPartner.isWaitReport == true)
									&& (pixelPartner.isWaitComplete == false)) {
						...
						if ((pixelPartner.isReadyForTransmission == true)
								&& (pixelPartnerManager
										.isPixelPartnerActive(pixelPartner))) {
							// pixelPartner is ready for transmission so
							// processPixelPartner.
							pixelPartnerManager
									.processPixelPartner(pixelPartner);
						}

						// Step 2 injection point for cookie management.
						/*
						 * No need yet to clean up the string, as the string
						 * will be re-serialized at the end of processing the
						 * pixelPartnerRegistry so that the final clean version
						 * of the pixelPartnerPersistedRegistry is in synch with
						 * the string.
						 */
						// pixelPartnerManager.updatePixelPartnerPersistedRegistryString(pixelPartner);
						if (pixelPartnerManager
								.isPixelPartnerActive(pixelPartner)) {
							pixelPartnerManager
									.setPixelPartnerPersistedRegistry(pixelPartner);
						}
					},
					pixelPartnerToCleanRegistryIterator : function(
							pixelPartner, registryIndex) {
						var pixelPartnerController = pixelPartner.controller;
						var pixelPartnerName = pixelPartner.pixelPartnerName;
						var pixelPartnerManager = pixelManagementService.controller.pixelPartnerManager;
						if (pixelPartnerController.setCleanUp) {
							var setCleanUp = pixelPartnerController.setCleanUp
									.bind(pixelPartner);
							setCleanUp();
						}
					},
					pixelPartnerWaitingRegistryIterator : function(
							pixelPartner, registryIndex) {
						var pixelPartnerController = pixelPartner.controller;
						var pixelPartnerName = pixelPartner.pixelPartnerName;
						var pixelPartnerManager = pixelManagementService.controller.pixelPartnerManager;
					...
					cleanUpExpiredParnterFromPersistedRegistry : function() {
						...
						var PIXEL_MANAGEMENT_PERSIST_COOKIE_NAME = pixelManagementService.constants.PIXEL_MANAGEMENT_PERSIST_COOKIE_NAME;
						var PIXEL_MANAGEMENT_PERSIST_PARTNER_LIST = pixelManagementService.constants.PIXEL_MANAGEMENT_PERSIST_PARTNER_LIST;
						var pixelPartnerPersistedValidRegistry = [];
						var length = pixelPartnerPersistedRegistry.length;
						for ( var i = 0; i < length; i++) {
							var pixelPartnerFromPersistedRegistry = pixelPartnerPersistedRegistry[i];
							if (!pixelPartnerManager
									.isPixelPartnerActive(pixelPartnerFromPersistedRegistry)) {
								// cleanup all variables in the cookie related
								// to that partner
								// remove the pixelPartner from
								// pixelPartnerPersistedRegistryFromCookie,
								// and from the pixelPartnerPersistedRegistry by
								// pushing the valid partner to
								// pixelPartnerPersistedValidRegistry
								pixelPartnerManager
										.removeExpiredCookieVar(pixelPartnerFromPersistedRegistry);
								/*
								 * NOTE: Instead of cleanup on the string while
								 * we are iterating we will serialize the string
								 * at the end of processing based on iterating
								 * over the final version of the
								 * pixelPartnerPersistedRegistry (which is based
								 * on the pixelPartnerPersistedValidRegistry.
								 */
								// pixelPartnerManager.removePixelPartnerFromPersistedRegistryString(pixelPartnerFromPersistedRegistry);
							} else {
								pixelPartnerPersistedValidRegistry
										.push(pixelPartnerFromPersistedRegistry);
							}
						}
						...
						/*
						 * Insertion point here for STEP 3: ITEM 5.b - loop
						 * through the now valid pixelPartnerPersistedRegistry
						 * and re-serialize the
						...
						pixelPartnerManager
								.updatePixelPartnerPersistedRegistryFromCookie();

						/*
						 * Set the PIXEL_MANAGEMENT_PERSIST_PARTNER_LIST cookie
						 * variable with the now updated version of the string
						 * such that all expired partners have been removed from
						 * the PIXEL_MANAGEMENT_PERSIST_PARTNER_LIST string.
						 */
					...
					updatePixelPartnerPersistedRegistryFromCookie : function() {
						...
						for ( var i = 0; i < length; i++) {
							var pixelPartnerFromPersistedRegistry = pixelPartnerPersistedRegistry[i];
							updatedString += pixelPartnerManagerHelper
									.getPixelPartnerPersistedRegistryString(pixelPartnerFromPersistedRegistry)
									+ ",";
						}
						pixelManagementService.model.pixelPartnerPersistedRegistryFromCookie = updatedString
								.replace(/,$/, "");
					},
					setPixelPartnerInboundParameters : function(pixelPartner) {
						...
						for ( var i = 0; i < pixelPartner.pixelPartnerInboundParameterList.length; i++) {
							...
							var inboundParameterValue = gidLib
									.getQuerystringParam(inboundParameterName,
											true);
					...
					setPixelPartnerPersistedParameters : function(pixelPartner) {
						...
						var pixelPartnerCookieVarPrefix = pixelPartnerManagerHelper
								.getPixelPartnerCookieVarPrefix(pixelPartner);
						for ( var i = 0; i < pixelPartner.pixelPartnerPersistedParameterList.length; i++) {
							...
							var persistedParameterNameForCookie = pixelPartnerCookieVarPrefix
									+ persistedParameterName;
							var persistedParameterValue = gidLib.getCookieVar(
									PIXEL_MANAGEMENT_PERSIST_COOKIE_NAME,
									persistedParameterNameForCookie);
							...
							/*
							 * Check to see if the inbound parameter has value
							 * and if so then overwrite the persisted value in
							 * the cookie.
							 */
							if (inboundParameterValue != null
									&& inboundParameterValue != "") {
								gidLib.setCookieVar(
										PIXEL_MANAGEMENT_PERSIST_COOKIE_NAME,
										persistedParameterNameForCookie,
										inboundParameterValue);
					...
					isPixelPartnerActive : function(pixelPartner) {
						var startDate = new Date(
								pixelPartner.pixelPartnerStartDate);
						var endDate = new Date(pixelPartner.pixelPartnerEndDate);
						var today = Date
								.parse(pixelManagementService.constants.DATE_IN_USE);
					...
					removeExpiredCookieVar : function(pixelPartner) {
						...
						var pixelPartnerCookieVarPrefix = pixelPartnerManagerHelper
								.getPixelPartnerCookieVarPrefix(pixelPartner);
						while (gidLib.getCookie(
								PIXEL_MANAGEMENT_PERSIST_COOKIE_NAME).indexOf(
								pixelPartnerCookieVarPrefix) != -1) {
							var cookieString = gidLib
									.getCookie(PIXEL_MANAGEMENT_PERSIST_COOKIE_NAME);
							var startIndex = cookieString
									.indexOf(pixelPartnerCookieVarPrefix);
							var endIndex = cookieString
									.indexOf("=", startIndex);
							var cookieKey = cookieString.substring(startIndex,
									endIndex);
							gidLib.removeCookieVar(
									PIXEL_MANAGEMENT_PERSIST_COOKIE_NAME,
									cookieKey);
					...
					removePixelPartnerFromPersistedRegistryString : function(
							pixelPartner) {
						var pixelPartnerManager = pixelManagementService.controller.pixelPartnerManager;
						var pixelPartnerManagerHelper = pixelPartnerManager.helper;
						var registryVar = pixelManagementService.model.pixelPartnerPersistedRegistryFromCookie;
						if (registryVar != null) {
							var startIndex = registryVar
									.indexOf(pixelPartner.pixelPartnerCookieVarPrefix);
							// length of the Partner-Date format
							// "_YYYY/MM/DD-YYYY/MM/DD" = 22
							var expiredVar = registryVar
									.substring(
											startIndex,
											startIndex
													+ pixelPartner.pixelPartnerCookieVarPrefix.length
													+ 22);
							if (startIndex == 0) {
								registryVar = registryVar.replace(expiredVar
										+ ",", "");
							} else {
								registryVar = registryVar.replace(","
										+ expiredVar, "");
							}
						}
					...
					processPixelPartner : function(pixelPartner) {
						...
						if (pixelPartnerController) {
							if (pixelPartnerController.setPixelPartnerModel
									&& pixelPartnerController.setPixelPartnerTransmissionTemplate) {
								pixelPartnerManager
										.setPixelPartnerInboundParameters(pixelPartner);
								pixelPartnerManager
										.setPixelPartnerPersistedParameters(pixelPartner);
								var setPixelPartnerModel = pixelPartnerController.setPixelPartnerModel
										.bind(pixelPartner);
								setPixelPartnerModel();
								var setPixelPartnerTransmissionTemplate = pixelPartnerController.setPixelPartnerTransmissionTemplate
										.bind(pixelPartner);
								setPixelPartnerTransmissionTemplate();
								if (pixelPartner.pixelPartnerTransmissionTemplate != null) {
									pixelPartner.isReadyForTransmission = true;
									pixelPartnerManager
											.setPixelPartnerTransmissionTemplateToDom(pixelPartner);
									pixelManagementTransmission = new pixelManagementService.constructors.AbstractPixelPartnerTransmission(
											pixelPartner);
									pixelPartnerTransmissionLog
											.push(pixelManagementTransmission);
									if (pixelPartnerController.setCleanUp) {
										pixelPartnerToCleanRegistry
												.push(pixelPartner);
					...
					setPixelPartnerTransmissionTemplateToDom : function(
							pixelPartner) {
						...
						if (pixelPartnerTransmissionElement == null) {
							 ...
							 */
							var pixelPartnerTransmissionElement = Element
									.extend(document.createElement('div'));
							pixelPartnerTransmissionElement.id = pixelPartnerTransmissionElementId;
							pixelManagementTransmissionsElement
									.appendChild(pixelPartnerTransmissionElement);
						}
						pixelPartnerTransmissionTemplate = pixelPartner.pixelPartnerTransmissionTemplate;
						if (pixelPartnerTransmissionTemplate != null) {
							pixelPartnerTransmissionMarkup = pixelPartnerTransmissionTemplate
									.evaluate(pixelPartner);
							pixelPartnerTransmissionElement
									.update(pixelPartnerTransmissionMarkup);
					...
					helper : {
						getPixelPartnerCookieVarPrefix : function(pixelPartner) {
							...
							if (pixelPartner) {
								pixelPartnerCookieVarPrefix = "pixMan_"
										+ pixelPartner.pixelPartnerCookieVarPrefix
										+ "_";
							}
							return pixelPartnerCookieVarPrefix;
						},
						getPixelPartnerPersistedRegistryString : function(
								pixelPartner) {
							var PixelPartnerPersistedRegistryString = null;
							if (pixelPartner) {
								PixelPartnerPersistedRegistryString = pixelPartner.pixelPartnerCookieVarPrefix
										+ "_"
										+ pixelPartner.pixelPartnerStartDate
										+ "-"
										+ pixelPartner.pixelPartnerEndDate;
							}
...
with (myTestPartner) {
	...
	controller.setPixelPartnerModel = function() {
		alert("setPixelPartnerModel() running: pixelPartnerName = "
				+ this.pixelPartnerName);
	}
	controller.setPixelPartnerTransmissionTemplate = function() {
		alert("setPixelPartnerTransmissionTemplate() running: pixelPartnerName = "
				+ this.pixelPartnerName);
	...
	controller.setCleanUp = function() {
		alert("setCleanUp() running: pixelPartnerName = "
				+ this.pixelPartnerName);
	}
}
