/* SiteCatalyst code version: H.25.5   - Crocs revision 1.1  updated 20131030
Copyright 1997-2013 Omniture, Inc. More info available at
...
/* Conversion Config */
s.currencyCode=omnCurrency;
/* Link Tracking Config */
s.trackDownloadLinks=true;
s.trackExternalLinks=true;
s.trackInlineStats=true;
s.linkDownloadFileTypes="exe,zip,wav,mp3,mov,mpg,avi,wmv,pdf,doc,docx,xls,xlsx,ppt,pptx";
s.linkInternalFilters="javascript:,demandware.net," + omnIntFilters;
...
s.visitorNamespace="demandwarecrocs";
s.trackingServer=omntrackingserver;
s.trackingServerSecure=omntrackingserversecure;
...
s._channelPattern="Display Ads|dotomi_banner>Display Ads|dotomi_bannerad>Display Ads|google_banner>Display Ads|LHJ_banner>Display Ads|meredith_banner>Email|bv_email>Email|email_>Email|exacttarget_>Paid Search|google_mobile_ppc_>Paid Search|google_ppc_>Paid Search|msn_mobile_ppc_>Paid Search|msn_ppc_>Paid Search|yahoo_ppc_";


/* ChannelManager Map 
 
 Paid Search Pattern: "_ppc_"
 Natural Search - No adid
 Email - Pattern: "_email_:"
 Affiliate - Parameter: siteid
 Banner Ads (DFA) - Pattern "_bannerad_"
...
function s_doPlugins(s){
	...
	var temp1=s.getQueryParam('adid');
	var temp2=s.getQueryParam('siteID');
	...
	/* Channel Manager */
	s.channelManager('adid,siteID');
	s.eVar72=(document.URL.indexOf("siteID") > -1) ? "Affiliate" : s._channel;
	s.eVar73=s._keywords;
	
	/* Online Quote Amount Stacking */
	s.eVar71=s.crossVisitParticipation(s.eVar72,'s_v71','1','5','>','purchase',0);

	/* Track Internal Campaign */
	if(!s.eVar25){
		s.eVar25=s.getQueryParam('intid');
	}
	s.eVar25=s.getValOnce(s.eVar25,"v25",0);

	/* Track cgid */
	if(!s.eVar29){
		s.eVar29=s.getQueryParam('cgid');
	}

	/* Track cross sell */
	if(!s.eVar45){
		s.eVar45=s.getQueryParam('cross');
	}

	/* Campaign Stacking Code  */
	...
	s.campaignStackingSuccessEvent = 'purchase';
	...
	if(!s.eVar31)
		s.eVar31=s.getQueryParam('q');

	/* Refined Search Values */
	if(!s.prop32)
		s.prop32=s.getQueryParam('prefn1');

	if(!s.eVar32)
		s.eVar32=s.getQueryParam('prefn1');

	if(!s.prop33)
		s.prop33=s.getQueryParam('prefn2');

	if(!s.eVar33)
		s.eVar33=s.getQueryParam('prefn2');

	if(!s.prop34)
		s.prop34=s.getQueryParam('prefn3');

	if(!s.eVar34)
		s.eVar34=s.getQueryParam('prefn3');

	if(!s.prop35)
		s.prop35=s.getQueryParam('prefn4');

	if(!s.eVar35)
		s.eVar35=s.getQueryParam('prefn4');

	if(!s.prop36)
		s.prop36=s.getQueryParam('pmin');

	if(!s.eVar36)
		s.eVar36=s.getQueryParam('pmin');

	if(!s.prop37)
		s.prop37=s.getQueryParam('pmax');

	if(!s.eVar37)
		s.eVar37=s.getQueryParam('pmax');

	if(!s.prop38)
		s.prop38=s.getQueryParam('prefv1');

	if(!s.eVar38)
		s.eVar38=s.getQueryParam('prefv1');

	if(!s.prop39)
		s.prop39=s.getQueryParam('prefv2');

	if(!s.eVar39)
		s.eVar39=s.getQueryParam('prefv2');

	if(!s.prop40)
		s.prop40=s.getQueryParam('prefv3');

	if(!s.eVar40)
		s.eVar40=s.getQueryParam('prefv3');

	if(!s.prop41)
		s.prop41=s.getQueryParam('prefv4');

	if(!s.eVar41)
		s.eVar41=s.getQueryParam('prefv4');

	if(!s.prop42)
		s.prop42=s.getQueryParam('srule');

	if(!s.eVar42)
		s.eVar42=s.getQueryParam('srule');

	if(!s.prop43)
		s.prop43= (!s.getQueryParam('start')) ? '0' : s.getQueryParam('start');

	if(!s.eVar43)
		s.eVar43= (!s.getQueryParam('start')) ? '0' : s.getQueryParam('start');

	if(!s.prop44)
		s.prop44=s.getQueryParam('sz');

	if(!s.eVar44)
		s.eVar44=s.getQueryParam('sz');

	/* Copy confirmation info  to evar and sprop */
	if(s.purchaseID){
		s.eVar50=s.purchaseID
	}
	if(s.purchaseID){
		s.prop50=s.purchaseID
	}
	if(s.state){
		s.eVar49=s.state
	}
	...
	/* Add Search and Refinement Event for to track Visits */
	if(s.events == "event11"){
		s.events="event10,event11";
	}
	...
	if(s.eVar47)
	     s.eVar47=s.maskEmail(s.eVar47);
     
	if(omnFirstName){s.eVar46=omnFirstName;}

	if(omnAssetID == "tracking/omniture_homepage"){
		s.channel="homepage";
		s.eVar10="homepage";
		s.prop8=omnLocale;
		s.eVar8=omnLocale;
	...
	else if(omnAssetID == "tracking/omniture_default"){
		s.pageName=omnBC1_2_3;
		s.channel=omnsprop0;
		s.eVar10=omnsprop0;
		s.prop11=omnBC1_2;
		s.eVar11=omnBC1_2;
	...
	else if(omnAssetID == "tracking/omniture_search_results"){
		s.pageName=omnBC1_2_3;
		s.channel=omnsprop0;
		s.eVar10=omnsprop0;
		s.prop11=omnBC1_2;
		...
		s.eVar58=omnSearchResultCount;
	}
	
	else if(omnAssetID == "tracking/omniture_product_detail"){
		var temp1=omnBC1_2_3;
		var temp2=omnProductTitle;
		s.pageName=temp1+"|"+temp2;
		s.prop11=omnBC1_2;
		s.eVar11=omnBC1_2;
		s.channel="product detail";
		s.eVar10="product detail";
		s.prop12=omnBC1_2_3;
		s.eVar12=omnBC1_2_3;
		s.prop8=omnLocale;
		s.eVar8=omnLocale;
		s.eVar75=omnVersion;
		s.events="prodView,event8,event16";
	}
	
	else if(omnAssetID == "tracking/omniture_cart_open"){
		s.channel="checkout";
		s.eVar10="checkout";
		s.prop8=omnLocale;
		s.eVar8=omnLocale;
		s.eVar75=omnVersion;
		s.prop30=omnPromotionCode;
		s.eVar30=omnPromotionCode;
	}
	
	else if(omnAssetID == "tracking/omniture_cart_remove_product"){
		s.channel="checkout";
		s.eVar10="checkout";
		s.prop8=omnLocale;
		s.eVar8=omnLocale;
	...
	else if(omnAssetID == "tracking/omniture_cart_view"){
		s.channel="checkout";
		s.eVar10="checkout";
		s.prop8=omnLocale;
		s.eVar8=omnLocale;
	...
	else if(omnAssetID == "tracking/omniture_cart_promotion_applied"){
		s.channel="checkout";
		s.eVar10="checkout";
		s.prop8=omnLocale;
		s.eVar8=omnLocale;
		...
		s.prop30=omnPromotionCode;
		s.eVar30=omnPromotionCode;
		s.events="event15,scPromotion";
	}
	
	else if(omnAssetID == "tracking/omniture_checkout_step_one"){
		s.pageName="checkout:step1";
		s.channel="checkout";
		s.eVar10="checkout";
		s.prop8=omnLocale;
		s.eVar8=omnLocale;
	...
	else if(omnAssetID == "tracking/omniture_checkout_step_two"){
	...
	else if(omnAssetID == "tracking/omniture_checkout_step_two_b"){
	...
	else if(omnAssetID == "tracking/omniture_checkout_step_three"){
		s.pageName="checkout:billing address";
		s.channel="checkout";
		s.eVar10="checkout";
		s.prop8=omnLocale;
		s.eVar8=omnLocale;
	...
	else if(omnAssetID == "tracking/omniture_checkout_step_three_b"){
		s.pageName="checkout:payment method";
		s.channel="checkout";
		s.eVar10="checkout";
		s.events="event4";
		s.prop8=omnLocale;
	...
	else if(omnAssetID == "tracking/omniture_checkout_step_four"){
		s.pageName="checkout:review order";
		s.channel="checkout";
		s.eVar10="checkout";
		s.prop8=omnLocale;
		s.eVar8=omnLocale;
	...
	else if(omnAssetID == "tracking/omniture_checkout_step_five"){
	...
	else if(omnAssetID == "tracking/omniture_myaccount_view"){
		s.channel = "account";
		s.pageName="view account";
		s.prop4="My Account";
		s.prop7=s.eVar7="Crocs";
		s.prop8=omnLocale;
	...
	s.eVar17=s.getTimeParting('h','-5',currentYear); // Set hour 
	s.eVar18=s.getTimeParting('d','-5',currentYear); // Set day
	s.eVar19=s.getTimeParting('w','-5',currentYear); // Set Weekend / Weekday
	
	s.eVar16=s.getNewRepeat(); // New or Repeat Visit
	s.eVar20=s.getDaysSinceLastVisit(); // Set Days since last visit
	s.eVar51=s.campaignStacking();  // Campaign Stacking
	...
	/* Copy Campaign Variables to additional eVar for alternate attribution */
	s.eVar52="D=v0";
	s.eVar53="D=v0";
	s.eVar54="D=v0";
	s.eVar55="D=v0";
	...
	/* Copy Internal Campaign Variables to additional eVar for alternate attribution */
	s.eVar57="D=v25";


	/* set persistency on campaign tracking and append campaign value to pageName */
	if(s.campaign){s.c_w('crcampaignname', s.campaign, 0);}

	if(s.c_r('crcampaignname')){
		s.prop5=s.c_r('crcampaignname') + ":";
		if(s.pageName){
			s.prop5=s.prop5 + s.pageName;
		}else{
			s.prop5=s.prop5 + document.location;
		}
	}
	
	if(pageData.pageType == "finder"){
		s.pageName = "shoefinder";
		s.channel = "search results";
	}
}

var omnsprop0, omnsprop1, omnsprop2, omnBC1_2, omnBC1_2_3, omnLocale, omnVersion, omnUser, omnFullSku, omnProducts, omnPromotionCode;

s.doPlugins=s_doPlugins;

s.prop20=location.href;
...
+"ue = '93vtld7i4yjgco#2e~8kf6rqbn0pzh_w1u-m5sxa^', index = alphabet."
...
+"var dc=new Date('1/1/2000');var f=15;var ne=8;if(dc.getDay()!=6||"
+"dc.getMonth()!=0){return'Data Not Available'}else{;z=parseInt(z);"
+"if(y=='2009'){f=8;ne=1};var gmar=new Date('3/1/'+y);var dsts=f-gmar.getDay("
+");var gnov=new Date('11/1/'+y);var dste=ne-gnov.getDay();var spr=new Date('3/'"
+"+dsts+'/'+y);fl=new Date('11/'+dste+'/'+y);var cd=new Date();"
+"if(cd>spr&&cd<fl){z=z+1}else{z=z};var utc=cd.getTime()+(cd.getTimezoneO"
+"ffset()*60000);var tz=new Date(utc + (3600000*z));var thisy=tz.getFullYear("
+");var days=['Sunday','Monday','Tuesday','Wednesday','Thursday','Fr"
+"iday','Saturday'];if(thisy!=y){return'Data Not Available'}else{;var thi"
+"sh=tz.getHours();var thismin=tz.getMinutes();var thisd=tz.getDay();var dow="
 ...
 * Edited to check cval for 'undefined'
 */
s.getNewRepeat=new Function(""
+"var s=this,e=new Date(),cval,ct=e.getTime(),y=e.getYear();e.setTime"
+"(ct+30*24*60*60*1000);cval=s.c_r('s_nr');if(typeof cval === 'undefined' || cval.length==0){s.c_w("
