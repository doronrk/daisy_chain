/*! Certona */
//resxclsx.js v4.2 Copyright 2004-2011 Certona Corporation www.certona.com. All rights reserved.
    ...
    function cw(p) {
        try {
            if (d == "1" || d == "2" || d == "3") {
                ...
                var ap = "appid=" + (resx.appid !== undefined ? resx.appid : "") + "&tk=" + (a(b) ? b : "") + "&ss=" + (a(l) ? l : "") + "&sg=" + (a(d) ? d : "") + "&pg=" + (a(ao) ? ao : "") + "&vr=" + cs + "&bx=" + o,
                    cu = "";
                if (resx.rrelem !== undefined) {
...
app.certona = app.certona || {};
app.certona.pdpCallback = function(data){
    var t = null;
    for (var ag = 0; ag < data.Resonance.Response.length; ag++) {
        if (data.Resonance.Response[ag].display == "yes") {
            if(data.Resonance.Response[ag].output.indexOf("certonaCarousel") > -1){
                if(data.Resonance.Response[ag].scheme == "product3_rr"){
                    var scheme = $("#product2_rr");
                } else if(data.Resonance.Response[ag].scheme == "product2_rr"){
                    var scheme = $("#product3_rr");
                } else {
                    var scheme = $("#" + data.Resonance.Response[ag].scheme);
                }
                if(scheme.length > 0){
                    var displayItems = 4;
                    scheme.html(data.Resonance.Response[ag].output);
                    
                    var carousel = scheme.find(".jcarousel");
                    
                    if(data.Resonance.Response[ag].scheme == "search_rr"){
                        carousel.css("width", "630px");
                        displayItems = 3;
                    }
                    
                    carousel.jcarousel({wrap: "both"});
                    
                    var carouselLeft = carousel.parent().find(".certonaCarouselLeft");
                    carouselLeft.click(function(){
                        $(this).parent().find(".jcarousel").jcarousel("scroll", "-=4");
                    });

                    var carouselRight = carousel.parent().find(".certonaCarouselRight");
                    carouselRight.click(function(){
                        $(this).parent().find(".jcarousel").jcarousel("scroll", "+=4");
                    });
                    
                    scheme.find(".dividerText").css("text-transform", "lowercase");
                    
                    if(carousel.find("li").length <= displayItems){
                        carouselLeft.hide();
                        carouselRight.hide();
                    }
                    
                    if(data.Resonance.Response[ag].scheme == "product3_rr"
                    || data.Resonance.Response[ag].scheme == "product2_rr"){
                        carousel.css("height", "247px");
                    }
                }
            } else if(data.Resonance.Response[ag].scheme == "product3_rr"){
                app.certona.createCarousel("#productCarouselOne", data.Resonance.Response[ag].output, false);
                $(".gridDiv h4").css("margin-left", "20px");
            } else if(data.Resonance.Response[ag].scheme == "product2_rr"){
                app.certona.createCarousel("#productCarouselTwo", data.Resonance.Response[ag].output, true);
            } else {
                t = document.getElementById(data.Resonance.Response[ag].scheme);
                if (t != null && t != "null" && t != "") {
                    t.innerHTML = data.Resonance.Response[ag].output;
                }
            }
        }
    }
...
app.certona.createCarousel = function(id, html, wrap){
    var carousel = $(id);
    
    carousel.html(html);
    carousel.find("div:first").insertBefore(carousel);
    
    if(wrap){
        carousel.find(".gridDiv").wrap("<li></li>");
        carousel.find("li").wrapAll("<ul>");
    }
    carousel.parent().show();
    carousel.jcarousel({wrap: "both"});
    
    var carouselLeft = carousel.parent().find(".carouselLeft");
    carouselLeft.click(function(){
        carousel.jcarousel("scroll", "-=4");
    });
    carouselLeft.css("top", "+=74");
    
    var carouselRight = carousel.parent().find(".carouselRight");
    carouselRight.click(function(){
        carousel.jcarousel("scroll", "+=4");
    });
    carouselRight.css("top", "+=74");
};

app.certona.certonaCategoryCallback = function(data){
    for(scheme in data.resonance.schemes){
        if(data.resonance.schemes[scheme].scheme == "subcategory_rr"){
            for(item in data.resonance.schemes[scheme].items){
                $.ajax({
                    'type': 'POST',
                    'url': app.config.urls.productCard + "?pid=" + data.resonance.schemes[scheme].items[item].id,
                    'success': function(content){
                        if(content.indexOf("html") > -1){
                            return;   
                        }
                        $("#certonaCategory").append(content);
                        var last = $("#certonaCategory .gridDiv[class='gridDiv']").addClass("initialized");
                        last.fadeIn();
                        app.producttile.initAll();
                        app.producttile.initPromotions(last);
                    }
                });
            }
            app.producttile.initSwatchHover();
        }
    }
};