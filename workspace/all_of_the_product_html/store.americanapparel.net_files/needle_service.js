...
N.nps='2';else
N.nps='3';}
if(typeof(AA)!='undefined'&&AA.Config&&AA.Config.LANGUAGE&&AA.Config.LOCALE){np.locale=AA.Config.LOCALE.toLowerCase();np.language=AA.Config.LANGUAGE.toLowerCase();}
var rand=Math.floor((Math.random()*10));if(rand<8){np.group='needler';}else{np.group='agent';}
if(typeof($)=='function'&&$('.nav-list').length!=0){var breadArray=$('.category-breadcrumbs a').length>0?$('.category-breadcrumbs a'):$('.product-breadcrumbs a'),breadLength=breadArray.length,bread="";if(breadLength==0){bread=$('.nav-list').find('a:first').html();}else{if(breadLength==1){bread=breadArray[0].innerHTML;}else{bread=breadArray[0].innerHTML+"|"+breadArray[1].innerHTML;}}
np.newCat=bread.replace(/&amp;/g,'&').toLowerCase();}
