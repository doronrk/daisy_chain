...
More info available at /web/20120528031931/http://www.omniture.com */
//Specify the Report Suite ID(s) to track here 

/*var s_account='pier1dev';
if((document.URL.indexOf('development')>-1)||(document.URL.indexOf('staging')>-1)||(document.URL.indexOf('uat')>-1))
       s_account='pier1staging';
if(document.URL.indexOf('www.pier1.com')>-1)
       s_account='pier1prod';*/

var s_account = sAccount;
...
{
	var crossSellString = '';
	 //Adding Plugin for capturing time from entry to purchase
    s.eVar27 = s.getTimeToEvent('sc_eVar27', '0', 'purchase', 'purchase', 0) ;
	//End
	...
	if(!s.campaign){
		//make sure to check the # param values too.
		s.campaign=s.getQueryParam('s_cid') ? s.getQueryParam('s_cid') : s.getQueryParam('s_cid',null,null,1);
		}
	//if(s.campaign)
	...
	if(!s.eVar3){
		s.eVar3=s.getQueryParam('icid') ? s.getQueryParam('icid') : s.getQueryParam('icid',null,null,1);
		}
	
	
	/* Genesis Integration - Responsys */
		/* Capture Email Message/Campaign Id in rmid parameter */ 
		if(!s.eVar35){
			s.eVar35=s.getQueryParam("rmid")
		}
		/* Capture Recipient/User Id in rrid parameter */
		if(!s.eVar36){
			s.eVar36=s.getQueryParam("rrid")
		}
	/* Genesis Integration Ends */
	...
	//Determine the nav type from either url param or hash param
	var navParamType = s.getQueryParam('nav') ? s.getQueryParam('nav') : s.getQueryParam('nav',null,null,1);
	var navSearchKeyword = "";
	//did the user land on the page because of recommendations?
	if(document.location.hash.indexOf('cross-sell') > -1 || document.location.hash.indexOf('ct-cross-sell') > -1){
		if(document.location.hash.indexOf('ct-cross-sell') > -1){
			crossSellString = 'Certona Cross Sell';
		}else{
			crossSellString = 'Cross Sell';
		}
		s_crossSell();
	}
	//check to see if there are search terms if eVar2 isnt populated
	if(!s.eVar2){
		navSearchKeyword = s.getQueryParam('q') ? s.getQueryParam('q') : s.getQueryParam('q',null,null,1);
		//if navSearchKey check to see if s(searchterm) is populated this only happens on
		//on category landing pages
		if(!navSearchKeyword.length && s.getQueryParam('s',null,null,1).length){
			navSearchKeyword = s.getQueryParam('s',null,null,1);
		}
	}
	...
	if((s.eVar2 || navSearchKeyword) && !s.eVar1)
		s.eVar1='Internal Search';
	
	if(s.eVar3 && !s.eVar1 && navParamType.toUpperCase() != 'TILE')
		s.eVar1='Internal Campaign';
	
	if(s.getQueryParam('nav',null,null,1) && !s.eVar1)
		s.eVar1 = navParamType + '_nav';
	
	if(s.eVar4&& !s.eVar1)
		//s.eVar1='browse';  //removing per US7265
	
	if(s.eVar7 && !s.eVar1)
	    if(crossSellString.length){
	    	s.eVar1=crossSellString;
	    }else{
			s.eVar1='Cross Sell';
		}
...
//Adding Plugin for capturing time from entry to purchase
/*
* Utility Function: getTimeToEvent v 0.1
*/
s.getTimeToEvent = function (cookieName, startEvent, stopEvent, resetEvent, timeToExpire) {
    var e = s.events ? s.events : '';
    t = new Date();
    k = new Date();
    timeToExpire = timeToExpire * 86400000
    var a = new Date();
    a.setTime(a.getTime() + timeToExpire);

    if (!s.c_r(cookieName) && startEvent === '0') {
        s.c_w(cookieName, t.getTime(), timeToExpire ? a : 0);
    } else if (!s.c_r(cookieName) && (e.indexOf(resetEvent) > -1)) {
        s.c_w(cookieName, t.getTime(), timeToExpire ? a : 0);
    }

    if (s.c_r(cookieName) && (e.indexOf(stopEvent) > -1)) {
        if (s.eVar38 !== s.pageName) // Avoid page refresh
        {
            var td = 86400,
                th = 3600,
                tm = 60;
            ...
            var td = 86400,
                ...
                tm = 60;
            var temp
            var day = Math.floor(time / td);
            temp = time % td;
            diff = '';
            if (day !== 0) {
                diff = diff + day + ' day ';
            }
            var hr = Math.floor(temp / th);
            temp = temp % th;
            if (hr !== 0) {
                diff = diff + hr + ' hr ';
            }
            var min = Math.floor(temp / tm);
            diff = diff + min + ' min';
    ...
    if (s.c_r(cookieName) && (e.indexOf(resetEvent) > -1)) {
        s.c_w(cookieName, t.getTime(), timeToExpire ? a : 0);
    }
}
