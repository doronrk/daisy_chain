...
}return new Elements(Array.filter(this,(typeof A=="string")?function(C){return C.match(A);}:A,B));}});Document.implement({newElement:function(A,B){ var elem=this.createElement(A); if(Browser.Engine.trident&&B){["name","type","checked"].each(function(C){if(!B[C]){return ;
} elem.setAttribute(C,B[C]); if(C!="checked"){delete B[C];}});} return $(elem).set(B);},newTextNode:function(A){return this.createTextNode(A);
