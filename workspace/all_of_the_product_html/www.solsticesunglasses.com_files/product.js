(function(app){
	if (app) {
		...
		app.Product = function(response) {
			...
			// boolean to tell us whether we're on quick view or not
			...
			var getRatingSection = function(containerId) {
				...
				jQuery(containerId+" #pdpReadReview").click(function(e) {
					jQuery(containerId+" #pdpTabsDiv").tabs("select", "pdpTab3");
				} );
			...
			var setAvailabilityMsg = function(msg) {
				jQuery(myContainerId+" .availability:last .value").html(msg.replace( /&lt;/g, '<' ).replace( /&gt;/g, '>' ));
			}
			...
			return {
				...
				enableA2CButton: function() {
					...
					jQuery(this.containerId+" .addtocartbutton:last").addClass('button-gold-large');
					jQuery(this.containerId+" .addtocartbutton:last").removeClass('button-gold-large-disabled');
				},
				...
				disableA2CButton: function() {
					...
					jQuery(this.containerId+" .addtocartbutton:last").removeClass('button-gold-large');
					jQuery(this.containerId+" .addtocartbutton:last").addClass('button-gold-large-disabled');
				},
				...
				/*showImages: function(selectedVal, vals)  {
					// If we have a selectedVal of "" then use the first swatch
					// switch our detailView image
					if(selectedVal == "") {
						selectedVal = vals[0].val;
					}
						...
						if (this.val === selectedVal && this.images) {
							...
							if (this.images.small.length > 0) {
								if (isQuickView) {
									...
									jQuery(that.containerId+" .productimage .MagicZoom").attr("href", thisVal.images.large[0].url).attr("alt", thisVal.images.large[0].alt);
									jQuery(that.containerId+" .productimage .MagicZoom img").attr("src", thisVal.images.medium[0].url);
									jQuery(that.containerId+" .productimage .MagicZoomBigImageCont div img").attr("src", thisVal.images.large[0].url);
								}
							}							
							...
							if ((this.images.small.length > 1 || that.isSubProduct())) {
								jQuery.each(this.images.small, function(){
									...
									jQuery(that.containerId+" .productthumbnails:last").append(jQuery("<img/>").attr("src", this.url).attr("alt", this.alt).attr("title", this.title).click(function(e){
										jQuery(that.containerId+" .productimage .MagicZoom").attr("href", thisVal.images.large[0].url).attr("alt", thisVal.images.large[imageInd].alt);
										jQuery(that.containerId+" .productimage .MagicZoom img").attr("src", thisVal.images.large[imageInd].url.replace('large', 'prod_detail'));
										jQuery(that.containerId+" .productimage .MagicZoomBigImageCont div img").attr("src", thisVal.images.large[imageInd].url);
									}));
								});
							}
						}
				...
				showImages: function(selectedVal, vals)  {
					...
					// switch our detailView image
					if(selectedVal == "") {
						selectedVal = jQuery(jQuery('input[name="idcolorlookup"]')[0]).val();
					}
					var variantId = jQuery('input[name="idcolorlookup"][value="' + selectedVal + '"]').attr('id').replace(/ /g,'');
					
					if(jQuery('#' + variantId + '-ZoomInUrl').length > 0) {
						jQuery('#productimageZoomInUrl').val(jQuery('#' + variantId + '-ZoomInUrl').val());
						jQuery('#productimageZoomInAlt').val(jQuery('#' + variantId + '-ZoomInAlt').val());
						jQuery('#productimageZoomInTitle').val(jQuery('#' + variantId + '-ZoomInTitle').val());
					}
					// Switch our main image to our new value
					jQuery('.productimages .productimage img').attr('src', jQuery('#' + variantId + '-ZoomOutUrl').val());
					jQuery('.productimages .productimage img').attr('alt', selectedVal);
					jQuery('.productimages .productimage img').attr('title', jQuery('#' + variantId + '-ZoomOutTitle').val());
					// Set our selected alternate image
					jQuery('.altImage').removeClass('selected');
					jQuery('a[variantcolor=\"' + selectedVal + '\"]').addClass('selected');
					// set our dropdown
					jQuery("#pdpMain .variationattributes .variantdropdown select").val(selectedVal);
					if (vals == null) {
						this.varAttrSelected({data: {id: 'colorLens', val: selectedVal}});
					}
					jQuery('#detailView').html('Detail View');
				},
				...
				*/
				...
				show: function(options) {
					...
					jQuery('#detailView').click(function() {
						var thisSrc = jQuery('.productimages .productimage img').attr('src');
						var thisAlt = jQuery('.productimages .productimage img').attr('alt');
						var thisTitle = jQuery('.productimages .productimage img').attr('title');
						
						// Copy over from our cached hidden fields.
						jQuery('.productimages .productimage img').attr('src', jQuery('#productimageZoomInUrl').val());
						jQuery('.productimages .productimage img').attr('alt', jQuery('#productimageZoomInAlt').val());
						jQuery('.productimages .productimage img').attr('title', jQuery('#productimageZoomInTitle').val());
															
						jQuery('#productimageZoomInUrl').val(thisSrc);
						jQuery('#productimageZoomInAlt').val(thisAlt);
						jQuery('#productimageZoomInTitle').val(thisTitle);
						
						if (jQuery('#detailView').html() == 'Detail View') {
							jQuery('#detailView').html('Full View');
						} else {
							jQuery('#detailView').html('Detail View');
						}	
						return false;
					...
					jQuery('.altImage').click(function() {
						thisProduct.showImages(jQuery(this).attr('variantcolor'), null);
						return false;
					});
					...
					if (model.master || model.variant) {
						...
						jQuery(thisProduct.containerId + " .variationattributes .swatches").each(function(){
							...
							if (pdpVarId === "colorLens") {
								...
								.hover(function(e){
									thisProduct.showSelectedVarAttrVal("colorLens", this.innerHTML);										
									thisProduct.showImages(this.innerHTML, colorAttrDef.vals)
								}).mouseleave(function(e) {
									if (thisProduct.selectedVarAttribs["colorLens"]) {
										thisProduct.showImages(thisProduct.selectedVarAttribs["colorLens"], colorAttrDef.vals)
									}
									else {
										thisProduct.showImages("", [{val: "", images: model.images}]);
									}
									
									thisProduct.showSelectedVarAttrVal("colorLens", thisProduct.selectedVarAttribs["colorLens"] || "");
								});
							}
							else {								
						...
						thisProduct.showImages('', thisProduct.getAttrByID('colorLens').vals);
						
						//if (thisProduct.selectedVarAttribs["colorLens"]) {
							// show swatch related images for the current value								
							// thisProduct.showImages(thisProduct.getAttrByID('colorLens').vals[0].val, thisProduct.getAttrByID('colorLens').vals);
						//}
						//else {
