			...
			bindClickToAllRefineLinks : function() {
				...
				*/
				...
				jQuery(".compareCheck").click(function() {
					...
					if (box.checked === true) {
						app.compare.addProduct({id: data.id, category: data.catId, boxId: data.boxId, img: jQuery(this).parents('.product').find("div.image").find("p.productimage a img")});
					} else {
						app.compare.removeProduct({id: data.id, category: data.catId, boxId: data.boxId, img: jQuery(this).parents('.product').find("div.image").find("p.productimage a img")});
					}
				});
