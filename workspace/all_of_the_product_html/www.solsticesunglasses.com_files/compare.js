	...
	if (app) {
		...
		var refresh = function() {
			if (suppressRefresh) {
	   		...
	   		if (count > 0) {
	   			jQuery('.compareitem img.compareproduct').parent().removeClass('compareitemempty');
	   			jQuery('.compareitem img.compareproduct').removeClass('compareproductempty');
	   			jQuery('#compareItems').show();
				jQuery('.subcategorysortby').css('height', '104px');
	   			jQuery('.compareitem img.compareproduct[alt=Empty Compare Slot]').parent().addClass('compareitemempty');
	    		jQuery('.compareitem img.compareproduct[alt=Empty Compare Slot]').addClass('compareproductempty');
	   		} 
	   		else {
	   			jQuery('#compareItems').hide();
	   			jQuery('.subcategorysortby').css('height', '52px');
	   		}
	   	};
		...
		app.compare = {	
			initialize: function(options) {
				...
				for (var i = 0; i < NUM_COMPARE_PRODUCTS; i++) {
