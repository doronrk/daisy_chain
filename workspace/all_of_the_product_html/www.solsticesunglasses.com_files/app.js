...
/*						
						complete: function(xhr, status) {
						  if (status === 'error' || !xhr.responseText) {
							  ...
							  var jsonObj = jQuery.evalJSON(xhr.responseText);
							  options.callback(jsonObj, status);
			...
			close: function(delay) {
				if ( app.minicart.timer != null || delay == 0) {
					...
					if(jQuery.browser.msie) {
						jQuery('.minicartcontent').hide();  // used hide for IE because it is unable to fade transparent png 
					} else {
