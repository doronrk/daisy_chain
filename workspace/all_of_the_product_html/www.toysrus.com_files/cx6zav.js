...
function oiq_doTag() {
	...
	if (window.oiq_pUser) {
		...
		oiq_user_img.src = "http://px.owneriq.net/euid?pt=cx6zav&uid="+encodeURIComponent(window.oiq_pUser);
	}
...
window.oiq_pt = oiq_ddPush(window.oiq_pt, "cx6zav");
	...
	if(oiq_parsedReferer) {
		if(oiq_parsedReferer.host && oiq_parsedReferer.protocol && oiq_parsedReferer.query) {
			if(oiq_parsedReferer.host.match(/google|bing|ask|aol/gi))	{
				...
				if(oiq_searchString) {
					var oiq_searchTagURL = 'http://px.owneriq.net/esq?pt=cx6zav&URL=';
