...
//disable debug messages for production environment
lpMTagConfig.pluginsConsoleDebug = false;

try{
   var LogoBrand = jQuery('#hdrLogo > a').html();
if(LogoBrand == 'ToysRUs') { lpMTagConfig.vars.push(["page","LOB","TRU"]); } else
if(LogoBrand == 'BabiesRUs') { lpMTagConfig.vars.push(["page","LOB","BRU"]); } else {
lpMTagConfig.vars.push(["page","LOB","TRU"]);
}
}catch(e){}
...
lpMTagConfig.plugins['simpleDeploy'] = {
	...
	config: {
		...
		buttons: [{
			"name": "chat-"+lpMTagConfig.getLPVarValue("LOB")+'-'+lpMTagConfig.getLPVarValue("language")+'-registry',
			"pid":	"BRU-lpChatButton-SiteStripe",
			"afterStartPage": true,
			'ovr':	'lpMTagConfig.db1'
		},{
			"name": "chat-"+lpMTagConfig.getLPVarValue("unit")+'-'+lpMTagConfig.getLPVarValue("language")+'-sitestripe',
			"pid":	"lpChatButton-SiteStripe",
			"afterStartPage": true,
			'ovr':	'lpMTagConfig.db1'
		},{
			"name": "chat-"+lpMTagConfig.getLPVarValue("unit")+'-'+lpMTagConfig.getLPVarValue("language")+'-cart',
			"pid":	"lpChatButton-Cart",
			"afterStartPage": true,
			'ovr':	'lpMTagConfig.db1'
		},{
			"name": "chat-"+lpMTagConfig.getLPVarValue("unit")+'-'+lpMTagConfig.getLPVarValue("language")+'-sitestripe-RD',
			"pid":	"lpChatButton-SiteStripe-RD",
			"afterStartPage": true,
			'ovr':	'lpMTagConfig.db1'
		},{
			"name": "chat-"+lpMTagConfig.getLPVarValue("unit")+'-'+lpMTagConfig.getLPVarValue("language")+'-cart-RD',
			"pid":	"lpChatButton-Cart-RD",
			"afterStartPage": true,
			'ovr':	'lpMTagConfig.db1'
		}]
	}
...
var lp_section = jQuery('#TRUFamilyBrandTitle').text();
if(typeof(lp_section) !== 'undefined' && lp_section !== '') {
	lpMTagConfig.vars.push(["session","Section",lp_section]);
}

var lp_productName = jQuery('#lTitle > h1').text();
if(typeof(lp_productName) !== 'undefined' && lp_productName !== '') {
	lpMTagConfig.vars.push(["page","ProductName",lp_productName]);
}

var lp_price = jQuery('#price > ul > li > span').text();
if(typeof(lp_price) !== 'undefined' && lp_price !== '') {
	lpMTagConfig.vars.push(["page","ProductPrice",lp_price]);
}

var lp_cartItems = jQuery('#cartItemCountId').text();
if(typeof(lp_cartItems) !== 'undefined' && lp_cartItems !== '') {
	lpMTagConfig.vars.push(["session","CartItems",lp_cartItems]);
}

var lp_brand= jQuery('#lTitle > ul > li.first > h3 > label').text().replace(' ','');
if(typeof(lp_brand) !== 'undefined' && lp_brand!== '') {
	lpMTagConfig.vars.push(["page","Brand",lp_brand]);
}


try{
var regex = new RegExp("https:|cart/index.jsp");
if(!document.URL.match(regex)) {
var mc_val = document.createElement('span'); 
jQuery(mc_val).load('/minicart/index.jsp div.minicart_total_price span.value', function(result) { var resCartValue = jQuery(mc_val).find('span.value').html();
   if(resCartValue !== 'null'){ resCartValue = '$0.00'; }
lpMTagConfig.vars.push(["session","CartTotal",resCartValue ]);
});
}
} catch(e) {}


var lp_storeLocation = jQuery('#hdrSection1 > ul.nav > li.myStore.headerTip.hasStore > div > div.storeSearch.tip > div.tipcontent > div.tipcontenttop > div').text();
if(typeof(lp_storeLocation ) !== 'undefined' && lp_storeLocation !== '') {
	lpMTagConfig.vars.push(["session","StoreLocation",lp_storeLocation ]);
} else {
	lpMTagConfig.vars.push(["session","StoreLocation","None Selected"]);
}

var lp_OrderNumber = jQuery('#orderNumberValue').text();
if(typeof(lp_OrderNumber ) !== 'undefined' && lp_OrderNumber !== '') {
	lpMTagConfig.vars.push(["page","OrderNumber",lp_OrderNumber ]);
}

var lp_OrderTotal = jQuery('#costSummary table.summary tbody tr td.subTotal').text();
if(typeof(lp_OrderTotal ) !== 'undefined' && lp_OrderTotal !== '') {
	lpMTagConfig.vars.push(["page","OrderTotal",lp_OrderTotal ]);
}
