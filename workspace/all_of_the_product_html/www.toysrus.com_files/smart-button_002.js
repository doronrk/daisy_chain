...
wcsbLoadImage('https://www.dsply.com/index.php?pid=rv4a0pklnx&e=product-detail-page-view&site=toysrus&cpi='+escCpi+'&localtimestamp='+timestamp+'');
}
}
...
function wcsbGetCpi ()
{
if (typeof(wcCpi) != 'undefined')
  return wcCpi;

var urlParts = wcsbGetUrlParts(location.href);;
return wcsbGetParameter(urlParts.query, 'productid');
}
function wcsbCallProductButton()
{
setTimeout(wcsbSa, 1);
wcsbLoadScript('http://content.webcollage.net/toysrus/smart-button?ird=true&channel-product-id=' + encodeURIComponent(wcsbGetCpi()), 'wcsb-auto');
}

function wcsbIsWrongTRUSite ()
{
  // Wrong site = this script on non-US site. Right site = anything else (to allow US testing environments)
  var sd = location.hostname.toLowerCase().split('.');  
  return (sd[sd.length - 2] == 'toysrus' && sd[sd.length - 1] != 'com');
}

if (typeof(wcsbAvoidDoubleResponse)=='undefined' && !wcsbIsWrongTRUSite())
{
