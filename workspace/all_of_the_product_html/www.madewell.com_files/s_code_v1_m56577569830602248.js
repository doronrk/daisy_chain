...
function s_doPlugins(s) {
	...
	//add the current domain to the linkInternalFilters.
	...
	if (identifySite()=="jcrew")
		...
		s.linkInternalFIlters=s.apl(s.linkInternalFilters , "jcrew.com", ',', 2);
		}
	else if (identifySite()=="jcrew dev")
		{
		s.prop8=s.eVar28="J.Crew Dev";	
		s.linkInternalFIlters=s.apl(s.linkInternalFilters , "jbmacs", ',', 2);
		}
	else if (identifySite()=="factory")
		{
		s.prop8=s.eVar28="Factory";
		s.linkInternalFIlters=s.apl(s.linkInternalFilters , "factory.jcrew.com", ',', 2);
		}
	else if (identifySite()=="madewell")
		{
		s.prop8=s.eVar28="Madewell";
		s.linkInternalFIlters=s.apl(s.linkInternalFilters , "madewell.com", ',', 2);
    ...
    /* Find out the last five Refinement Value the site within the last 90 days */
	s.eVar26=s.crossVisitParticipation(s.eVar16,'s_ev26','1','5','>','scAdd,purchase',0);

    /* Find out the last five Refinement Attribute within the last 90 days */
	s.eVar27=s.crossVisitParticipation(s.eVar17,'s_ev27','1','5','>','scAdd,purchase',0);
	...
	/*
	//removed due to the reuse of eVar27
...
//Automate setting of prop22 and eVar42
...
else if ((!document.referrer) && (!s.eVar5) && (s.prop41.indexOf("View Shopping Bag")==-1))
{
	s.eVar5="Direct";	
}
...
if(s.c_r('productnum')&&s.events.indexOf('purchase')>-1)
	...
	/**Disables tracking if s.pageName and previous pageName (prop41) are set to "Checkout Billing". 
	This is to fix double tracking when the page reloads when switching payment methods **/
	
	if ((s.pageName=="Checkout Billing")&&(s.prop41=="Checkout Billing")&&(s.linkTrackVars=="None,events,products"))
		s.abort=true;

	
	//pass pageName into temporary variable
	s._tempPageName=s.pageName;
	s._tempCampaign=s.campaign;
	s._temp_eVar14=s.eVar14;
	//enable dynamic compression of variables
	s.dynComp();
	//copy temp pageName variable back into pageName
	s.pageName=s._tempPageName;
	s.campaign=s._tempCampaign;
	s.eVar14=s._temp_eVar14;
	
	//Removes "setupLinkTrack" on Confirmation page
	if ((!s.events)||(s.events&&(s.events.toLowerCase().indexOf("purchase")==-1)))
		{
		s.hbx_lt = "manual" // manual, auto
		s.setupLinkTrack("prop41,,,prop25","SC_LINKS");
		s.eVar43=s.prop25;
		}

	
	//set non-category, non-product pages
	if ((!s.eVar62)&&((!s.prop11)||((s.prop11)&&(s.prop11.toLowerCase()!="checkout")))&&((!s.events)||((s.events)&&(s.events.toLowerCase().indexOf("prodview")==-1))))
		s.eVar62="non-category";
		
		
		
	if ((s.prop11)&&(s.prop11=="Quick Shop"))
		{
		/* s.eVar27=s.eVar62=""; */
		s.eVar62="";
		}
	...
	/*Responsys Message ID*/
	s.eVar65=s.getQueryParam('Rmid');
	s.eVar65 = s.getValOnce(s.eVar65,'s_var_65',0);

	/*Responsys Recipient ID*/
	s.eVar64=s.getQueryParam('Rrid');
	s.eVar64 = s.getValOnce(s.eVar64,'s_var_64',0);
...
function s_recommendations(recProduct) {
	...
	if (document.location.href.indexOf("discover")>=0)
		{
		s.eVar5='Discovered Carousel';

		s.tl(true,'o','Discovered Carousel Click');
		}
	else
		{
