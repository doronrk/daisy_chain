...
var app = (function (app, $) {
	...
	function initializeEvents() {
		...
		// Email Signup Submit Event
		// Originally in mailinglist.isml (called from footer.isml)
		$('#emailsignup').submit(function() {
			...
			emailInput = $('#MailingList')
			
			//show response message - welcome...  
        	responseMsg.hide()  
            .addClass('response-waiting')  
            .text('Welcome to our email list.')  
            .fadeIn(200);  
	        ...
	        $.ajax({  
	            ...
	            success:function(data){  
	                ...
	                if(data.success){
	            		...
	            		responseMsg.text("${Resource.msg('footer.mailinglisterrormessage','components',null)}");
	                }
	            }  
	        })  
...
(function (app, $) {
	...
	app.storefront = {
		init : function () {
			...
			$(".homepageMain .product-tabs").tabs();
			
			$(".homeproducttile #quickviewbutton").on("click", function (e) {
					e.preventDefault();
			...
			function slideCarousel_initCallback(carousel) {
				...
				var numSlides = $('#mycarousel li').size();
				var slideShowNav = '<div class="jcarousel-control">';
				for( i=1; i <= numSlides; i++) {
					slideShowNav = slideShowNav + 
						'<a href="#" id="'+i+'" class="link-'+i+'">' + i +'</a>';
				}
				slideShowNav = slideShowNav + '</div>';
				 

				$('#mycarousel .jcarousel-clip').append(slideShowNav);
				
				$('.jcarousel-control a.link-1').css('border-top', '0px');
			...
			function slideCarousel_itemVisible(carousel, item, idx, state) {
				// jfriend 071613
				// After adding in the wrap:circular property (below), the active state
				// was lost after a full cycle. This isn't the most elegant solution, but it
				// works for now up to a full 6 cycles through. The chances of someone sitting
				// on the page for that amount of time, and then noticing, are pretty slim.
				// In the future this could be looked at for a better solution.
				
				//The above issue fixed. You can reuse it for other jcarousels as well.
				$('.jcarousel-control a').removeClass('active');
				$('.jcarousel-control').find('.link-'+idx).addClass('active');
				if($('.jcarousel-control').find('a').last().hasClass('active')){
					setTimeout(function(){$('.jcarousel-control').find('a').first().trigger('click');},8000);
				}
			}
			
			var mycarousel_fadeOut = function(carousel, state) {
			    if (state !== "init") {
			        $(carousel.clip.context).find('img').fadeOut(0);
			    }
			};

			var mycarousel_fadeIn = function(carousel, state) {
			    if (state !== "init") {
			        $(carousel.clip.context).find('img').fadeIn(0);
			    }
			};
...
(function (app, $) {
	...
	function loadZoom() {
		...
		if(app.quickView.isActive()) { return; }
	...
	function replaceImages() {		
		...
		// jfriend 062013
		// Because the View Full Details button is within div.product-image-container
		// We have to manually add back in the button during a Color swatch image click.
		var viewDetailsButton = $("#view-full-details");
		
		imageContainer.html(newImages.html() + '<div id="view-full-details">' + viewDetailsButton.html() + '</div>');

		if( $("#view-full-details").text().indexOf('undefined') >= 0) {
			 $("#view-full-details").hide();
		}	
		newImages.remove();
		setMainImageLink();
		...
		jQuery("#scrolling-thumbs").jcarousel({
			scroll: 1,
			visible: 6,
			vertical: true,
			wrap: ""
	...
	function initializeEvents() {
		...
		$cache.pdpMain.on("click", ".videothumb .pdpSmallVideoClick", function () {
			var ylink = $(this).attr('ylink');
			var videohtml = '<iframe width="450" height="646" src="'+ ylink +'" frameborder="0" allowfullscreen></iframe>';
			$('.product-primary-image').hide();
			$('.videosection').html(videohtml);
		});
		
		$cache.pdpMain.on("click", ".videothumb .qvSmallVideoClick", function () {
			var ylink = $(this).attr('ylink');
			var videohtml = '<iframe width="310" height="441" src="'+ ylink +'" frameborder="0" allowfullscreen></iframe>';
			$('.product-primary-image').hide();
			$('.videosection').html(videohtml);
			

		});
		// productthumbnail.onclick() - end
		...
		$cache.pdpMain.on("change", ".variation-select", function(e){
			...
			// jfriend 071013
			// #44667 Adding to Cart using Size and Qty labels
			// Because we switched the Size option to a dropdown, 
			// with a Size label option, we need to make sure the
			// customer doesn't select a legit option, then switch
			// back to the Size label option. To prevent this, we're
			// automatically resetting the add to cart button to 
			// disabled when that scenario occurs.
			...
			// jfriend 061913
			// Because the size option is now a dropdown, I had to add
			// a check for the uuid manually so that editing a product
			// from the QV of the cart page (Edit Details link) updated
			// the approprite cart pli if you changed the size.
			var uuid = $cache.pdpForm.find("input[name='uuid']").val();
		...
		$cache.pdpMain.on("click", "div.product-detail a[href].swatchanchor", function (e) {
			...
			//Override disable button
			//setTimeout("jQuery('#add-to-cart').removeAttr('disabled')", 500);
	...
	function setAddToCartHandler(e) {
		...
		var color = $(".selected-value").text();
				
		var size = $("#va-size option:selected").text();
		var isSubItem = $(this).hasClass("sub-product-item");
		
		//If the Qty isn't selected already, then make it ONE (1).
		if(qty.length === 0 || isNaN(qty.val()) || parseInt(qty.val(), 10) === 0) {
			qty.val("1");
		}
		
		// selected-value is the color selected.
		if ($(".selected-value").length === 1)
		{
			//Is the color selected?
			if(color.length === 0 || color.toUpperCase().trim() == "CHOOSE A COLOR") {
				alert("Please select a color!")
				return false;
			}
			
			//Is the size selected?
			if(size.length === 0 || size.toUpperCase() == "SIZE") {
		...
		initializeDialog : function (eventDelegate, eventTarget) {
			$(eventDelegate).on("click", eventTarget, function (e) {
				...
				var dlg = app.dialog.create({target:$("#send-to-friend-dialog"), options:{
					...
					height:'auto',
					title:this.title,
					dialogClass:'shadow enhance-title',
					open:function() {
...
(function (app, $) {
	...
	function initializeEvents() {
		...
		$cache.main.on("click", ".refinements a, .pagination a, .breadcrumb-refinement-value a", function (e) {
			...
			// jfriend 062513
			// #43048 Search result page with refinements not able to select category
			// Category refinements were behaving incorrectly (double hash, unclickable).
			// Commented out old way and duplicated line from else condition and everything
			// seems to work as it should now. I believe this is because the categories in 
			// the URL are "nicely" named and therefore the cid querystring param isn't being used.
		...
		$cache.main.on("change", ".sort-by select", function (e) {
			...
			window.location.search = uri.query.substr(1);
			return false;
		})
		...
		loadBonusOption : function () {
			...
			app.dialog.create({
				...
				options : {
					...
					title_custom: app.resources.BONUS_PRODUCT 
				}
			});
	...
	app.giftcard = {
		checkBalance : function (id, callback) {
			...
			app.ajax.getJson({
				url: url,
				callback: function (data) {
					var giftCertAmount = $("#dwfrm_billing_giftCertAmount");
					var redeemGiftCert = $("#redeemGiftCert");
					var GiftCardError = $("#GiftCardError");
					GiftCardError.show();
					redeemGiftCert.show();
					giftCertAmount.show();
					giftCertAmount.val(data.giftCard.balance);
					var bal=giftCertAmount.val();
					bal=bal.replace('USD','$');
					giftCertAmount.val(bal);
					if (bal) {
						GiftCardError.hide();
					}
...
(function (app, $) {
	...
	/*
	 * This function has been motified to unbind(off) the click event instead of 
	 * using jQuery one() to handle multipule bindings of the click event.
	 * 
	 ...
	 */
	function bindQvButton() {
		$cache.qvButton.off('click').click(function(e){
			e.preventDefault();
			app.quickView.show({
				url : $(this).attr("href"),
				source : "quickview"
...
(function (app, $) {
	...
	app.minicart = {
		...
		init : function () {
			...
			// jfriend 052213
			// Removed collapse/expand functionality from minicart
			...
			$searchField.focus(function () {
				if(!$resultsContainer) {
					...
					$resultsContainer = $("<div/>").attr("id", "suggestions").appendTo($searchContainer).css({
						"top" : $searchContainer[0].offsetHeight+4,
						"left" : 0,
...
function lookbook() {
		...
		$cache = {
					slide : $('.slide'),
					slider : $('#lookbook-slider'),
					wrapper : $('#wrapper')
		};
	//	carousel.jcarousel(app.components.carouselSettings);	

		function slideCarousel_initCallbackLB(carousel) {
				
				$('.jcarousel-control a.link-1').css('border-top', '0px');
 				$('.jcarousel-item').css('width', '1050px');
			   ...
			   $('.jcarousel-control a').bind('click', function() {
			    ...
			    $('#jcarousel-next-button').bind('click', function() {
			        carousel.next();
			        return false;
			    });
			    
			     $('#jcarousel-prev-button').bind('click', function() {
			        carousel.prev();
			        return false;
			    });
			    
				//$('a.breadcrumb-last').attr("href", "/s/loveculture/lookbooks.html");
			   
			   $cache.slide.width($cache.wrapper.width());			   
			}
			
			function slideCarousel_itemVisibleLB(carousel, item, idx, state) {
				// jfriend 071613
				// After adding in the wrap:circular property (below), the active state
				// was lost after a full cycle. This isn't the most elegant solution, but it
				// works for now up to a full 6 cycles through. The chances of someone sitting
			...
			$cache.slider.jcarousel({
				...
				buttonNextHTML: ".next",
				buttonPrevHTML: ".prev",
				itemFallbackDimension: '1050px',
				initCallback: slideCarousel_initCallbackLB,
				itemFirstInCallback: slideCarousel_itemVisibleLB
			});
...
jQuery(document).ready(function () {
	app.init();
		//creates lookbook carousel using jQuery jcarousel plugin
	lookbook();
	
	$( ".homepageMain #header" ).css( "background","none");
	$(window).scroll(function(){
		if ($(".homepageMain").length){
			if ($(this).scrollTop() == 0) {
				$( ".homepageMain #header .header-main-section" ).css("background","transparent");
				$( ".homepageMain #navigation" ).css("background","transparent");
		    } else {
		    	$( ".homepageMain #header .header-main-section" ).css( "background-color","#FFFFFF");
		    	$( ".homepageMain #navigation" ).css( "background-color","#FFFFFF");
		    }
		}
	});
	...
	if ($(".homepageMain").length) {
		var $homecache = {};
		$homecache = {
			item : $(".js #mycarousel li.jcarousel-item"),
			items : $(".js #mycarousel ul#homepage-slides")
		};
		
		var windowWidth = $(window).width();
		if (windowWidth > 1000){
			$homecache.item.css("width", windowWidth+'px');
		} else {
			$homecache.item.css("width", '1000px');
		}
		
		var itemWidth = $homecache.item.width();
		var leftWidth = parseInt($homecache.items.css("left"));
		var leftWidthShare = parseInt(leftWidth/1000); 
		var itemscount =  $(".js #mycarousel ul#homepage-slides li").length;
		
		if (windowWidth > 1000){
			$homecache.items.css("width", itemscount*itemWidth+'px');
			$homecache.items.css("left", leftWidthShare*itemWidth+'px');
		} else {
			$homecache.items.css("width", '4000px');
			$homecache.items.css("left", leftWidthShare*1000+'px');
		}
	}
...
$(window).resize(function () {
	if ($(".homepageMain").length){
		...
		if (windowWidth1 > 1000){
			$homecache.item.css("width", windowWidth1+'px');
		} else {
			$homecache.item.css("width", '1000px');
		}
