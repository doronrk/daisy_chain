...
s.linkInternalFilters="javascript:,finishline.com,securecheckout.billmelater.com,paypal.com"
...
s._channelDomain="Social Media|facebook.com,twitter.com,digg.com,linkedin.com,myspace.com,wanelo.com,instagram.com,pinterest.com,polywore.com,plus.google.com,youtube.com";
s._channelPattern="Affiliates|AFL->Content|CON->Display Ads|DSP->Email|EM->Paid Search|PPC->Shopping Comparison|SCC->Social Media|SM->Text Messages|TXT-";
...
function s_doPlugins(s) {
	...
	s.eVar48=s.getPreviousValue(s.pageName,'gpv_pn','');
	...
	*/
    ...
    s.eVar55=s.getValOnce(s._channel,'s_cm_ch',0);
   
    //paid social media
    if(s.eVar55 == "Paid Non-Search" && (s.eVar53.toLowerCase() == "tw_finl" || s.eVar53.toLowerCase() == "fb_finl"))
        s.eVar55 = "Social Media";
    
    //adwords/adcenter paid search
    if(s.eVar55 == "Paid Non-Search" && (s.eVar53.toLowerCase() == "adwords" || s.eVar53.toLowerCase() == "adcenter"))
        s.eVar55 = "Paid Search";

    //Check for affiliates
    var siteId = s.getQueryParam('siteID');
    if(siteId) {
        s.eVar55 = "Affiliates";
        s.eVar53 = siteId.split('-')[0];
    }
    
	//email matches
    if(s.eVar55 == "Paid Non-Search" && (
            "t5_,t2_,t3_,t4_,r4_".indexOf(s.eVar53.substr(0,3).toLowerCase())>-1    //check first three characters
         || "r12_,r10,r01_".indexOf(s.eVar53.substr(0,4).toLowerCase())>-1          //check first four characters
       )) {
        s.eVar55 = "Email";
    }
	
	//remove invalid evar13 value
    if(s.eVar13 && s.eVar13.toLowerCase() == "pdp:traditional view") s.eVar13 = "";
    
    /*copy paid channels to eVar13*/
	if("Affiliates, Paid Search, Shopping Comparison, Display Ads, Social Media, Email".indexOf(s.eVar55) > -1)
        s.eVar13 = s.eVar55;
	
	
	/*Cross Visit Participation*/
	s.eVar65=s.crossVisitParticipation(s.eVar13,'s_v65','1','5','>','purchase',0);
    
	/*Check for Affiliate --OLD--*/
    ...
    */
	...
	// Time Parting
  		...
  		s_hour=s.getTimeParting('h','-5',currentYear);
  		s_day=s.getTimeParting('d','-5',currentYear);
  		s_timepart=s_day+"|"+s_hour;
  		s.eVar71=s_timepart.toLowerCase(); // In CST
...
/*
 * Plugin: Cross Visit Participation
 */
