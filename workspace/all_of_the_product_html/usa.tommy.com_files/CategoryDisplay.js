...
categoryDisplayJS={
	...
	**/
	...
	/**
	* variable for the newsletter cookie expiry days.
	**/
	newsletterCookieExpiry: 1,
	...
	*/
	...
	/**
	* setUpdateTaxes This function will set the flag "updateTaxes" which is used to determine if the shopping cart needs taxes to be updated.
	*
	* @param {Boolean} updateTaxes Flag which indicates whther to use taxes will be updated.
	*
	**/
	...
	**/
	setSelectedAttribute : function(selectedAttributeName , selectedAttributeValue){ 
		...
		this.selectedAttributes[selectedAttributeName] = selectedAttributeValue;
		    ...
		    var re = new RegExp("([?|&])" + selectedAttributeName + "=.*?(&|$)","i");
		    if (this.moreInfoUrl.match(re))
		    	this.moreInfoUrl = this.moreInfoUrl.replace(re,'$1' + selectedAttributeName + "=" + selectedAttributeValue + '$2');
		    else
	...
	**/
	changePrice : function(entitledItemId,isPopup,displayPriceRange){
		...
		if(catalogEntryId!=null){
			...
			//this.displayPromoMessage(catalogEntryId);
		}
		else{
	 ...
	 */	
	 displayPrice : function(catEntry){
		...
		if(document.getElementById('quickInfoPopup') && document.getElementById('quickInfoPopup').value=='Y')
			popup = true;

		if(popup == true){
			
			if( parseFloat(catEntry.offerPrice.split('$')[1]) < parseFloat(catEntry.listPrice.split('$')[1])){
				document.getElementById('listPrice').innerHTML = catEntry.listPrice;
				document.getElementById('listPrice').style.display = "inline";
				dojo.addClass(dojo.byId('productPrice')," newPrice");
				$('.saleSpanRed').text('Sale');
			}
			else{
				document.getElementById('listPrice').style.display = "none";
				dojo.removeClass(dojo.byId('productPrice')," newPrice");
				$('.saleSpanRed').empty()
			}
		...
		if(popup == false){
			...
			var innerHTML = "";
			if(catEntry.listPrice == "" ||(!catEntry.listPriced ||  parseFloat(catEntry.listPrice.split('$')[1]) <= parseFloat(catEntry.offerPrice.split('$')[1]))){
				innerHTML = "<span class='price'>" + catEntry.offerPrice + "</span>";
			}
			else{
				innerHTML = "<span class='price listPrice'>" + catEntry.listPrice + "</span>"+
							"<span class='price offerPriceRed'>"+" " + catEntry.offerPrice + "</span>";
							
			}
			
			if(categoryDisplayJS.displayPriceRange == true){
				for (var i=0;i<catEntry.priceRange.length;i++){
					if(catEntry.priceRange[i].endingNumberOfUnits != 'null'){
						...
						innerHTML = innerHTML + "<span class='price'>" + tempString + "</span>";
					}
					else{
	...
	**/
	getCatalogEntryId : function(){
		...
		for(attribute in this.selectedAttributes){
			//Parature Ticket 6809 - Comparison will be made to html escaped text.
			var text = this.selectedAttributes[attribute];
			attributeArray.push(attribute + "_" + $('<div />').text(text).html());
		}
	...
	**/
	getCatalogEntryIdforBundleProduct : function(productId, selectedAttributes){
		var attributeArray = [];
		for(attribute in selectedAttributes){
			if(!this.notNull(selectedAttributes[attribute]))
				return null;
			attributeArray.push(attribute + "_" + selectedAttributes[attribute]);
		}
	...
	**/
	resolveSKU : function(attributeArray){
		...
		var catentryInventoryAssociationJSONArray = {
			    catInv: []
		};
		...
		for(x in this.entitledItems){
			...
			var inventory = this.entitledItems[x].inventory;
			...
			if(attributeCount != 0 && attributeArrayCount >= attributeCount){
				...
				if(attributeCount == matchedAttributeCount){
					console.debug("CatEntryId:" + catentry_id + " for Attribute: " + attributeArray+ " inventory value === "+ inventory);
					// create a new json object array to store all entities that match the attribute criteria -- for RTC WI 19348 -- for 5.4 release 
					// each json object stores the matching catentry id and its corresponding inventory quantity
					catentryInventoryAssociationJSONArray.catInv.push({
				        "CatentryId" : catentry_id,
				        "Inventory": inventory
				    });
				}
			}
		}
		// return null when the json array is empty, meaning no matching catentry found
		if(catentryInventoryAssociationJSONArray.catInv.length == 0 )  {
			return null;
		}		
		//Finding catentry with minimum inventory from the obtained json array 
		var minInventory= catentryInventoryAssociationJSONArray.catInv[0].Inventory;
		var minInventorycatentryId= catentryInventoryAssociationJSONArray.catInv[0].CatentryId;
		for(var i = 0; i < catentryInventoryAssociationJSONArray.catInv.length; i++)  {
		    var obj = catentryInventoryAssociationJSONArray.catInv[i];
		    objectInventory = parseInt(obj.Inventory);
		    minInventoryNumber = parseInt(minInventory);
		    if(objectInventory != 0 && (objectInventory<=minInventoryNumber || minInventoryNumber == 0))  {
		    	minInventory=obj.Inventory;
		        minInventorycatentryId=obj.CatentryId;
		    }
		}
		console.debug("catentry with min inventory: "+minInventorycatentryId+" inventory quantity: "+minInventory);				
		return minInventorycatentryId;
	},
...
getDefaultItem : function(productId){
	...
	**/
	AddBundle2ShopCartAjax : function(form,itemNo){
			
			//clear the previous error message in the product
			//$('.bundleProductMessage').empty()
			...
			var itemsAdded = 0;
			var catEntryId = form["catEntryId_" + itemNo].value;
			var productId = catEntryId;					
			...
			var qty = form["quantity_" + itemNo].value;
			if(qty > 0){
				itemsAdded++;
				if(catEntryId!=null){
					...
					var sizeContainer = document.getElementById("sizeContainer_"+productId); 
					if(sizeContainer)
						MessageHelper.formErrorHandleClient(sizeContainer, MessageHelper.messages['ERR_RESOLVING_SKU']);
					else 
						MessageHelper.displayErrorMessage(MessageHelper.messages['ERR_RESOLVING_SKU']);
					
					//$("#bundleProductMessage_"+productId).html(MessageHelper.messages["BUNDLE_ATTENTION_MESSAGE"])
					return;
				}
				...
				if (contractIdElements != null && contractIdElements != "undefined") {
					for (j=0; j<contractIdElements.length; j++) {
						if (contractIdElements[j].checked) {
							form["contractId_" + itemNo].value = contractIdElements[j].value;
							break;
						}
					}
				}
				var contractId = form["contractId_" + itemNo].value;
				if (contractId != null && contractId != '') {
					this.updateParamObject(params,"contractId",contractId,false,-1);
				}
			}else
				$("#bundleProductMessage_"+productId).html(MessageHelper.messages["BUNDLE_ITEM_QTY_MESSAGE"])			
		if(itemsAdded > 0){
		if(!submitRequest()){
			return;
			}
	...
	/**
	 * This function is used to change the price displayed in the Product
	 * Display Page on change of a attribute of the product using an AJAX call.
	 * This function will resolve the catentryId using entitledItemId and
	 * displays the price of the catentryId.
	 * 
	 */
	updateInventory : function() {
		...
		if (catalogEntryId != null) {
			...
			// check if the json object is already present for the catEntry.
			if (this.itemInventoryJsonOject[catalogEntryId] != null && this.itemInventoryJsonOject[catalogEntryId] != 'undefined') {
				this.updateQuantity(this.itemInventoryJsonOject[catalogEntryId].catalogEntry);
				console.debug("CategoryDisplay.updateInventory: using stored json object.");
			}
			// if json object is not present, call the service to get the
			// details.
			else {
				var parameters = {};
				parameters.storeId = this.storeId;
				...
				$.ajax(getAbsoluteURL() + "GetCatalogEntryInventoryByID",
				{
					type:'POST',
					data:parameters,
					dataType:'json',
					...
					{
						if(data) categoryDisplayJS.handleInventoryServiceResponse(data);
						else console.debug("CategoryDisplay.updateInventory: Request completed successfully, but no data was returned.");
					},
					error:function()
					{
						console.debug("CategoryDisplay.updateInventory: Unexpected error occurred during an xhrPost request.");
					}
				});
			}

		} else {
			console.debug("CategoryDisplay.updateInventory: all attributes are not selected.");
		}
	 ...
	 */
	handleInventoryServiceResponse : function(serviceResponse,productId) {
	...
	**/
	Add2ShopCart : function(entitledItemId,form,quantity,isPopup){
		...
		if (dojo.byId(entitledItemId)!=null) {
			//the json object for entitled items are already in the HTML. 
			 entitledItemJSON = eval('('+ $("<div />").text(dojo.byId(entitledItemId).innerHTML).html() +')');
		}else{
			//if dojo.byId(entitledItemId) is null, that means there's no <div> in the HTML that contains the JSON object. 
			//in this case, it must have been set in catalogentryThumbnailDisplay.js when the quick info
			entitledItemJSON = this.getEntitledItemJsonObject(); 
	...
	*
	**/
	ReplaceItemAjax : function(entitledItemId,quantity, productName){
		...
		if(typeId != null && typeId != ""){
			...
			//to handle item updates in shipping page - modifying to add a different item of the same product, 
			//deleting and adding a new item should retain the shipping address and shipping method

			//Single Shipment..get the common addressId and shipModeId..
			if(document.getElementById("addressId_all"))
				addressId = document.getElementById("addressId_all").value;
			if(document.getElementById("shipModeId_all"))
				shipModeId = document.getElementById("shipModeId_all").value;
		}
		...
		if(catalogEntryId!=null){
			...
			{
				if(quantity > this.itemInventoryJsonOject[catalogEntryId].catalogEntry.inventory.quantity)
				{
					MessageHelper.formErrorHandleClient('replaceCartItemAjax', MessageHelper.messages["ITEM_LEVEL_OOS_MSG"]);
					return;
				}
			}
			...
			if(removeOrderItemId == ""){
				...
				*/
				...
				this.ReplaceItemAjaxHelper(catalogEntryId,quantity,removeOrderItemId,addressId,shipModeId, productName);
			}
		}
		else{
	
			MessageHelper.formErrorHandleClient('replaceCartItemAjax', MessageHelper.messages['ERR_RESOLVING_SKU']);
		}
	},
		...
		if(catalogEntryId!=null){
			if(removeOrderItemId == ""){
				...
				if(form.shipmentTypeId != null && form.shipmenTypeId != ""){
					...
					form.URL.value = "OrderChangeServiceItemAdd?calculationUsage=-1,-2,-5,-6,-7&catEntryId="+catalogEntryId+"&quantity="+quantity+"&addressId="+addressId+"&shipModeId="+shipModeId+"&URL=OrderChangeServiceShipInfoUpdate?URL="+form.URL.value;
			    }
				else{
					form.URL.value = "OrderChangeServiceItemAdd?calculationUsage=-1,-2,-5,-6,-7&catEntryId="+catalogEntryId+"&quantity="+quantity+"&URL="+form.URL.value;
				}
	...
	**/
	AddItem2ShopCartAjax : function(catEntryIdentifier, quantity, customParams)
	{
		...
		if(dojo.isArray(catEntryIdentifier) && dojo.isArray(quantity)){
			...
			if(customParams['singleSKUPopUp'] == 'true'  && this.selectedAttributes.Size == ""){
				MessageHelper.formErrorHandleClient('addToCartLinkAjax', MessageHelper.messages['ERR_RESOLVING_SKU']);
				return;
			}
		}
	...
	*
	**/
	ReplaceItemAjaxHelper : function(catalogEntryId,qty,removeOrderItemId,addressId,shipModeId, productName){
		...
		if(addressId != null && addressId != "" && shipModeId != null && shipModeId != ""){
			...
			var fromSBPage = false;
			if(document.getElementById("fromSBPage")){
				 fromSBPage = document.getElementById("fromSBPage").value;
				 if(fromSBPage){
					 	params3.calculationUsage = "-1,-2,-3,-4,-5,-6,-7";
						params2.calculationUsage = "-1,-2,-3,-4,-5,-6,-7";
						params2.addressId = addressId;
						params2.shipModeId = shipModeId;
				 }
			}		
		...
		wc.service.declare({
			...
			,failureHandler: function(serviceResponse) {
				...
				if(serviceResponse.errorMessageKey == '_ERR_INVENTORY_NOT_AVAILABLE') {
					...
					if(y == 1){
						document.location.href = "OrderShippingBillingView?storeId=" + ServicesDeclarationJS.storeId + "&catalogId=" + ServicesDeclarationJS.catalogId + "&langId=" + ServicesDeclarationJS.langId + "&orderId=" + serviceResponse.orderId+"&prdRmd="+escape(productName);
					} else {
	...
	*
	**/
	AddBundle2WishListAjax : function(form,itemNo){
		...
		if(typeof THProduct!=THUtil.UNDEFINED) THProduct.removeWishlistListeners(productId);
	},
	...
	*
	**/
	Add2WishListAjaxByID:function(catalogEntryId, isPopUp)
	{
		if(catalogEntryId!=null){
			...
			if(typeof THProduct!=THUtil.UNDEFINED) THProduct.removeWishlistListeners();
		}
		else if (isPopUp == true){
			MessageHelper.formErrorHandleClient('addToCartLinkAjax', MessageHelper.messages['ERR_RESOLVING_SKU']);
		} else{
	...
	*
	**/
	Add2WishListAjax:function(entitledItemId, isPopUp)
	{
		...
		this.Add2WishListAjaxByID(catalogEntryId, isPopUp);
...
var url = "AjaxOrderItemDisplayView?storeId="+this.storeId+"&catalogId="+this.catalogId+"&langId="+this.langId;
						...
						+"<input type='hidden' value='-1,-2,-5,-6,-7' name='calculationUsage' id='OrderItemAddForm_calcUsage_"+this.merchandisingAssociationItems[0].catEntry_Identifier+"'/>\n"
...
setOrderId : function(orderId)
{
	...
	**/
	...
	showAndHideOverlay:function(divId){	
	...
	displayPromoMessage:function (catalogEntryId) {
		
		for(x in this.entitledItems){
			var catentry_id = this.entitledItems[x].catentry_id;
			if(catalogEntryId == catentry_id){
				var Attributes = this.entitledItems[x].DescriptiveAttributes;
				var attributeCount = 0;
				for(index in Attributes){
					attributeCount ++;
				}
				...
				if(attributeCount > 0){
					var promoMessage = '';
					if(Attributes["GenericPromotion1"] != undefined) {
						promoMessage = Attributes["GenericPromotion1"];
					
					} else if(Attributes["GenericPromotion2"] != undefined){
						promoMessage = Attributes["GenericPromotion2"];
					
					} else if(Attributes["GenericPromotion3"] != undefined) {
						promoMessage = Attributes["GenericPromotion3"];
					}
					console.debug("Promotion message :::"+promoMessage+":::");
					dojo.byId('prodPromoTagline1').innerHTML = promoMessage;
					dojo.byId('prodPromoTagline2').innerHTML = promoMessage;
	...
	/**
	*  
	* Shows Newsletter signup overlay in Home page if the cookie is not set.
	*
	**/
	showSignupForm:function() {

		if(!dojo.cookie('newsletter')) {
			
			dojo.cookie('newsletter', 'true',{path:'/', expires:this.newsletterCookieExpiry});
			
			parseWidget("emailCapture");
			dijit.byId('emailCapture').closeButtonNode.style.display='none';
			closeAllDialogs(); // close all dijit.dialogs first
			dijit.byId('emailCapture').show();
			
			// the component that grays out the screen behind,
	...
	**/
	signupForNewsletter: function (form, id){
		...
		if(!MessageHelper.isValidEmail(signUpEmail.value)){
			MessageHelper.formErrorHandleClient(signUpEmail, MessageHelper.messages["ERROR_INVALIDEMAILFORMAT"]);
			return false;
		} 
	},
	
	fetchProductNavLinks: function (storeId, langId, catalogId, categoryId, productId, parent_category_rn, top_category, catentryType) {
		var parameters = {};
		...
		parameters.top_category=top_category;
		parameters.catentryType=catentryType;
		
		dojo.publish("ajaxRequestInitiated");
		dojo.xhrPost({
				url: getAbsoluteURL() + "GetPrdNavURLsByCatgrpAndCatentry",				
				handleAs: "json-comment-filtered",
				content: parameters,
				service: this,
				load: this.updateProductNavigation,
				error: function(errObj,ioArgs) {
					console.debug("CategoryDisplay.fetchProductNavLinks: Unexpected error occurred during an xhrPost request.");
	...
	updateProductNavigation: function (serviceResponse, ioArgs) {
		
		if(serviceResponse.categoryName != '' && serviceResponse.searchLink != '') {
			if(dojo.byId('productCatLink')!=null){
			dojo.byId('productCatLink').innerHTML = "<a href='"+ serviceResponse.searchLink +"' class='prodNavLinks'>Back To "+ serviceResponse.categoryName +"</a>";
			$('.category_name_display').text(serviceResponse.categoryName);
			}
		}
		if(serviceResponse.prevLink != '' && serviceResponse.nextLink == '') {
			dojo.byId('productNavLink').innerHTML = "<a href='"+ serviceResponse.prevLink +"' class='prodNavLinks previous'>Previous</a>";
		} else if (serviceResponse.prevLink == '' && serviceResponse.nextLink != '') {
			dojo.byId('productNavLink').innerHTML = "<a href='"+ serviceResponse.nextLink +"' class='prodNavLinks next'>Next</a>";
		} else if (serviceResponse.prevLink != '' && serviceResponse.nextLink != '') {
			dojo.byId('productNavLink').innerHTML = "<a href='"+ serviceResponse.prevLink +"' class='prodNavLinks previous'>Previous</a>" +
				"<a href='"+ serviceResponse.nextLink +"' class='prodNavLinks'>Next</a>";
		}
	},
	
	/**
	 * This function initiates PayPal checkout from Mini Cart/Shopping Cart 
	 *
	 * @param orderTotal
	 */
	initiatePayPalCheckout:function(orderTotal) {
