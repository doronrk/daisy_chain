       ...
       ,modelChangedHandler: function(message, widget) {
              ...
              if(message.actionId in order_updated || message.actionId == 'AjaxDeleteOrderItemForShippingBillingPage'){
                     ...
                     if(message.actionId == 'AjaxAddOrderItem'){
                            ...
                            CommonControllersDeclarationJS.orderAddedId = message.orderItemId + "";
                            showDropdown = true;
                     }
...
wc.render.declareRefreshController({
       ...
       ,modelChangedHandler: function(message, widget) {
              ...
              {
            	  	$('#cart_rr').addClass('hidden').appendTo(TH.$body); //JP: Removed populated recommendations before refresh occurs - SEE BELOW for re-injection
                    widget.refresh(renderContext.properties);
              }
       }
       ...
       ,postRefreshHandler: function(widget) {
           ...
           var renderContext = this.renderContext;
	         ...
	         if (typeof(savedOrdersJS) != null && typeof(savedOrdersJS) != 'undefined')
	         {
			  ...
			  cursor_clear();
			  //force refresh shoprunner messages
			  sr_updateMessages();
			  
			  //JP: Move the recommendations back into place after the refresh has occurred
			  if(THUtil.isMobile()) $('#shoppingBagColumn2 #cart_rr').replaceWith(TH.$body.children('#cart_rr').removeClass('hidden'));
			  else $('#content_wrapper #cart_rr').replaceWith(TH.$body.children('#cart_rr').removeClass('hidden'));
			  
			  
			  //Initialize DOMContentLoaded event to make sure that Google Wallet button displays after refresh area is 
			  //refreshed.
...
/** 
 * Declares a new refresh controller for Tealium tagging purpose.
 */
wc.render.declareRefreshController({
       id: "TealiumTaggingController",
       renderContext: wc.render.getContextById("TealiumTagging_Context"),
       url: "TealiumTaggingView",
       formId: ""
    
    ,modelChangedHandler: function(message, widget) {
           var controller = this;
           
           console.debug('model changed.............................'+message.actionId);
           
           var renderContext = this.renderContext;
           
           if (message.actionId == 'AjaxInterestItemAdd' || message.actionId == 'AjaxAddOrderItem') {
        	  
        	  var param = [];
        	  param.actionId = message.actionId;
        	  
        	  if(message.catEntryId != undefined)
        		  param.itemId = message.catEntryId;
        	  
        	  else if (message.orderItemId != undefined)
        		  param.itemId = message.orderItemId;
...
/** 
 * Declares a new refresh controller search results
 */
...
wc.render.declareRefreshController({
	...
	,postRefreshHandler: function(widget) {
			...
			console.debug("in post refreshscript after calling searchResultCallBack url, viewAll = " + controller.renderContext.properties["viewAll"]);

			var restrictedResultMsgDiv = document.getElementById("restrictedResultMsgDiv");
			if(restrictedResultMsgDiv != null){
				if(controller.renderContext.properties["viewAll"] != 'Y'){
					restrictedResultMsgDiv.style.display = 'none';
				}else{
					restrictedResultMsgDiv.style.display = 'inline';
					controller.renderContext.properties["viewAll"] = 'N';
				}
			}

			if(controller.renderContext.properties["viewAll"] == 'Y'){
				CatalogSearchDisplayJS.updateSortOption();
			}
