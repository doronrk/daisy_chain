 ...
 */
function clearSearchField(searchTerm) {
  searchText = document.getElementById("searchTermByUser").value;
  if(searchText == document.getElementById("searchTextHolder").innerHTML){
    document.getElementById("searchTermByUser").value = "";
  }
  else{

    if(searchTerm != 'Enter Keyword or item #'){
...
console.debug('clearThisField:: field.value::' +  field.value)
  text = field.value;
...
function fillSearchField() {
  if (document.getElementById("searchTermByUser").value == "") {
    document.getElementById("searchTermByUser").value = 'Enter Keyword or item #';
  }
...
function submitIBMSearchForm(form, enterkey) {
  var loc = location.href;

  //If page is secure, switch action to secure.  Dont do anything if in the "preview" context
//  if(loc.indexOf('https://') >=0 && loc.indexOf('/preview/') <0){
//    var action_url = form.action;
//
//    if(action_url.indexOf('https://') <=0){
//      form.action = action_url.replace('http://', 'https://');
//    }
//  }
  ...
  if(searchTerm == null || searchTerm == "" || searchTerm.indexOf("Enter Keyword")  >= 0){
    if(enterkey) return false;
  }else{
    searchTermArr = searchTerm.toLowerCase().split(" ");
    var temp="";
    var tempCount =  searchTermArr.length;

    for(var i=0; i<tempCount; i++)
    {
      if(temp != null && temp != "")
        temp=temp+" "+searchTermArr[i];
      else
        temp=searchTermArr[i];
    }

    temp = trim(temp);

    form.searchTerm.value = temp;

    var searchHistoryCookie = dojo.cookie("searchTermHistory");

    if(searchHistoryCookie != null && searchHistoryCookie != 'undefined' && typeof(searchHistoryCookie) != 'undefined') {
      var sTermLower = searchTerm.toLowerCase();
      var sDupe = false;
      ...
      for(var i = termsArray.length - 1; i > 0; i--) {
        ...
        if(theLowerTerm == sTermLower){
          sDupe = true;
        }
      }
      if(!sDupe){
        searchHistoryCookie = searchHistoryCookie + "|" + sTermLower;
        dojo.cookie("searchTermHistory", searchHistoryCookie, {expires: 365});
      }

    }else{
      var sTermLower = "|" + searchTerm.toLowerCase();
      dojo.cookie("searchTermHistory", sTermLower, {expires: 365});
    }

    disableButton("searchBtn");

    if(enterkey) return true;
    else form.submit();
  }
...
function doDynamicAutoSuggest(url, searchTerm, showHeader) {
  ...
  if(searchHistoryCookie != null && typeof(searchHistoryCookie) != 'undefined') {
    ...
    for(var i = termsArray.length - 1; i > 0; i--) {
      ...
      historyList = historyList + "<li id='suggestionItem_" + listCount + "' role='listitem' tabindex='-1'><a href='#' onmouseout='this.className=\"\"' onmouseover='enableAutoSelect(" + listCount + ");' onclick='selectAutoSuggest(this.title); return false;' title=\"" + theTerm + "\" id='autoSelectOption_" + listCount+ "'><strong>" + theTerm + "</strong></a></li>";
  ...
  //alert(("AutoSuggestDisplayController").url = url + "&term=" + escape(searchTerm) + "&showHeader=" + showHeader);
...
function showAutoSuggestIfResults() {
  ...
  if(document.getElementById("searchTermByUser").value.length <= AUTOSUGGEST_THRESHOLD) {
    showAutoSuggest(false);
  }
  else {
...
function selectAutoSuggest(term) {
  var searchBox = document.getElementById("searchTermByUser");
...
function retrieveCachedSuggestions(url, searchTerm) {
  if(searchTerm != 'Enter Keyword or item #'){
...
function updateBraFinder(field){
  //alert('value = ' + value);
  document.location = field.value;
}
function updateBraFinderFromButton(){
  //alert('value = ' + value);
  document.location = document.getElementById("facet-Size").value;
}

function updateRefinements(field){
    //alert('value = ' + value);
    document.location = field.value;
}

/**
 * This function expand/collapse the facets when the number of returned
 * facets greater/less than maximum number of allowed displayable facets
 * @element expand/collapse html element
 * @maxDisplayedFacets {integer} A number represents maximum number of allowed displayable facets
 * @collapseText {String} A Text displayed to indicate that there is element(s) can be collapsed
 * @expandText {String} A Text displayed to indicate that there is element(s) can be expanded
 * @collapseCSSStyle {String} a CSS class name that hides the expanded elements
 * @expandCSSStyle {String} a CSS class name that display the collapsed elements
 */
function expandCollapseSearchFacets (element, maxDisplayedFacets, collapseText, expandText, collapseCSSStyle, expandCSSStyle) {
  var facetsList = element.parentElement;
  if(facetsList != null) {

    if(element.innerHTML.indexOf(collapseText) > -1) {
      for( var i = maxDisplayedFacets; i < facetsList.children.length - 1; i++ ) {
        facetElement = facetsList.children[i];
        facetElement.className = collapseCSSStyle;
      }

      element.innerHTML = element.innerHTML.replace(collapseText, expandText);
    }else if (element.innerHTML.indexOf(expandText) > -1) { //Display the non-displayed facets.
      for( var i = maxDisplayedFacets; i < facetsList.children.length - 1; i++ ) {
        facetElement = facetsList.children[i];
        facetElement.className = expandCSSStyle;
      }

      element.innerHTML = element.innerHTML.replace(expandText, collapseText);
    }
  }
