 ...
 */
	 ...
	 */
		showHideMessageArea:function(topOffset){
			...
			fadeOutAnimArgsArray["onEnd"] = function(){
				...
				$jq('.frg_lightbox').addClass('frg_hidden');
			};
			
			// set message area to alpha and then make it display block
			dojo.style(node, "opacity", 0);
			dojo.style(node, "display", "block");
			$jq('.frg_lightbox').removeClass('frg_hidden');
     ...
     */
	     ...
	     * @param (int) topOffset how far from the top the message display area will be displayed. 
	     * @param (boolean) showType whether or not the message type should be appended to the actual message
	     * @param (string) title The message title to be displayed
	     * 
	     * @return (element) a HTML element that contains the error message. 
	     *
	     */
		
		frgDisplayErrorMessage:function(msg, topOffset,showType,title){	
			
			if (title) {
				dojo.byId('dialogTitleMessage').innerHTML = title;
			}
			
			this.displayErrorMessage(msg, topOffset,showType);
		...
		formErrorHandleClient:function(id,errorMessage){
			...
			if(element){
				if (this.identifier != (id + "_tooltip")) {
					...
					/* CUSTOM 
					 * @autor caugelli
					 * Create element <small> in case of validate error
					 * */
					if (id != null && id.type=='select-one') {
						$jq('#'+ id.id).attr('class', 'frg_error');
						$jq("label[for='" + id.id + "']").attr('class', 'frg_error');
						$jq('#'+ id.id).after('<small class="frg_error_caption">' + errorMessage + '</small>');
					} else if (id != null && id.type == 'checkbox'){
						$jq('#'+ id.id).attr('class', 'frg_error');
						/*id.parentNode.parentNode.id.attr('class', 'frg_error');*/
						$jq("label[for='" + id.id + "']").after('<small class="frg_error_caption">' + errorMessage + '</small>');
					} else {
						//$jq('#'+ id).attr('class', 'frg_error');
						$jq('#'+ id).addClass('frg_error');
						$jq("label[for='" + id + "']").attr('class', 'frg_error');
						$jq('#'+ id).after('<div class="clear"></div><small class="frg_error_caption">' + errorMessage + '</small>');
					}
     ...
     */
		...
		IsValidFiscalCode: function (cf, storeIdentifier) {
			if (storeIdentifier == 'Ferragamo_MX') {
				// il codice fiscale è valido se è lungo 0 o se, qualora sia > 0, è 12 o 13 caratteri. 
				return cf.length == 0 || ( cf.length >0 && (cf.length == 12 || cf.length == 13) );
				
			} else if (storeIdentifier == 'Ferragamo_IT') {
				
				var checkCF=cf.match(/[A-Za-z]{6}[0-9]{2}[A-Za-z]{1}[0-9]{2}[A-Za-z]{1}[0-9]{3}[A-Za-z]{1}/);
				if(checkCF==null || cf.length!=16) 
				{
					return false;
				}
				return true;
				
			} else if (storeIdentifier == 'Ferragamo_ES') {
				
				var checkCF=cf.match(/[yYxX0-9]{1}[0-9]{7}[A-Za-z]{1}/);
				if(checkCF==null || cf.length!=9) 
				{
			...
			} else if (storeIdentifier == 'Ferragamo_US') {
