//----------------------------------------------------------------
// Defining custom function to search product 
// whith one single <input> 
// @autor Augelli Cristiana
// 
//-----------------------------------------------------------------

if(typeof(CustomSearchJS) == "undefined" || CustomSearchJS == null || !CustomSearchJS){
	...
	CustomSearchJS = { 
		...
		*/
		...
		init:function(){
			...
			dojo.connect(dojo.byId("SimpleSearchForm_SearchTerm"), "onclick", CustomSearchJS, CustomSearchJS._onClick);
			...
			if(typeof(searchHistoryCookie) != 'undefined') {
				...
				for(var i = termsArray.length - 1; i > 0; i--) {
					...
					if(theLowerTerm.match("^"+searchTermLower) == searchTermLower) {
						...
						if(!repeatedTerm) {
							historyList = historyList + "<li id='suggestionItem_" + listCount + "' role='listitem' tabindex='-1'><a href='#' onmouseout='this.className=\"\"' onmouseover='CustomSearchJS.enableAutoSelect(" + listCount + ");' onclick='CustomSearchJS.selectAutoSuggest(this.title); return false;' title=\"" + theTerm + "\" id='autoSelectOption_" + listCount+ "'><strong>" + searchTerm + "</strong>" + theTerm.substring(searchTerm.length, theTerm.length) + "</a></li>";
	...
	wc.render.declareRefreshController({
	   ...
	   ,postRefreshHandler: function(widget) {
			  ...
			  if(response == null) {
					 ...
					 CustomSearchJS.retrievedCachedSuggestions = true;
					 var searchTerm = document.getElementById("SimpleSearchForm_SearchTerm").value;
					 if(searchTerm.length > CustomSearchJS.AUTOSUGGEST_THRESHOLD) {
							CustomSearchJS.doStaticAutoSuggest(searchTerm);
					 }
			  }
	   }
	});
	...
	wc.render.declareRefreshController({
		   ...
		   ,postRefreshHandler: function(widget) {
				  ...
				  CustomSearchJS.showAutoSuggestIfResults();
		   }
	});
