...
function cursor_wait(checkForOpera) {
	...
	//@autor caugelli
	//Custom visualize lightbox onload content
	$jq('#loading').removeClass('frg_hidden');
...
function cursor_clear() {
		...
		//@autor caugelli
		//Custom visualize lightbox onload content
		$jq('#loading').addClass('frg_hidden');
}	
...
function parseWidget(id)
{
    ...
    console.debug("parseWidget - Start parsing: " + id);
    
	var node;
	var widget = dijit.byId(id);
	
    console.debug("parseWidget - Found widget is: " + widget);
	...
	if (widget == null || widget == undefined)
	{
		...
		console.debug("parseWidget - Found node: " + node);
		
		if (node != null && node != undefined)
		{
			if (node.getAttribute("dojoType") != null && node.getAttribute("dojoType") != undefined)
			{
        		console.debug("parseWidget - parsing dojoType");
				dojo.parser.instantiate([node]);
			}
			else
			{
        		console.debug("parseWidget - parsing NOT dojoType");
				dojo.parser.parse(node);
			}
		}
	}
	
	console.debug("parseWidget - Finished parsing: " + id);
}
...
function addToWidgetsList(widgetId){
	if(!contains(widgetsList, widgetId))
...
/**
 * Set requestSubmitted to false
 */
function cursor_clear_new() {
...
function logOffClicked(url){
	
	var r = confirm(MessageHelper.messages['FRG_ARE_YOU_SURE_TO_LOGOUT']);
