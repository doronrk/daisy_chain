...
shoppingActionsJS={
	...
	**/
	...
	setProductPageConfiguration: function(config){
	 ...
	 */
	isDisplayPriceEnabled : function(){

		 if(this.productPageConfiguration == null)
			 return false;
		 
		 return this.productPageConfiguration.getBoolean('productpage.feature.displayprice.enable');
	 ...
	 * Check whether the feature is enabled based on the loaded configuration.
	 */
	isInventoryStatusCheckEnabled : function(){
	...
	**/
	resolveSKU : function(attributeArray){
		...
		if(this.entitledItems.length == 1){
			// if there only one item, add2cart is enabled
			shoppingActionsExt.toggleAdd2Cart(true);
	...
	**/
	setSelectedAttribute : function(selectedAttributeName , selectedAttributeValue, entitledItemId, skuImageId, imageField){ 
		...
		// custom filtering on attributes
		shoppingActionsExt.fill(shoppingActionsExt.filter(selectedAttributeName,selectedAttributeValue));
	},
	...
	**/
	...
	isTaxCodeEmpty : function (entitledItemId){
	
		var entitledItemJSON;
		
		// if no item is available
		if(!shoppingActionsExt.isProductAvailable(entitledItemId)){
			MessageHelper.hideAndClearMessage();
			MessageHelper.displayErrorMessage(MessageHelper.messages['FRG_PD_ADD_TO_CART_NOTAVAILABLE'] );
			return;
		}

		if (dojo.byId(entitledItemId)!=null && dojo.byId(entitledItemId)!=undefined) {
			//the json object for entitled items are already in the HTML. 
			var ih = dojo.byId(entitledItemId).innerHTML;
		...
		// if no item has been selected, show popup
		if(!shoppingActionsExt.isAdd2CartEnabled()){
			MessageHelper.hideAndClearMessage();
			MessageHelper.displayErrorMessage(MessageHelper.messages['FRG_PD_ADD_TO_CART_NOITEMSELECTED'] );
			return;
		}
		
		var jsonString = document.getElementById('skusJSON').innerHTML;
		var jsonData = JSON.parse(jsonString);
		if (jsonData[catalogEntryId].fields['field5'] == null || jsonData[catalogEntryId].fields['field5'] == ""){
			MessageHelper.displayErrorMessage(MessageHelper.messages['FRG_EMPTY_TAX_CODE_MSG'] );
			return true;
		}
		return false;
	},
	
	isTaxCodeEmptyWL : function (catalogEntryId){
				
		var jsonString = document.getElementById('skusJSON_'+catalogEntryId).innerHTML;
		var jsonData = JSON.parse(jsonString);
		if (jsonData[catalogEntryId].fields['field5'] == null || jsonData[catalogEntryId].fields['field5'] == ""){
			MessageHelper.displayErrorMessage(MessageHelper.messages['FRG_EMPTY_TAX_CODE_MSG'] );
			return true;
	...
	**/
	...
	/**
	 * Add2FiammaShopCartAjax This function is used to add a catalog entry to the shopping cart using an AJAX call.
	 * 						If 'customerCode' is empty call the standard Add2ShopCartAjax, otherwise do an add2cart form Fiamma product.
	 * 
	 * @param {String} catEntryId Variable to find the entitledItemId DIV containing a JSON object which holds information about a catalog entry. You can reference CachedProductOnlyDisplay.jsp to see how that div is constructed.
	 * @param {int} quantity The quantity of the item to add to the cart.
	 * @param {String} customerCode Code inserted by customer to customize the product of type Fiamma
	 * @param {String} isPopup If the value is true, then this implies that the function was called from a quick info pop-up. 	
	 * @param {Object} customParams - Any additional parameters that needs to be passed during service invocation.
	 */
	Add2FiammaShopCartAjax : function(catEntryId,quantity,customerCode,isPopup,customParams)
	{
		if (customerCode == undefined || customerCode == null || customerCode == ""){
			//non ci sono le iniziali quindi richiamo l'add2bag standard
			this.Add2ShopCartAjax('entitledItem_'+catEntryId,quantity,isPopup,customParams);
		} else if ($jq("#fiammaForm").valid()){
			var entitledItemSKUId = "entitledItemSKU_"+catEntryId;
			var entitledItemSKUJSON;
	
			// if no item is available
			if(!shoppingActionsExt.isProductAvailable(entitledItemSKUId)){
				MessageHelper.hideAndClearMessage();
				MessageHelper.displayErrorMessage(MessageHelper.messages['FRG_PD_ADD_TO_CART_NOTAVAILABLE']);
				return;
			}
			
			if (dojo.byId(entitledItemSKUId)!=null && dojo.byId(entitledItemSKUId)!=undefined) {
				//the json object for entitled items are already in the HTML. 
				var ih = dojo.byId(entitledItemSKUId).innerHTML;
				entitledItemSKUJSON = eval('('+ ih.toString() +')');
			}
			//else{
				//if dojo.byId(entitledItemSKUId) is null, that means there's no <div> in the HTML that contains the JSON object. 
				//in this case, it must have been set in catalogentryThumbnailDisplay.js when the quick info
			//	entitledItemSKUJSON = this.getEntitledItemJsonObject(); 
			//}
			
			//this.setEntitledItems(entitledItemSKUJSON);
			//var catalogEntryId = this.getCatalogEntryId(entitledItemSKUId);
			
			var partnumber = "";
			for(x in entitledItemSKUJSON){
				partnumber = entitledItemSKUJSON[x].PartNumber;
			}
			categoryDisplayJS.setCommonParameters(this.langId,this.storeId,this.catalogId,this.userType);
			categoryDisplayJS.AddItem2FiammaShopCartAjax(partnumber,quantity,customerCode);
		} else {
			//formato iniziali non valido
			MessageHelper.displayErrorMessage(FiammaCustomizeJS.ERR_MSG_INVALIDFORMAT);
		}
	},
	
	Add2ShopCartAjaxMy : function(partNumber,quantity,isPopup,customParams,config_name)
	{	
		
		if(partNumber!=null){
			this.AddItem2ShopCartAjaxMy(partNumber,quantity,customParams,config_name);
			this.baseItemAddedToCart=true;
			if(dijit.byId('second_level_category_popup') != null){
				hidePopup('second_level_category_popup');
			}
	...
	SetCurrencyPreference : function(ean,configuration,currency)
	{	
		var params = [];
		params.currency	= currency;
		params.langId = this.langId;
		params.URL = "ConfiguratorAsiaView?ean="+ean+"&configuration="+configuration;
		
		wc.service.invoke("AjaxSetCurrencyPreference", params);
		
	},
	
	
	
	AddItem2ShopCartAjaxMy : function(partNumber, quantity, config_name)
	{
		var params = [];
		...
		params.description = config_name;
		
		
		var ajaxShopCartService = "AddOrderItemMy";
		
		if(!isPositiveInteger(quantity)){
			alert(storeNLS['QUANTITY_INPUT_ERROR']);
			return;
		}
	...
	AddItem2ShopCartAjax : function(catEntryIdentifier, quantity, customParams)
	{
		...
		if(customParams != null && customParams != 'undefined'){
			for(i in customParams){
				...
				//custom
				if (params["customerCode"] != null && params["customerCode"] != 'undefined') {
					ajaxShopCartService = "AjaxAddOrderFiammaItem";
				}
				//custom
			...
			for (x in this.entitledItems) {
				...
				for(y in Attributes){
					...
					if (entitledSwatchName == swatchAttrName && entitledSwatchValue == swatchAttrValue) {
						...
						// CUSTOM check needed in order to avoid loading images for items that does not have any image
						if(null != dojo.byId(this.skuImageId) && itemImage != null && itemImage != undefined){
	...
	**/
	changePrice : function(entitledItemId,isPopup,displayPriceRange,productId){
		...
		if(catalogEntryId!=null){
			...
			if(!this.isInventoryStatusCheckEnabled())
				shoppingActionsExt.toggleAdd2Cart(false);
		}
