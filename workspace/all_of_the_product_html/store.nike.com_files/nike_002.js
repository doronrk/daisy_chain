...
},FlashBridge:b,getImageRenditionUrl:function(f,h){f=f||"";
h=h||{};
var g=h.width,e=h.height,j=h.quality,i=h.format||f.substr(f.lastIndexOf(".")+1),d=[f,".dimg/"];
if(g){d.push(g)
}d.push("x");
if(e){d.push(e)
}if(j){d.push("q");
...
d.push(i);
return d.join("")
},validateInternalUrl:function(d){if(d.toLowerCase().substring(0,7).indexOf("http://")==-1){if(d.toLowerCase().substring(d.length-5).indexOf(".html")==-1){d=d+".html"
}}return d
...
if(isAnchor&&thisTarget!="_blank"&&thisHref!="#"&&!thisHref.match(currentPath+"#")){var configFollowLink=dataConfig.followLink;
var dataAttributeFollowLink=$(this).data("followLink");
if(typeof dataAttributeFollowLink==="undefined"){if(typeof configFollowLink==="undefined"||configFollowLink==true){window.setTimeout(function(){window.location.href=event.currentTarget.href
},300)
}}else{if(dataAttributeFollowLink){window.setTimeout(function(){window.location.href=event.currentTarget.href
},300)
...
var d=function(o){var o=c.extend({playerId:390534381001,videoId:""},o);
o.playerId=(/iPhone|iPod|iPad/i).test(navigator.userAgent)?713160923001:o.playerId;
...
nike.cq.define("nike.cq.UI.Component.VideoPlayer",["jquery","nike.cq.Content","nike.cq.Content.Component.VideoPlayer","nike.cq.Content.Component.VideoPlayerType","nike.cq.Utils"],function($,Content,VideoPlayerContent,VideoPlayerType,Utils){var playerRef=null,experienceModuleRef=null,videoPlayerRef=null,lastBrightCoveId=null,onVideoTemplateLoaded=function(id){lastBrightCoveId=id;
playerRef=brightcove.getExperience(id);
experienceModuleRef=playerRef.getModule(APIModules.EXPERIENCE);
videoPlayerRef=playerRef.getModule(APIModules.VIDEO_PLAYER)
};
var component=nike.cq.ns("nike.cq.UI.Component");
component.onVideoTemplateLoaded=onVideoTemplateLoaded;
return function(elementOrConfig){var pseudoUniqueId="nike-cq-video-player-component-"+new Date().getTime()+Math.floor(Math.random(1000)),isElement=!$.isPlainObject(elementOrConfig),element=(isElement&&elementOrConfig?$(elementOrConfig):elementOrConfig&&elementOrConfig.element?$(elementOrConfig.element):$("<div>",{"class":"nike-cq-video-player-component"})).attr("id",pseudoUniqueId),config=$.extend(true,{data:{},onInit:function(){},onLoad:function(){},onRemove:function(){}},!isElement?elementOrConfig:{}),dataLength=(function(){var length=0;
for(var dataItem in config.data){length++
}return length
})(),videoPlayer=new VideoPlayerContent(dataLength>0?config.data:element),BaseVideoPlayer=function(config){this.element=null;
this.objectTag=$("<div>");
this.objectTagId="";
this.isValid=typeof(config.isValid)=="function"?config.isValid:function(){return false
};
...
this.generateControls=function(config){config=$.extend({appendTo:$("<div>"),autoPlay:false,defaultWidth:"100%",onPlayEvent:"onVideoPlaying",onPauseEvent:"onVideoPaused",registerEvent:function(eventName,callback){},onReady:function(){},play:function(){},pause:function(){},stop:function(){},seek:function(){},getTotalTime:function(){},getCurrentPosition:function(){}},config);
var element=config.appendTo,playText="Play",pauseText="Pause",stopText="Stop";
var playPause=$("<button>",{"class":"video-playback-control-playpause "+(config.autoPlay?"video-playback-control-pause":"video-playback-control-play")}),stop=$("<button>",{"class":"video-playback-control-stop"}),playhead=$("<span>",{"class":"video-playback-control-playhead",css:{position:"absolute",left:0}}),progress=$("<span>",{"class":"video-playback-control-progress",css:{position:"absolute"}}).append(playhead),progressContainer=$("<span>",{"class":"video-playback-control-progress-container",css:{position:"absolute",cursor:"hand"}}).append(progress),currentTime=$("<span>",{"class":"video-playback-control-current-time",text:"0:00"}),timeDivider=$("<span>",{"class":"video-playback-control-time-divider",text:"/"}),totalTime=$("<span>",{"class":"video-playback-control-total-time",text:"0:00"}),timeContainer=$("<span>",{"class":"video-playback-control-time-container"}).append(currentTime).append(timeDivider).append(totalTime),container=$("<div>",{"class":"video-playback-controls",css:{width:config.defaultWidth}}).append(playPause).append(stop).append(progressContainer).append(timeContainer);
this.controlsElement=container;
element.append(container);
element.delegate(".video-playback-control-pause","click",function(e){e.preventDefault();
config.pause()
}).delegate(".video-playback-control-play","click",function(e){e.preventDefault();
config.play()
}).delegate(".video-playback-control-stop","click",function(e){e.preventDefault();
config.stop()
}).delegate(".video-playback-control-progress-container","click",function(e){e.preventDefault();
playhead.css("left",e.offsetX);
var jumpTo=Math.round(config.getTotalTime()*(e.offsetX/$(this).width()));
config.seek(jumpTo)
});
config.onReady(function(){config.registerEvent(config.onPlayEvent,function(){playPause.removeClass("video-playback-control-play").addClass("video-playback-control-pause").text(pauseText)
});
config.registerEvent(config.onPauseEvent,function(){playPause.removeClass("video-playback-control-pause").addClass("video-playback-control-play").text(playText)
});
var isTotalTimeSet=false,totalTimeVal=0,lastCurrentTime=0;
var interval=window.setInterval(function(){if(!isTotalTimeSet){totalTimeVal=config.getTotalTime()/60;
var totalTimeMinutes=Math.floor(totalTimeVal),totalTimeSeconds=Math.round((totalTimeVal-totalTimeMinutes)*60),totalTimeSeconds=totalTimeSeconds<10&&totalTimeSeconds>=0?"0"+totalTimeSeconds:totalTimeSeconds;
if(totalTimeVal>0){totalTime.text(totalTimeMinutes+":"+totalTimeSeconds);
isTotalTimeSet=true
}}var currentTimeVal=config.getCurrentPosition()/60,currentTimeMinutes=Math.floor(currentTimeVal),currentTimeSeconds=Math.round((currentTimeVal-currentTimeMinutes)*60),currentTimeSeconds=currentTimeSeconds<10&&currentTimeSeconds>=0?"0"+currentTimeSeconds:currentTimeSeconds;
if(lastCurrentTime!=currentTimeVal){currentTime.text(currentTimeMinutes+":"+currentTimeSeconds);
...
return this
},BrightcoveVideoPlayer=new BaseVideoPlayer({isValid:function(data){return data.getVideoId()!=""&&data.getPlayerId()!=""
},generate:function(data,element,onLoad){var pseudoUniqueObjectId="nike-cq-video-player-component-"+new Date().getTime()+Math.floor(Math.random()*1000)+"-object",locale=Content.getPageData("locale"),useSecureConnection=window.location.protocol==="https:",brightcoveDomain=(useSecureConnection)?"https://sadmin.brightcove.com":"http://admin.brightcove.com",objectTag=$(['<object id="',pseudoUniqueObjectId,'">','<param name="wmode" value="transparent" />','<param name="bgcolor" value="#FFFFFF" />','<param name="width" value="',data.getWidth(),'" />','<param name="height" value="',data.getHeight(),'" />','<param name="disableRatings" value="true" />','<param name="playerID" value="',data.getPlayerId(),'" />','<param name="isVid" value="true" />','<param name="isUI" value="true" />','<param name="autoStart" value="',data.getAutoStart(),'"  />','<param name="@videoPlayer" value="',data.getVideoId(),'" />','<param name="locale" value="',locale,'" />','<param name="templateLoadHandler" value="nike.cq.UI.Component.onVideoTemplateLoaded" />','<param name="labels" value="http://assets.nikevideo.nike.com/video_player/xml/translate/2.0/',locale,'_Labels.xml" />','<param name="linkBaseUrl" value="',window.location.href,'" />','<param name="secureConnections" value="',useSecureConnection,'" />','<param name="htmlFallback" value="true" />',"</object>"].join("")),player="",experience="",experienceModule="";
var onReady=function(){if(document.getElementById(element.get(0).id)){element.empty();
brightcove.createExperience(objectTag.get(0),element.get(0),true);
onLoad()
}else{window.setTimeout(onReady,100)
}};
var onTemplateLoaded=function(id){};
var onTemplateReady=function(){};
var onVideoComplete=function(){if(config.onVideoComplete){var customFunction=eval(config.onVideoComplete);
if(typeof(customFunction)==="function"){customFunction.call(this)
}}};
if(typeof(brightcove)=="object"){onReady()
}else{$.getScript(brightcoveDomain+"/js/BrightcoveExperiences.js",function(){$.getScript(brightcoveDomain+"/js/APIModules_all.js",function(){onReady()
})
})
}this.element=element;
this.objectTag=objectTag;
this.objectTagId=pseudoUniqueObjectId
},remove:function(){if(typeof brightcove=="object"&&experienceModuleRef){experienceModuleRef.unload();
document.getElementById(this.objectTagId).parentNode.innerHTML=""
}}}),SinaVideoPlayer=new BaseVideoPlayer({isValid:function(data){return data.getVideoId()!=""
},generate:function(data,element,onLoad){var that=this;
if(Utils.hasFlash()){var pseudoUniqueId="nike-cq-video-player-component-"+new Date().getTime()+Math.floor(Math.random()*1000)+"-object",videoContainer=$("<div>",{id:pseudoUniqueId});
element.empty().append(videoContainer);
var swfBridge=Utils.FlashBridge.getSWF(pseudoUniqueId);
nike.UI.embedFlash({src:"/etc/designs/nike/resources/video/sina/SinaPlayer.swf",id:pseudoUniqueId,width:data.getWidth(),height:data.getHeight(),flashVars:{videoID:data.getVideoId(),swfLocation:"/etc/designs/nike/resources/video/sina/",autoPlay:true,onReady:swfBridge.onReady()},params:{wmode:"opaque",menu:false,allowScriptAccess:"always",bgcolor:"#000000"},onLoad:function(movie){var swf=movie.ref;
that.generateControls({autoPlay:false,appendTo:element,play:function(){swf.play()
},pause:function(){swf.pause()
},stop:function(){swf.stop()
},seek:function(pos){swf.seek(pos)
},getTotalTime:function(){return swf.getVideoTotalTime()
},getCurrentPosition:function(){return swf.getVideoCurrentPosition()
},onPlayEvent:"onVideoPlaying",onPauseEvent:"onVideoPaused",registerEvent:function(eventName,callback){swf.addEventListener(swfBridge.createEventHandler(eventName,callback))
},onReady:swfBridge.onReady});
onLoad()
}})
}this.element=element
},remove:function(){config.element.remove()
}}),YouTubeVideoPlayer=new BaseVideoPlayer({isValid:function(data){return data.getVideoId()!=""
},generate:function(data,element,onLoad){var pseudoUniqueId="nike-cq-youtubevideo-player-component-"+new Date().getTime()+Math.floor(Math.random()*1000)+"-object";
var videoContainer=$(document.createElement("div")).attr("id",pseudoUniqueId);
element.empty();
element.append(videoContainer);
var that=this;
if($("#youtube_api_script_tag").size()<1){var tag=document.createElement("script");
tag.src="//www.youtube.com/iframe_api";
tag.id="youtube_api_script_tag";
var firstScriptTag=document.getElementsByTagName("script")[0];
firstScriptTag.parentNode.insertBefore(tag,firstScriptTag);
window.onYouTubeIframeAPIReady=function(){that.player=new YT.Player(pseudoUniqueId,{height:data.getHeight(),width:data.getWidth(),videoId:data.getVideoId(),playerVars:{rel:0},events:{onReady:onPlayerReady}})
}
}else{setTimeout(function(){that.player=new YT.Player(pseudoUniqueId,{height:data.getHeight(),width:data.getWidth(),videoId:data.getVideoId(),events:{onReady:onPlayerReady}})
},1000)
}function onPlayerReady(event){}},remove:function(){if(this.player&&this.player.stopVideo){this.player.stopVideo()
}config.element.remove()
}}),QQVideoPlayer=new BaseVideoPlayer({isValid:function(data){return data.getVideoId()!=""
},generate:function(data,element,onLoad){var srcUrl="http://v.qq.com/iframe/player.html?vid="+data.getVideoId()+"&tiny=0&auto=0",videoContainer=$('<iframe frameborder=0 src="'+srcUrl+'" allowfullscreen></iframe>');
videoContainer.width(data.getWidth());
videoContainer.height(data.getHeight());
element.empty();
element.append(videoContainer)
},remove:function(){config.element.remove()
}});
var videoObj;
switch(videoPlayer.getPlayerType()){case VideoPlayerType.BRIGHTCOVE:videoObj=BrightcoveVideoPlayer;
break;
case VideoPlayerType.SINA:videoObj=SinaVideoPlayer;
break;
case VideoPlayerType.YOUTUBE:videoObj=YouTubeVideoPlayer;
break;
case VideoPlayerType.QQ:videoObj=QQVideoPlayer;
break;
case VideoPlayerType.NONE:default:if(!isElement&&!config.data.playerType){config.data.playerType=VideoPlayerType.BRIGHTCOVE;
videoPlayer=new VideoPlayerContent(dataLength>0?config.data:element);
videoObj=BrightcoveVideoPlayer
}else{videoObj=new BaseVideoPlayer()
}break
}$.extend(this,{getElement:function(){return config.element
},getControlsElement:function(){return videoObj.getControlsElement()
},remove:function(){videoObj.remove();
config.onRemove.call(this)
}});
(function(){var that=this;
config.element=element;
if(videoObj.isValid(videoPlayer)){config.onInit.call(this);
videoObj.generate(videoPlayer,config.element,function(){config.onLoad.call(that)
})
}}).call(this);
return this
}
...
n=a(window).width();
var U=1;
if(a(u).find(".nike-cq-content-slide-540-container").length){if(n<=768){L=768;
O=768;
s=768;
p=((s+K*2)*w);
v=0
...
O=((s+K*2)*r);
p=((s+K*2)*w)
}}}else{if(a(u).find(".nike-cq-nikeplus-activity-tout-container").length){if(n<=768){L=768;
O=620;
v=74;
if(r>=3){r=2
}}else{if(n<=1024){v=40;
...
var V;
if(a(u).find(".nike-cq-content-slide-540-container").length){V=a(document.createElement("div")).addClass("window-carousel-container").css({width:O+"px",height:J+"px",margin:"0 auto ",overflow:"hidden",position:"relative"})
}else{if(a(u).find(".nike-cq-nikeplus-activity-tout-container").length){V=a(document.createElement("div")).addClass("window-carousel-container").css({width:O+"px",margin:"0 auto ",height:J+"px",overflow:"hidden",position:"relative"})
}else{if(a(u).find(".nike-cq-notification-banner-container").length){V=a(document.createElement("div")).addClass("window-carousel-container").css({width:O+"px",height:J+"px",margin:"0 auto ",overflow:"hidden",position:"relative"})
}}}u.children("."+A.slideClass).wrapAll(X);
u.children("."+A.slideContainerClass).wrapAll(V);
...
H()
}else{if(a(u).find(".nike-cq-content-slide-540-container").length){a(window).bind("orientationchange",function(){i(true)
});
window.onresize=function(U){i(true)
};
i()
}else{if(a(u).find(".nike-cq-nikeplus-activity-tout-container").length){a(window).bind("orientationchange",function(){i(true)
});
window.onresize=function(U){i(true)
};
i()
...
var W=M?2:0;
V.stop(true,true).animate({left:-Q*(j+W)},{duration:y,complete:function(){if(M){if(j<=-1){V.css("left",-Q*(l.length+1));
o=l.length-1;
...
nike.cq.require("jquery",function(b){var c=function(){var e=null,d=function(){clearTimeout(this.timerRef);
this.timerRef=setTimeout(f,100)
},f=function(){if(b("div.nike-cq-hyperlink-component").length>0){var g=b("div.nike-cq-hyperlink-component").attr("class");
g=g.replace("mediumpx","");
g=g.replace("largepx","");
if(b(window).width()<=800){b("div.nike-cq-hyperlink-component").attr("class","mediumpx "+g)
}else{b("div.nike-cq-hyperlink-component").attr("class","largepx "+g)
}}};
return{clearTime:d}
},a=c();
b(window).resize(function(){a.clearTime()
});
b(document).ready(function(){a.clearTime()
})
});
