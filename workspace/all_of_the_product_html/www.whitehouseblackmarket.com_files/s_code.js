...
Last Updated by M. Hajala - 2/12/2013 */

/************************ ADDITIONAL FEATURES ************************
Plugins
*/

//var s_account="chchicoscomdev"
...
s.linkInternalFilters="javascript:,whitehouseblackmarket.com"
...
s._channelPattern="Paid Search|knc>Affiliates|afc>Banners|bac>Comparison Shopping|cs>Sweepstakes|swc>Mobile|moc>Email|emc>Social Media|smc>Natural Search Provider|nsc"
...
s.pageURL=document.URL.toLowerCase();
s.channelManager('cmp,sourceid,rmid','','cmgr_getOnce','','cmgr_direct','1');
s.campaign=s._campaign.toLowerCase();	
	/* Set a cookie with 'paid' using a 30-day expiration, and only set direct if last campaign was not 'paid' */
	var ec=new Date();
	    ect=ec.getTime();
		ec.setTime(ect+30*86400000);
	    
	if(s._channel && s._channel=='Paid Search'){
		s.c_w("cChannel","paid",ec);
		s.eVar22=s._channel;
		s.eVar32=s._partner;
	...
	if(s._channel && s._channel=='Affiliates' || s._channel=='Banners' || s._channel=='Comparison Shopping' || s._channel=='Sweepstakes' || s._channel=='Mobile' || s._channel=='Email' || s._channel=='Social Media' || s._channel=='Natural Search Provider' || s._channel=='Unknown Paid Channel'){
		s.c_w("cChannel","paid",ec);
		s.eVar22=s._channel;
		s.eVar32="non-search";
		s.eVar33="non-keyword search";
		s.eVar34="non-unpaid referrer";
	}
	if(s._channel && s._channel=='Referrers' || s._channel=='Other Natural Referrers') {
		if(s.c_r('cChannel')=="paid"){
			s.campaign="";
		}
		else{
			s.campaign='ref|'+s.campaign;
			s.eVar22="Referrers";
			s.eVar32="non-search";
			s.eVar33="non-keyword search";
			s.eVar34=s._referringDomain;
	...
	if(s._channel && s._channel=='Direct Load'){
		if(s.c_r('cChannel')=="paid"){
			s.campaign="";	
	  }
	  else{
		
		s.campaign="direct load";
		s.eVar22=s._channel;
		s.eVar32="non-search";
		s.eVar33="non-keyword search";
		s.eVar34="no referrer";
	  }
	}
	
	//Set evar35/36 equal to s.campaign when the channel is paid search and persist value in prop6
if(s.eVar22 && s.eVar22=='Paid Search'){
	s.eVar35=s.eVar36=s.campaign;
	s.prop6=s.getAndPersistValue(s.eVar22,'cp_marketing_channel',0);
}

s.prop6=s.getAndPersistValue(s.eVar22,'cp_marketing_channel',0);

s.eVar24 = s.crossVisitParticipation(s.campaign,'s_cpm','90','5','>','purchase');

s.clickThruQuality('cmp,sourceid,rmid','event4','event5');

if(!s.eVar37)
	s.eVar37=s.getQueryParam('LkshrPID,GANPID');

if(!s.eVar38)
	s.eVar38=s.getQueryParam('LkshrCID,GANCID');

/* Remove pre-pended 4 digit year from Purchase ID and copy to eVar43 */
	if(s.purchaseID)
	  s.eVar43=s.purchaseID;
	
}

s.Media.trackUsingContextData = true 
s.Media.contextDataMapping = {
"a.media.name":"eVar42,prop13",
...
Track based on seconds
s.Media.trackSeconds = 5;
*/


/* Currently being set in video.js
...
+"substring(i+s._length);i=x.indexOf(o,i+l)}return x");
...
/*
* Plugin: getTimeToComplete 0.Custom - return the time from start to stop
*/
...
+"3600,tm=60,r=2,u;u=tm;v=v*r/u;return (Math.round(v)/r)}}return '';");

/* WARNING: Changing any of the below variables will cause drastic
changes to how your visitor data is collected. Changes should only be
made when instructed to do so by your account manager.*/
s.visitorNamespace="chicos"
s.trackingServer="omni.whitehouseblackmarket.com"
s.trackingServerSecure="somni.whitehouseblackmarket.com"
s.dc=122
