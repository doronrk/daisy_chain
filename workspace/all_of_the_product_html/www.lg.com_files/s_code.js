/* SMG-5098, LGEUS-4029 : modify */
/*SMG-6231 20140703 modify*/
if((typeof countryCd != "undefined" && typeof defaultCookie != "undefined" && defaultCookie.toUpperCase() == "OFF" && typeof ecScodeFlag != "undefined" && ecScodeFlag == false && (getCookies(countryCd + '_' + ecScodeCate) == '' || getCookies(countryCd + '_' + ecScodeCate) == 'N')) ||  (typeof countryCd != "undefined" && typeof defaultCookie != "undefined" && defaultCookie.toUpperCase() == "ON" && typeof ecScodeFlag != "undefined" && ecScodeFlag == false && getCookies(countryCd + '_' + ecScodeCate) == 'N')) {
/*SMG-6231 20140703 modify*/
	var s_account;
	var s=new Object();
	for (var i=1; i<20; i++)
	...
	function s_gi(s_account){return;}
	function t(){return;}
	function tl(a,b,c){return;}
	function getPageName(){return;}
	function channelExtract(a,b,c){return;};

	s.pageName=s.events="";
	s.t = t;
	s.tl = tl;
	s.getPageName = getPageName;
	s.channelExtract = channelExtract;
	s.linkTrackVars = null;
	s.linkTrackEvents = null;
} else {
	var this_locale = window.location.pathname.split("/")[1];
	var s_account = "lge" + this_locale.replace("_", "");
	if (s_account == "lgegr") {
		s_account = "lgehellas"
	} else {
		if (s_account == "lgedk" || s_account == "lgefi" || s_account == "lgeno") {
			s_account = "lgenordic" + this_locale
		} else {
			if (s_account == "lgers") {
				s_account = "lgesr"
			} else {
				if (s_account == "lgese") {
					...
					if (s_account == "lgecommon") {
						s_account = "lgeglobal"
					}
				}
			}
		}
	} if (location.host.indexOf("dev") > 0 || location.host.indexOf("stg") > 0 || (location.host.indexOf("www") == -1 && location.host.indexOf("wws") == -1)) {
		s_account = s_account + "dev";
	}

/* Site Catalyst Native Code */
...
/* End of Site Catalyst Native Code */


/* Original Location of s_gi */
	...
	s.linkInternalFilters = "javascript:,us.lge.com,www.lge.com,lge.com,lg.com,www.lg.com";
	...
	if (s_account == "lgebr") {
		s.trackingServer = "lgelectronics.122.2o7.net";
		s.trackingServerSecure = "lgelectronics.122.2o7.net";
	} else {
		s.trackingServer = "omniture.lg.com";
		s.trackingServerSecure = "tracking.lg.com";
	}
	/* End of SSL Tracking Server */
        /* LGESEOWA-225 20141023 add*/
  var  adservConfig = {
        tEvar:              '',
        aID:                '',
        cID:                '',
  ...
  /* //LGESEOWA-225 20141023 add*/
	function s_doPlugins(a) {
		...
		s.Media.trackVars="events,prop26,eVar47,eVar48,eVar49,contextData.bc_tags,contextData.bc_refid,contextData.bc_player,contextData.bc_playertype,contextData.bc_playlist";
		s.Media.trackEvents="event56,event57,event58,event59,event60,event61,event62";
		s.Media.trackMilestones="25,50,75";
		s.Media.segmentByMilestones = true;
		s.Media.trackUsingContextData = true;
		s.Media.contextDataMapping = {
			"a.media.name":"eVar47,prop26",
			"a.media.segment":"eVar48",
			"a.contentType":"eVar49",
			"a.media.timePlayed":"event62",
			"a.media.view":"event56",
			"a.media.segmentView":"event57",
			"a.media.complete":"event61",
			"a.media.milestones":{
				25:"event58",
				50:"event59",
				75:"event60"
		...
		if(!s.eVar13){
			...
			s.eVar13=s.getValOnce(s.eVar13,'s_ev13',0);
			if(s.eVar13){
		...
		/* //LGEUS-4333 20140925 add*/
		if(!s.eVar66){
			s.eVar66=s.getQueryParam('icid');
			s.eVar66=s.getValOnce(s.eVar66,'s_ev66',0);
			if(s.eVar66){
				s.events=s.apl(s.events,"event23",",",2);
			}
		}
		a.prop23 = a.getTimeParting("h", "-8");
		a.prop24 = a.getTimeParting("d", "-8");
		a.prop25 = a.getTimeParting("w", "-8");
		if (a.prop3) {
			a.eVar3 = a.prop3
		}
		if (a.prop4) {
		...
		if (!a.campaign) {
			a.campaign = a.getQueryParam("cmpid")
		}
		if (!a.eVar13) {
			a.eVar13 = a.getQueryParam("intcmp")
		}
		a.prop12 = a.getAndPersistValue(a.campaign, "s_cp_persist", 0);
		if (a.getQueryParam("s_kwcid")) {
...
/* //LGEUS-4333 20140925 add*/
	...
	s.clickThruQuality = new Function("scp", "tcth_ev", "cp_ev", "cff_ev", "cf_th", "if(i<=1){var ev=(s.events?s.events+',':'');if(s.getQueryParam(scp)){s.events=ev+tcth_ev;if(s.c_r('cf')){var tct=parseInt(s.c_r('cf'))+1;s.c_w('cf',tct,0);if(tct==cf_th&&cff_ev){s.events=s.events+','+cff_ev;};}else{s.c_w('cf',1,0);}}else{if(s.c_r('cf')>=1){s.c_w('cf',0,0);s.events=ev+cp_ev;}}i++;}");
	s.getGoogleRank = new Function("ce,ie,ev1,ev2,dn", "var s=this,dr,rd,p,pa,kr,kw,dn=dn||'';qp='resnum,cd';dr=s.referrer||typeof s.referrer!='undefined'?s.referrer:document.referrer;if(!dr||!ce||!ie)return;rd=s.split(dr,'/');if(rd[2].substring(0,11)!='www.google.')return;kw=s.getQueryParam('q,as_q',' ',dr);if(!kw)return;if(ev1)s[ev1]=kw;kr=rd[3].substring(0,4)=='url?'?s.getQueryParam(qp,'|',dr):'';if(kr.indexOf('|')>-1)kr=kr.substring(0,kr.indexOf('|'));if(!kr||kr=='0'){if(ev2)s[ev2]='no rank available';return;}if(ev2)s[ev2]=kr;p=s.products;pa=s.split(p,',');pa[0]=s.split(pa[0],';');pa[0][0]=pa[0][0]||'';pa[0][1]=pa[0][1]||dn;pa[0][2]=pa[0][2]||'';pa[0][3]=pa[0][3]||'';pa[0][4]=s.apl(pa[0][4],ie+'='+kr,'|',2);pa[0]=s.join(pa[0],{delim:';'});pa=s.join(pa,{delim:','});s.events=s.apl(s.events,ce,',',2);s.events=s.apl(s.events,ie,',',2);s.products=pa;return;");
