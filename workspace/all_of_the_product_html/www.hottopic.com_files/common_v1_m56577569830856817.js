...
//Changes for SBO colorbox close event - start
function bindSBOPopup(elementName){
	//Creating a variable to hold the utag_data of the parent page.
	var parentUtagData = null;

	//dumping the utag data of the parent
	if(isHTTealiumEnabled && utag_data != null && typeof(utag_data) != 'undefined'){
	  parentUtagData = $.extend(true, {}, utag_data); //This is just assigning utag_data value to parentUtagData for use later.
	}

	$(elementName).colorbox({width:909,height:1030,scrolling:false,onClosed:function(){
		if(isHTTealiumEnabled){
			if(parentUtagData != null) {
				utag_data = parentUtagData;
				if(utag != null && utag.view != null && typeof utag != 'undefined' &&
						typeof utag.view != 'undefined' && typeof utag != undefined &&
							typeof utag.view != undefined){
					try{
						utag.view(utag_data);
					}catch(e){}
				}
...
//Changes for SBO colorbox close event - end