...
(function(){
    ...
    _shoprunner_com.retailerID = 'HOLLISTER';
    _shoprunner_com.loginValidationURL = '/webapp/wcs/stores/servlet/ShopRunnerLogin';
    ...
    /*
         ...
         * 2 - Production
         */
    _shoprunner_com.environmentID = ANF.params.env.shoprunnerEnvID;
    ...
    if ($('body').hasClass('checkout')) {
        _shoprunner_com.calls = {
            on_sign_out: function() {
                // User sign out
                $.when(ANF.checkout.global.updateShippingMethod()).then(function () {
                    ANF.checkout.shipping.updateSummaryShippingTotal();
                });
            },
            on_sign_in: function(){
                // User Signs in via ShopRunner Login Modal
                $.when(ANF.checkout.global.updateShippingMethod()).then(function () {
                    $('.shoprunner-radio').attr('checked', 'checked');
                     ANF.checkout.shipping.updateSummaryShippingTotal();
                });
            }
        }
