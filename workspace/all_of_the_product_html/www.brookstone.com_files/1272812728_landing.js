...
if(window.location.protocol.toLowerCase() == 'http:'){	
	try {
		...
		if(ci_ITD()===false){
			...
			ca.async = true;
			ca.id = "_casrc";
			ca.src = "https://t.channeladvisor.com/v2/23000349.js";
		...
		//Adhesive.com
		if(ci_ITD()===false){
			//Visitor tag - homepage
			if(ci_pagetype == 'HOME'){
				CI_ExternalJS('http://ib.adnxs.com/seg?add=961621&t=1');
			}else{
				//Shopper tag - non-homepage landing
				CI_ExternalJS('http://ib.adnxs.com/seg?add=961660&t=1');
			}
		}
		...
		try{
			if(ci_ITD()===false){
				ci_FP("googleads.g.doubleclick.net/pagead/viewthroughconversion/1072562879/?value=0&label=xewnCMyAlgUQv4W4_wM&guid=ON&script=0");
			}
		}catch(err){}
