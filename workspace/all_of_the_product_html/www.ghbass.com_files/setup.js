...
{
	var base_initPage = mybuys.initPage;
	return function()
	{
		 if((mybuys.params["pt"]) && (mybuys.params["pt"].indexOf("purchase") != -1))
		 {
			if(mybuys.params['email'])
			{
				var testemail = Math.max(this.params['email'].toUpperCase().indexOf("BRAJESH1006@GMAIL.COM"));
				if(testemail<0)
				{
					base_initPage.apply(mybuys);
				}
			}
			else
			{
...
var mbJQPageTypes = [ "PRODUCT_DETAILS", "CATEGORY" ];

var mbCarouselZoneKeys = { "PRODUCT_DETAILS": [ { "zoneKey": "1", "visibleItems": 3, "totalItems": 12, "scrollItems": 3, "isVertical": false, "fallbackDimension": 260, "callback": mbcarouselInitCallback_prod1 } ], "CATEGORY": [ { "zoneKey": "1", "visibleItems": 3, "totalItems": 12, "scrollItems": 3, "isVertical": false, "fallbackDimension": 220, "callback": mbcarouselInitCallback_cat1 } ] };

function mbcarouselInitCallback_cat1(carousel) 
{
	jQuery(".MB_CAT1 .MB_CAROUSELLEFT").bind('click', function() { jQuery(".carousel_root").mbjcarousel("scroll", "-=3" ); return false; });
	jQuery(".MB_CAT1 .MB_CAROUSELRIGHT").bind('click', function() { jQuery(".carousel_root").mbjcarousel("scroll", "+=3"); return false; });
	return false;
}
...
function loadjQuery(callback)
{
	...
	{
		var url = 'https://t.p.mybuys.com/clients/GHBASS/js/lib/jquery-1.8.2.js';
...
{
	if (typeof jQuery.mbjcarousel == "undefined")
	{
		var url = 'https://t.p.mybuys.com/clients/GHBASS/js/lib/jcarousel.js';
		loadScript(url, function ()
		{
			callback();
		}
	...
	if (jQueryRequired)
	{
		loadjQuery(
			function ()
			{
				...
				if (typeof mbAddToCartZoneKeys[mybuys.pagetype] != "undefined")
				{
					...
					for (var i = 0; i < info.length; i++)
					{
						if (mybuys.params.wrz.split(",").indexOf(info[i].zoneKey) > -1)
						{
							initAddToCart(mybuys.pagetype, info[i].zoneKey, info[i].numSlots);
						}
					}
				}
			}
...
{
	jQuery(".carousel_root").mbjcarousel(
		{
			"wrap": "circular",
			"animation": 1000
		});
	callback(jQuery(".carousel_root").mbjcarousel);
}

function mbcarouselInitCallback_prod1(carousel) 
{
	jQuery(".MB_PROD1 .MB_CAROUSELLEFT").bind('click', function() { jQuery(".carousel_root").mbjcarousel("scroll", "-=3" ); return false; });
	jQuery(".MB_PROD1 .MB_CAROUSELRIGHT").bind('click', function() { jQuery(".carousel_root").mbjcarousel("scroll", "+=3"); return false; });
	return false;
}



mybuys.setClient("GHBASS");
