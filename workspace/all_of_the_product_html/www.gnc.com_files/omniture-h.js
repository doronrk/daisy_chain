...
s.trackingServer='metrics.gnc.com';		//FPC URL (comment out if not using)
s.trackingServerSecure='smetrics.gnc.com';	//FPC SSL URL (comment out if not using)
s.campaignID='cid,camp';			//campaign variable NOTE: follow the settings on the client configuration Campaign Tracking tab
/* End Partner Config Section */
s.internalCID='ab';			//internal campaign id
//s.visitorNamespace='<<domain>>';	//visitor namespace for cookie migration and for 3rd party cookies
...
function productInfoTrack(category,product,linkName) {
        s.linkTrackVars='products,prop22,evar43,events';
        s.linkTrackEvents='event30';
        s.prop22=linkName;
	s.products=category+";"+product+";;;;eVar43="+linkName;
	s.events='event30';
...
function checkoutInitiationCustomLinkTrack() {
	s.linkTrackVars='events';
        s.linkTrackEvents='scCheckout';
        s.events='scCheckout';
        s.tl(this,'o','Checkout Initiation');
        s.linkTrackVars='None';
        s.linkTrackEvents='None';
}

function prodFlavorTrack(category,product,medium,source,domain,superpid,prodfindmethod,pid,linkName) {
        s.linkTrackVars='server,products,channel,prop1,prop2,prop3,prop4,prop22,eVar30,eVar32,eVar33,eVar43,events';
        s.linkTrackEvents='prodView,event1,event30';
	s.server=domain
	s.prop4=medium;
	s.prop3=source;
	...
	s.prop22=linkName;
	s.products=category+";"+product+";;;;evar43="+linkName+"|evar33="+prodfindmethod+"|evar30="+pid+"|evar32="+superpid;
	s.events='prodView,event1,event30';
	s.tl(this,'o',linkName);
	s.linkTrackVars='None';
   	s.linkTrackEvents='None';
}
...
/*Null s.products and s.events when addToCart param is present*/
	if(typeof addToCartFromMyList != "undefined" && addToCartFromMyList){
		s.events = null;
		s.products = null;
	}
...
/*******************************************************/
	s.prop7=s.getAndPersistValue(s.campaign,'s_p3_persist',0);
	if(s.prop7)
...
/**********************************************************************************************/
	...
	s.prop12=s.getAndPersistValue(s.eVar1,'s_cp_persist',0);
	if(s.prop12)
...
/****************************************************************************************/
	s.prop6=s.getDaysSinceLastVisit('gdslv');
...
/**********************************************************************/
	var prev_value=s.getPreviousValue(s.pageName,'gpv_p6','');
	s.prop19=prev_value;
	if (s.prop19){
...
/**********************************************************************/
	s.eVar23 = s.crossVisitParticipation(s.campaign,'s_cpmCVP','30','5',' > ','purchase');
}
