 ...
 */
	 ...
	 */
		hideMessageArea:function(){
			...
			if(dojo.byId('ErrorMessageText1') != null){
				dojo.byId('ErrorMessageText1').innerHTML = "";
			}
		},
     ...
     */
		displayErrorMessage:function(msg, topOffset,showType){	
		 ...
		 /**
	     * This function is used to display the error messages specificaly for storelocator . 
	     * @param (string) msg The error/information message to be displayed
	     * @param (int) topOffset how far from the top the message display area will be displayed. 
	     * @param (boolean) showType whether or not the message type should be appended to the actual message
	     *
	     ...
	     *
	     */
			displayErrorMessage1:function(msg,relobj,pos){	
			dojo.create('span', {
		        innerHTML: msg,
		        className: 'error storelocator-error allerrors',
		        style: { }
		    }, relobj, pos);
			
			},
////DEstroy th error message
			destroymsg: function(){
				var uierror = dojo.query('span.storelocator-error');
				if(uierror.length)
					dojo.destroy(uierror[0]);
			},
     ...
     */
		displayStatusMessage:function(msg,topOffset){
			...
			if (dojo.byId('ErrorMessageText') != null) {
				if (dojo.byId('ErrorMessageText1')) {
					dojo.byId('ErrorMessageText1').innerHTML = msg;
					this.showHideMessageArea(topOffset);
					dojo.byId('MessageArea').focus();
					setTimeout("dojo.byId('ErrorMessageText1').focus()", 1000);
				...
				/*
				 * Infosys (08/11/2013) - Added below section to remove existing
				 * error message in errorMessageSection before displaying the
				 * new status message
				 */
				var msgSection = dojo.byId('ErrorMessage_General');
				if (msgSection != null) {
     ...
     */
		hideAndClearMessage : function() {
			...
			if(dojo.byId("label-billingdropdown_1")!=null)
				{
			var errorLabel = dojo.byId("label-billingdropdown_1");
			UtilitiesJS.toggleErrorMsg(errorLabel, "none");
			errorLabel.innerHTML = "";
				}
			if(dojo.byId("label-shippingdropDown")!=null)
				{
     ...
     */
		...
		/**
	    *
	    *This function will check whether email n confirm email are same for pick up form on shipping page
	    *
	    *@param (strEmail) the email address 
	    *
	    **@param (strEmail2) the confirm email address string 
	    *@return (boolean) true if email matches
	    
	    */
		isConfirmEmail:function(strEmail,strEmail2){
     ...
     */
		...
		displayStockErrorMsg : function (msg) {
			var msgSection = document.getElementById('oostockErrorMsg');
			...
			fadeOutAnimArgsArray["node"] = node;
			fadeOutAnimArgsArray["duration"] = 500;
			fadeOutAnimArgsArray["delay"] = 25000;
			fadeOutAnimArgsArray["onEnd"] = function(){
				dojo.style(node, "display", "none");
				dojo.style(node, "opacity", 100);
			};
