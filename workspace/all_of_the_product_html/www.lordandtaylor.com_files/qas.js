...
var PROXY_PATH = "/webapp/wcs/stores/servlet/HBCStorefrontAssetStore/qas/qas_proxy.jsp";
...
/*Hassane - Added condition for defect 3580*/ 
if (!QAS_PROMPTS) {
  var QAS_PROMPTS = {
					"InteractionRequired":
					{
...
//functions added for changing parameters
function QAS_setAddressFields(addrs){
	ADDRESS_FIELD_IDS = addrs;
}
function QAS_setCountryFields(countries){
	COUNTRY_FIELD_IDS = countries;
}
function QAS_convCtry(ctryIn) {
	var ctryOut = DEFAULT_DATA;

	if ( (ctryIn != "") && (ctryIn != undefined) )
	{
		for ( var i = 0; i < COUNTRY_MAP.length; i++ )
		{
			if ( ctryIn.toLowerCase() == COUNTRY_MAP[i][0].toString().toLowerCase() )
			{
				ctryOut = COUNTRY_MAP[i][1].toString();
			}
		}		
	}	
...
{	
	...
	// Checking if address validation should be run based on js var which comes from properties file setup in LTHeader.jsp
	//  If js var doesn't exist, then default to true for running addr validation.
	var isAddrValidOn = (typeof(IS_ADDR_VALIDATION_ON) !== "undefined" && IS_ADDR_VALIDATION_ON != null) ?
		IS_ADDR_VALIDATION_ON : true;
	
	//Allow dynamic setting of postOnclick
	if(callback!=null && callback!=""){
		postOnclick = callback;
	}
		
	if (isAddrValidOn && ((preOnclick == null) || (preOnclick())))
	{
		var m = new Main(postOnclick, buttonID);
		m.process();
	}
	else if (!isAddrValidOn && ((preOnclick == null) || (preOnclick())) && (postOnclick != null))
	{
		postOnclick();
	}
...
function Main(clickEvent, buttonID)
{
	...
	this.ajaxError = function(xml, text, msg)
	{
		...
		if ( DISPLAY_ERRORS )
		{
			console.warn(text + " - Error with AJAX call. Check to make sure pro web is running correctly."); 
		}
...
function Address(addressIds, countryIds, countryList)
{
		...
		for ( i = 0; i < ids.length; i++ )
		{
			...
			for ( j = 0; j< ids[i].length; j++ )
			{
				...
				if ( fieldValue == undefined )
				{
					if ( DISPLAY_ERRORS ) 
					{
						console.warn("ID '" + ids[i][j] + "' is undefined");
					}
				}
				else
				{
			...
			//push converted country into the temporary variable
			tempAddress.push( getConvCtryCode() );
	...
	var getConvCtryCode = function() {
		//get the country from the form
		var c3 = $('#' + cIds[i]).val();
		
		return QAS_convCtry(c3);
	}
	...
	var returnAddresses = function()
	{
		for ( i = 0; i < ids.length; i++ )
		{
			...
			if (cleanedAddresses[uniqueTracker[i]] != undefined)
			{
                for ( j = 0; j < ids[i].length; j++ )
                {
            		// 2012-10-17 - AndyK - Use only first 5 zip code digits (not the +4) for USA addr.
                	// j == 5, is the postal code field
                	if (j == 5 && getConvCtryCode() == "USA") {                		
                		$('#' + ids[i][j]).val(cleanedAddresses[uniqueTracker[i]][j].substring(0,5));
                	} else {
...
function Clean(searchString, country_3, proxyPath, layout, ajaxErr, ajaxTimeout)
{
	...
	var savePickList = function()
	{
		...
		// 2012-10-17 - AndyK - Use only first 5 zip code digits (not the +4) for USA addr.	
		if (moniker.length >= 3 && moniker.substring(0,3) == "USA" && postCode.length > 5) {
		...
		switch(m_clean.verifylevel)
		{
			case "Verified":
				...
				if ( m_clean.country == "USA")
				{
					...
					{
						// undoing 3324  : HAwaii address mentioned in comment # 14. by PALLAVI on May 15 is a valid address in QAS response, 
						 // it is fine if address check is merging street 2 in street 1, a valid scenario.   
						m_callback();
					}
				}
				...
				else if (m_clean.country == "CAN")
				{
					...
					{
						// Abraham Raghib: reverted back this change as this was causing an issue that when the province is entered wrong, WAS would respond saying that
						// address is verified but it will fix the province so then the fixed province needs to be picked up.
						m_callback();
						//me.useOriginal();
					}
				}
			...
			case "InteractionRequired":
				...
				{
					// 2012-10-18 - AndyK - Use only first 5 zip code digits (not the +4) for USA addr.
					//  index 5 is the postal code field, index 6 is the country
					if (m_clean.result.length >= 7) {
						if (QAS_convCtry(m_clean.country) == "USA" && m_clean.result[5].length > 5) {
							m_clean.result[5] = m_clean.result[5].substring(0,5);
						} 
					}
...
function Interface(editCall)
{
	...
	var load = function()
	{
		...
		// 2012-10-17 - AndyK - Add zIndex of 10010 - to display above dojo dialogs and navigation.		
		$("#QAS_Dialog").dialog(
		{
			modal: true,
