...
var excludeDomains = ['babylon.com','orbitz.com','cheaptickets.com','file-extractor.com','buildabear.com','marinescreens.com'];
...
function fireAdextentPixel(overrideUrl){
    ...
    if(!usedparam && canonical != "" && reurl.indexOf('symantec.com') == -1 && reurl.indexOf('pimkie.es') == -1 && reurl.indexOf('marinescreens.com') == -1 && !overrideUrl)
    {
...
if(typeof(adextent_multi_urls) != "undefined" && adextent_multi_urls && adextent_multi_urls.length){
    for(var i=0; i< adextent_multi_urls.length; ++i){
        fireAdextentPixel(adextent_multi_urls[i]);
    }
}
else{
    fireAdextentPixel();
...
function AdextentAutotag(){
    ...
    this.syncCart = function(urls){
    }

    this.removeFromCart = function(url){
    }
    this.visit = function(url){
    }

    this.conversion = function(code, urls, cartvalue){
    }

    this.addToSegment = function(code){
    }
}
