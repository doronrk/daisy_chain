	...
	var methods = {
		...
		buildContainer: function(){
			...
			if(settings.image !== '' && settings.content === ''){
				...
				vars.$this.append('<img class="wingImg" '
								       +'height="'+settings.height+'" '
									   +'width="'+settings.width+'" '
									   +'src="'+settings.image+'" border="0"/>');
				...
				if(settings.link !== ''){
					...
					methods.setupCM(vars.$this.children('.wingLink'));
				}
						...
						if((vars.$this.offset().top + settings.height + settings.buffer) < vars.parent.height()){
							...
							if(settings.animate){
								vars.animationTimout = window.setTimeout(function(){
									vars.$this.animate({bottom: bottom+'px'}, 
						...
						if((((vars.$this.offset().top + settings.height + settings.buffer)) < vars.parent.height())){
							...
							if(settings.animate){
								vars.animationTimeout = window.setTimout(function(){
									vars.$this.animate({top: settings.top + $(window).scrollTop() + 'px'}, 
													   {duration:settings.duration, queue:false});
								}, settings.duration);
							}else{
		...
		setupCM: function(elem){
			...
			elem.bind('click', function() {
				...
				cmCreateManualLinkClickTag(settings.cmdomain+'/?cm_sp='+cm,method.getcmstring(settings.cmtag));
			});
		},
