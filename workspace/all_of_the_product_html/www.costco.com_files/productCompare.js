...
var productCompare = new function() {
	...
	this.addItem = function(item) {
		...
		if(this.products.length >= this.MAX_PRODUCTS) {
			...
			/*CIS100075112 b)*/
			d=new Date();
			this.updateForAssistive(d.getTime());
			return true;
		}
	}
	...
	/*CIS100075112 d)*/
	this.updateForAssistive = function(text){
		//$('#specialElement').val(text);
		if ($('#specialElement').length > 0) {
			document.getElementById('specialElement').setAttribute("value",text);
		}
		//console.debug('node value changed to: ' + $('#specialElement').val());
	}
};
...
$(window).load(function() {
	...
	if($('body').hasClass('show-compare') && $('.compare-box').size()>0) {
		...
		/*CIS100075112 a) */
		$('#compare').after('<div><input type="hidden" name="specialElement" id="specialElement" value="" /></div>');
		d=new Date();
