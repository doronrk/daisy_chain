...
var s_account="limitedcomprod";
if(/^(staging|Development|Sandbox|dev[01][0-9])/i.test(document.location.host) )
s_account="limitedcomdev";

// Detect moovweb environment
if(/^m[\w]*\.staging/i.test(document.location.host) )
s_account="limitedmlimitedcomdev";
if(/^m[\w]*\.thelimited/i.test(document.location.host) )
s_account="limitedmlimitedcomprod";
...
s.linkInternalFilters="javascript:,thelimited.com,paypal.com,onlineaccess.mycreditcard.cc,mycreditcard.cc,mycardcare.com,content.thelimited.com,dev04.web.limitedstores.demandware.net,staging.thelimited.com,production.thelimited.com,thelimited.promo.eprize.com,thelimited.review.eprize.com,events.thelimited.com,50.thelimited.com"
...
s._channelDomain="Social Media|facebook.com,flickr.com,digg.com,twitter.com,myspace.com,youtube.com,linkedin.com,stumbleupon.com,del.icio.us,reddit.com,metacafe.com,technorati.com";
s._channelParameter="";
s._channelPattern="Paid Search|KNC>Email|EMC>Banner|BAC>Retargeting|RTC>Affiliate|AFC>Retargetting|RTC>Social Network|SNC>Direct Mail|DMC>Shopping Feed|SFC>Print Ad|PAC>Radio Ad|RAC>Traditional Media|TMC>Store Orders|STO";
...
function s_doPlugins(s) {
	...
	// s_code version -increment after each change
	s.prop29="ver. 2.31";
	if (isBlank(s.pageName) && isBlank(s.events))
		s.pageName=sc_autoPageName(s);

	//Cheetahmail Integration Variables
	s.eVar15=s.getQueryParam("om_mid"); //places query param value from om_mid in eVar15 variable
	s.eVar16=s.getQueryParam("om_rid"); //places query param value from om_rid in eVar16 variable

	// populate eVar29 with referrer and sets pathing per referrer
	if (document.referrer !=''){
		s.eVar29=document.referrer;
	} else {
		s.eVar29='typed/bookmarked';
	}
	/* Sets pathing per for s.prop12 to lower case */
	s.prop12=s.eVar29 + ":" + s.pageName
	s.prop12=s.prop12.toLowerCase();	//set s.prop to lower case



	//Change all page type and navigation custom variables (s.pagename, prop 1-4) to lower case
	/* Set s.prop1 to lower case if it is defined and not null*/
	if(window.s.prop1 && s.prop1!="")
	{
	s.prop1=s.prop1.toLowerCase();  //set s.prop to lower case
	}

	/* Set s.prop2 to lower case if it is defined and not null*/
	if(window.s.prop2 && s.prop2!="")
	{
	s.prop2=s.prop2.toLowerCase();  //set s.prop to lower case
	}

	/* Set s.prop3 to lower case if it is defined and not null*/
	if(window.s.prop3 && s.prop3!="")
	{
	s.prop3=s.prop3.toLowerCase();  //set s.prop to lower case
	}

	/* Set s.prop4 to lower case if it is defined and not null*/
	if(window.s.prop4 && s.prop4!="")
	{
	s.prop4=s.prop4.toLowerCase();  //set s.prop to lower case
	}
	
	/* Set s.prop4 to lower case if it is defined and not null*/
	if(window.s.prop6 && s.prop6!="")
	{
	s.prop6=s.prop6.toLowerCase();  //set s.prop to lower case
	}

	/* Set s.eVar1 if s.prop6 is there*/
	if (typeof(s.prop6) != "undefined") {
		s.eVar1 = s.prop6;
	}	
	
	/* Set s.eVar7 if s.prop7 is there*/
	if (typeof(s.prop7) != "undefined") {
		s.eVar7 = s.prop7;
	}	
	
	/* Set s.eVar19 if s.prop18 is there*/
	if (typeof(s.prop18) != "undefined") {
		s.eVar19 = s.prop18;
	}	
	
	/* Set s.eVar20 if s.prop19 is there*/
	if (typeof(s.prop19) != "undefined") {
		s.eVar20 = s.prop19;
	}	
	
	/* Set s.prop25 if s.eVar11 is there*/
	if (typeof(s.eVar11) != "undefined") {
		s.prop25 = s.eVar11;
	}	
	
	/* Set s.channel to lower case if it is defined and not null*/
	if(window.s.channel && s.channel!="")
	{
	s.channel=s.channel.toLowerCase();  //set s.channel to lower case
	}

	/* Set s.eVar4 to lower case if it is defined and not null*/
	if(window.s.eVar4 && s.eVar4!="")
	{
	s.eVar4=s.eVar4.toLowerCase();  //set s.channel to lower case
	}
	if (s.pageName != undefined)
		s.pageName=s.pageName.toLowerCase();


	//populate s.campaign with external campaign value
	s.campaign = s.getValOnce(s.getQueryParam('cid'),"s_cmp",0);

	//populate s.eVar2 with internal campaign value
	if(!s.eVar2) {
		s.eVar2=s.getQueryParam('icid');
		
		}

	//populate s.eVar50 with link type value
	if(!s.eVar50)
		s.eVar50=s.getQueryParam('navid');	

	//populate s.eVar35 with Product Position value
	if(!s.eVar35)
		s.eVar35=s.getQueryParam('ppid');			

	//populate s.eVar52 with store ID value
    if(!s.eVar52)
        s.eVar52=s.getQueryParam('StoreID');
	...
	//store eVar29 (referrer) value only once per session
	...
	s.eVar2 = s.getValOnce(s.eVar2,"s_icmp",0);

	//sets product finding method: internal search
	if(s.eVar1 && !s.eVar3)
		s.eVar3='internal search';

	//determines if the URL is external and sets product finding method: typed/bookmarked or external referrer
	...
	var eventArr = s.events.split(',');
	var foundProdView = false, foundEvent3 = false;
	for (var i = 0; i < eventArr.length; i++)
	{
		if (eventArr[i] == 'event3')
			foundEvent3 = true;
		else if (eventArr[i] == 'prodView')
			foundProdView = true;
	}
	if (foundProdView && !foundEvent3)
		s.events=s.events+',event3';
	eventArr = null;

	//New/Repeat Visitor
	s.prop20 = s.eVar33 = s.getNewRepeat();


	//Master Ring Code
      if ((typeof(s.eVar22) != "undefined") && s.eVar22.match(/^[0-9]+$/g) != null) {
            var str_eVar23 = s.eVar22;
            s.eVar23 = str_eVar23.substring((str_eVar23.length - 3), str_eVar23.length);
      }


	//Custom Purchase ID and Transaction ID
	if (typeof(s.purchaseID) != "undefined") {
		if (s.purchaseID!="")
		{
			s.eVar25 = s.transactionID = s.purchaseID;
		}
	}

	//Time Parting
	s.prop13 = s.eVar30 = s.getTimeParting('h','-5');
	s.prop14 = s.eVar31 = s.getTimeParting('d','-5');
	s.prop15 = s.eVar32 = s.getTimeParting('w','-5');

    //Campaign Pathing
	if (typeof(s.campaign) != "undefined") {
		if (s.campaign!="")
		{
			s.prop11 = s.campaign + ":" + s.pageName;
		} else {
			s.prop11 = s.pageName;
		}
	}
	
    //Pathing per login status
	if (typeof(s.prop23) != "undefined") {
		if (s.prop23!="")
		{
			s.prop24 = s.prop23 + ":" + s.pageName;
		} else {
			s.prop24 = s.pageName;
		}
	}	

	if (document.referrer.indexOf("thelimited.com") > -1)
	{
		var ExtReferrer = '';
	} else {
	...
	//External Referrer
	if (ExtReferrer != '') {
		s.eVar29 = ExtReferrer;
	}

    //External Referrer Pathing
	if (ExtReferrer != '') {
		s.prop12 = ExtReferrer + ":" + s.pageName;
	} else {
		s.prop12 = ExtReferrer;
	}

 /* channelManager v2.2	*/

	s.channelManager('cid',':','s_cm','0','s_dl');     //set the query strings to be tracked here.

	if(s._channel=="Natural Search"){
		s._channel="Organic";
        s._campaign= s._channel + "-" + s._partner + "-" + s._keywords.toLowerCase();
      }
	if(s._channel=="Referrers"){
		...
		s.eVar37=s._channel				//Channel Manager Channel into eVar
	  	s.eVar38=s._referrer			//Channel Manager Referrer into eVar
		s.eVar39=s._referringDomain		//Channel Manger Referring Domain into eVar
		s.eVar40=s._partner				//Channel Manager Partner into eVar
		s.eVar41=s._keywords			//Channel Manager Keywords into eVar
		s.eVar42=s.crossVisitParticipation(s.eVar37,'s_atx','90','10',' > ','','1');
    ...
    // Campaign Stacking
	s.eVar17=s.crossVisitParticipation(s.campaign,'s_cid','30','10','>','purchase');

	
	/* Automate Finding Method eVar if not set*/
	
if (s.eVar37&&s.eVar37.indexOf('Typed/Bookmarked')==-1&&!s.eVar3)
{	
	s.eVar3='external channel: ' + s.eVar37;
	s.eVar1='non-search';
	s.eVar2='non-internal campaign';
	s.eVar4='non-browse';
	s.eVar44='non-cross sell';
	s.eVar45='non-cross sell';
}
else if(s.eVar1&&!s.eVar3)
{
	s.eVar3='internal search';
	s.eVar2='non-internal campaign';
	s.eVar4='non-browse';
...
else if(s.eVar44&&!s.eVar3)
{
	s.linkTrackVars=s.apl(s.linkTrackVars,'eVar3,eVar1,eVar2,eVar4,eVar44,eVar45',',',2);
	s.eVar3='product recommendation';
	s.eVar1='non-search';
	s.eVar2='non-internal campaign';
	s.eVar4='non-browse';
}
...
function sc_autoPageName(s){
	var pN=document.location.pathname.replace(/^[/]|(\.html)|(\.htm)|thelimited_/gi,"").replace(/,/g,"_").replace(/\//g,": ");
	return trimLc(["auto",pN].join(": "));
}

function sc_logGlobalErr(msg,url,line){
	//If you want to suppress any errors, you can do it here by exiting the function
	...
	try{
		...
		sl.linkTrackVars="pageName,prop28,eVar46,events";
		sl.events=sl.linkTrackEvents="event50";
		sc_setPageError(sl,"Global JS",errDesc);
...
function sc_setPageError(s,errSrc,errDesc){
	s.eVar46=[errSrc,errDesc].join(": ").toLowerCase();
	s.prop28="D=v46";
...
{ 
  s.linkTrackVars='eVar44,eVar45';
  s.eVar44=s.pageName;
  if(s.products)
  {
	  var tempArray = s.products.substring(1).split(";");
	  s.eVar45=tempArray[0];
	  tempArray = null;
  }
  else s.eVar45='unknown at time of referral';
...
s.visitorNamespace="thelimited"
s.trackingServer="metrics.thelimited.com"
s.trackingServerSecure="smetrics.thelimited.com"
...
// Attach global error handling to automatic logging
