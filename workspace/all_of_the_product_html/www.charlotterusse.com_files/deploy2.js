...
lpMTagConfig.plugins['simpleDeploy'] = {
		...
		config : {
			...
			buttons : [{"name":lpMTagConfig.getLPVarValue("unit")+'-'+lpMTagConfig.getLPVarValue("language"),"pid":"lpChatButton","afterStartPage":true,'ovr':'lpMTagConfig.db1'},
			            {"name":lpMTagConfig.getLPVarValue("unit")+'-'+lpMTagConfig.getLPVarValue("language")+"-2","pid":"lpChatButton2","afterStartPage":true,'ovr':'lpMTagConfig.db2'},
                                     {"name":lpMTagConfig.getLPVarValue("unit")+'-'+lpMTagConfig.getLPVarValue("language")+"-3","pid":"lpChatButton3","afterStartPage":true,'ovr':'lpMTagConfig.db3'},
                                      {"name":lpMTagConfig.getLPVarValue("unit")+'-'+lpMTagConfig.getLPVarValue("language")+"-4","pid":"lpChatButton4","afterStartPage":true,'ovr':'lpMTagConfig.db4'}
                                  ]
		}
};
...
(function() {
    /**
     * This function checks whether or not the Omniture value passed in exists.
     * If it does exist, set it as the variable named in variableName.
     * Otherwise do nothing.
     * 
     * @name SetOmnitureValueAsVariable
     * @param omnitureValue This is the Omniture value to check against.
     * @param variableName This is the name of the variable to set if omnitureValue is a valid value.
     */
    function SetOmnitureValueAsVariable(omnitureValue, variableName) {
        if (typeof omnitureValue !== 'undefined') {
            lpMTagConfig.vars.push([variableName, omnitureValue]);
        }
    }
if(typeof document.getElementById("checkoutMid") !== undefined  && document.getElementById("checkoutMid") !== null){
var tds = document.getElementById("checkoutMid").getElementsByTagName("input");
ordertotal= tds[0].getAttribute("ga-order-total");

 if(typeof ordertotal!== undefined  && ordertotal !== null){setTimeout('lpMTagConfig.vars.push(["page","OrderTotal", ordertotal])',1000);}
}

if (typeof s !== 'undefined') {
    SetOmnitureValueAsVariable(s.products, 'Product');
    SetOmnitureValueAsVariable(s.pageName, 'pageName');
    SetOmnitureValueAsVariable(s.eVar11, 'PurchaseID');
    if(typeof s.events !== undefined){
        if(s.events=='event7'){
            lpMTagConfig.vars.push(["page",'CheckoutStep', '1']);
        }else if(s.events=='event8'){
             lpMTagConfig.vars.push(["page",'CheckoutStep', '2']);
        }else if(s.events=='event9'){
             lpMTagConfig.vars.push(["page",'CheckoutStep', '3']);
        }else if(s.events=='event10'){
            lpMTagConfig.vars.push(["page",'CheckoutStep', '4']);
        }
