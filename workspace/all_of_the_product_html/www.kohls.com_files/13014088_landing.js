...
var ci_vid= 13014088;
var ci_cookieDomain=".kohls.com";
var ci_refDomain=".kohls.com";
var ci_imgs=[];
...
try {
	...
	var ci_itemprice=CI_GetValue('CI_ItemPrices',null);
	var ci_itemavailability=CI_GetValue('CI_ItemAvailability',null);
	var av_url='';
	
	if(ci_itemid!==null){
		av_url="&sku=" + ci_itemid + "&ap="+ci_itemprice+"&a="+ci_itemavailability+"&ad=";
	}
	
	if(ci_cpncode!==null){
		ci_CC('ci_cpncode',ci_cpncode,10);
		ci_CC('ci_src',ci_srccode,10);
		ci_CC('ci_tid',"",-1);
	} else if(ci_src!==null && ci_sku!==null) {
		ci_CC('ci_cpncode',"",-1);
		ci_CC('ci_tid',ci_tid,10);
		ci_CC('ci_src',ci_src,10);
