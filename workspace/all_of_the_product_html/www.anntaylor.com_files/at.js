...
function productFormShowRequest(formData, jqForm, options) {
	var form = jqForm[0];
...
function showResponse(responseText, statusText)  {
	...
	if(responseText.pagetype == 'payment'){
		...
		if(responseText.error == "true"){
			...
			processErrors({ 'avs.failed': 'avsFaild', 'phoneNumber': 'phoneNumberInBillingAddress' });
	...
	if(responseText.pagetype == 'regPayment'){
		if(responseText.error == 'true'){
            processErrors({ 'avs.failed': 'avsFaildInRegBilling' });
		}else{
	...
	if(responseText.pagetype == 'guestcheckout'){
		if(responseText.error == 'true'){
			...
			loadFragment("/ann/checkout/checkoutProcess.jsp",".col-secondary",function(){ ATCheckout.init(); });
			loadFragment("/ann/cart/cart.jsp","#includeCart",function(){ ATCheckout.init(); });
		}
	}
...
function loadFragment(url,divId,callback){
	$.ajax({
		url: url,
		cache: false,
		type: "GET",
