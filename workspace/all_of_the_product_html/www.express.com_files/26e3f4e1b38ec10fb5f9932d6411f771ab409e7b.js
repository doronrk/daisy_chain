var adobeOrgId                  = '5F17123F5245B46D0A490D45@AdobeOrg';
var scTrackingServer            = 'metrics.express.com';

// TRACKING CONFIG
var visitor                     = new Visitor(adobeOrgId);
visitor.trackingServer          = scTrackingServer;
visitor.trackingServerSecure    = 's' + scTrackingServer;
...
s.trackingServer		= scTrackingServer;
s.trackingServerSecure	= 's' + scTrackingServer;
s.visitorNamespace 		= 'expressfashion';
s.visitor 				= Visitor.getInstance(adobeOrgId);

s.trackDownloadLinks = true;
s.trackExternalLinks = true;
s.trackInlineStats = true;
s.linkDownloadFileTypes = 'exe,zip,wav,mp3,mov,mpg,avi,wmv,pdf,doc,docx,xls,xlsx,ppt,pptx';
s.linkInternalFilters = 'javascript:,expressfashion.com,express.com,phx.corporate-ir.net';
...
function s_doPlugins(s) {
	...
	if (!s.pageLoaded) {
		...
		s.prop27 = 'http://' + document.location.hostname + document.location.pathname;
		...
		s.prop17 = s.tpEls[0];
		s.prop18 = s.tpEls[1];
		s.prop19 = s.getDaysSinceLastVisit('c19');
		s.prop20 = s.getVisitNum();
		
		// site section and subsection
		if (s.pageName && s.prop3) {
			var pageEls = s.pageName.split(' : ');
			
			if (s.prop3 == 'Product') {
				s.channel = 'Products';
				s.prop1 = s.channel + ' : ' + (pageEls[0] == 'Ensemble Detail' ? 'Ensemble' : 'Solo');
				s.prop2 = s.prop1 + (pageEls[1] ? ' : ' + pageEls[1] : '');
			} else {
				s.channel = pageEls[0];
				s.prop1 = s.prop2 = 'D=ch';
				if (pageEls.length > 1) {
					s.prop1 += '+" : ' + pageEls[1] + '"';
					if (pageEls.length > 2) {
						s.prop2 += '+" : ' + pageEls[1] + ' : ' + pageEls[2] + '"';
					} else {
						s.prop2 = s.prop1;
					}
				}
		...
		// internal search
		if (s.prop12) {
			s.prop12 = s.cleanUrlData(s.prop12.toLowerCase());
			var t_search = s.getValOnce(s.prop12, 'c12', 0);
			if (t_search) {
				if (s.campaign || s.eVar2 || s.eVar13) {
					s.pageName = 'Canned Landing Page';
					s.prop1 = s.prop2 = s.prop3 = 'D=pageName';
					s.prop14 = s.prop12;
					s.prop12 = '';
		...
		if (!s.eVar3) {
			if (s.campaign) {
	            ...
	            s.eVar3 = (s.prop3 == 'Product' ? 'redirected ' : '') + 'search';
			} else if (s.eVar4) {
				s.eVar3 = 'browse';
				...
				s.eVar6 = 'browse';
			} else if (s.purchaseID || s.events.indexOf('scAdd') > -1) {
				s.eVar3 = 'direct load';
	        }
	    }
		...
		if (s.eVar3) {
			if (!s.eVar1) {
				s.eVar1 = s.eVar72 = 'non-internal search';
			}
			if (!s.eVar2) {
	...
	// VARIABLE CONFIG
	s.prop28 = 'D=g';
	s.eVar10 = s.purchaseID ? 'D=purchaseID' : '';
	s.eVar20 = s.prop5 ? 'D=c5' : '';
	s.eVar23 = s.prop6 ? 'D=c6' : '';
	s.eVar37 = s.prop23 ? 'D=c23' : '';
	s.eVar41 = s.prop14 ? 'D=c14' : '';
	s.eVar48 = s.prop24 ? 'D=c24' : '';
	s.eVar58 = s.prop17 ? 'D=c17' : '';
	s.eVar59 = s.prop18 ? 'D=c18' : '';
	s.eVar70 = s.pageName ? 'D=pageName' : '';
	s.eVar71 = s.prop3 ? 'D=c3' : '';
	s.eVar73 = s.prop19 ? 'D=c19' : '';
	s.eVar74 = s.prop20 ? 'D=c20' : '';
	s.eVar75 = s.prop27 ? 'D=c27' : '';
...
s.Media.contextDataMapping = {
	'a.media.name': 'eVar16,prop30'
	,'a.media.view': 'event70'
	,'a.media.complete': 'event71'
	,'a.media.timePlayed': 'event73'
	,'a.media.milestones': {
         50: 'event72'
	}
};
s.Media.trackVars = 'events,eVar15,prop7';
s.Media.trackEvents = 'event70,event71,event72,event73';
...
+"Util.getQueryParam){pm=s.Util.getQueryParam(p);}if(pm){r+=(pm+',');"
+"}if(window[v]!=undefined){r+=window[v];}if(b){window[v]='';}return r;");
