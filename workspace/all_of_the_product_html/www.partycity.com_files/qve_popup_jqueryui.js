	...
	*/
	$.fn.ml_popup = function(dialog_title,close_text,url,type,width,height,showInCenter){
		...
		if ($('#popup-content').length == 0){
			$('body').append(
				overlay	= $('<div id="popup-content"><iframe name="' + frameid + '" frameborder="0" style="border:none;" width="' + (width) + '" height="' + (height) + '" scrolling="no" id="epqvPopup"></iframe></div>')
			)
		}
		else{
			//change iframe's src property
			$('#popup-content').html('<iframe name="' + frameid + '" frameborder="0" style="border:none;" width="' + (width) + '" height="' + (height) + '" scrolling="no" id="epqvPopup"></iframe>');
		}
