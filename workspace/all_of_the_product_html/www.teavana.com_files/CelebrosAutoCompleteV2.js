...
function CelebrosAutoComplete(siteKey, searchBox, resultFunction, frontEndServer, scriptServer, containerExpandDelegate, containerCollapseDelegate) {
    ...
    LoadDefaultImage(sServiceLocation, siteKey);
}
var xmlHttp = null;
...
function AjaxSendBanData(ajaxCommand, callData) {
    if (AjaxInit()) {
        if (isIE8) {
            xmlHttp.onload = AjaxOutputResult;
            xmlHttp.open("GET", ajaxCommand, true);
            xmlHttp.send();
        }
        else {
...
function AjaxOutputResultJSON(r) {
    var data = eval(r);
    defaultImageUrl = data.url;
}
function LoadDefaultImage(sServiceLocation, siteKey) {
    var script = document.createElement("script");
    script.setAttribute("src", sServiceLocation + '?DefaultImage=true&strSiteKey=' + siteKey + "&jsonp_callback=AjaxOutputResultJSON");
...
function initAC(containerExpandDelegate, containerCollapseDelegate) {
    ...
    myAutoComp.formatResult = function (oItem, aResultItem, sQuery) {
        ...
        if (aResultItem[1] == "") {
            ...
            else if (sPath.indexOf('CSXViewResults.aspx') != -1 && imageUrl.indexOf('http://') == -1) {
                try {
                    imageUrl = currentPrefix + imageUrl;
                }
                catch (err) { }
            }
            ...
            ["<table cellpadding=0 cellspacing=0 width=\"100%\" class=\"ACproduct\" style=\"background-color:Transparent;\"><tr><td><table cellpadding=0 cellspacing=0><tr><td class=\"ACProductImageCell\">",
             "<img class=\"ACProductImage\" align=\"center\" onError=\"this.onerror=null;this.src='" + defaultImageUrl + "'\" src=\"", imageUrl, "\" />",
