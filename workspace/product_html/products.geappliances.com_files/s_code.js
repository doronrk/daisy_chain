...
var s_account="geappliancesnewprod"
var s=s_gi(s_account)

/* Google Referrer Change Patch v1.1 */
/* Added 7/23/09 per: http://blogs.omniture.com/2009/05/15/updating-the-sitecatalyst-javascript-code-for-google-search-results-change/  */
...
/*Tracking Server Configuration Section*/

/*s.trackingServer="[INSERT-TRACKING-SERVER-HERE]"*/

s.visitorNamespace="generalelectric"
s.dc="d1" 
...
s.linkDownloadFileTypes="exe,zip,wav,mp3,mov,mpg,avi,wmv,pdf,doc,docx,xls,xlsx,ppt,pptx,dwg"
s.linkInternalFilters="javascript:,geappliances.com,esecurecare.net,sockloss.com,teamsite.indsys.ge.com,appl.ge.com,geappliances--pro.custhelp.com,geappliances.custhelp.com,brightcove.com,rehabstudio.com,geappliances.tumblr.com"
...
function s_doPlugins(s) {
    ...
    // Page Load Time Configuration
	
	s.prop51=s_getLoadTime();
	if(s_getLoadTime())s.events=s.apl(s.events,'event73='+s_getLoadTime(),',',1);
	...
	if(!s.campaign)
		s.campaign=s.getQueryParam('omni_key');
	...
	/*Internal Campaign Klick*/
	if(!s.eVar2){
		s.eVar2=s.getQueryParam('icid')
		var int_cmp=s.getValOnce(s.eVar2,'s_ev2',0);
	if(int_cmp)
		s.events=s.apl(s.events,'event8',',',0);
	}

	/* Product Refinement stuff removed from here */
	/* Is now in s_product_refinement.js */

	/*KB - Narrow by Product Type*/
	if(s.prop49)s.prop49=s.prop49.toLowerCase()
        /* Copy search term to eVar */
        if(s.prop49){
	    s.eVar49=s.prop49
	/* Set de-duped onsite search event */
	var t_search=s.getValOnce(s.eVar49,'ev49',0)
	if(t_search)
		s.events=s.apl(s.events,'event15',',',1)
	}

	/*KB - Narrow by Subject*/
	if(s.prop50) s.prop50=s.prop50.toLowerCase()
        /* Copy search term to eVar */
        if(s.prop50){
	    s.eVar50=s.prop50
	/* Set de-duped onsite search event */
	var t_search=s.getValOnce(s.eVar50,'ev50',0)
	if(t_search)
		s.events=s.apl(s.events,'event15',',',1)
	}
	...
	/*Application Type Being Searched*/
	...
	var tempH =s.getTimeParting('h','-5',year); // Set hour
	var tempD =s.getTimeParting('d','-5',year); // Set day
	var tempW=s.getTimeParting('w','-5',year); // Set Weekend / Weekday

	s.prop6=s.eVar6=tempH+':'+tempD+':'+tempW;

	/*Visit Num*/
	s.eVar7=s.prop7=s.getVisitNum();

	/*Search Performed on Page*/
	var temppage=s.getPreviousValue(s.pageName,'gpv_p13')
	if(s.prop1) {s.prop13=temppage;}

	/*Days Since Last Visit*/
	s.eVar5=s.getDaysSinceLastVisit('s_lv');

	/* File Download Link Handling */
	var s_dlink=new Array();
	s.linkLeaveQueryString=true;
	s_lc=s.split(s.linkDownloadFileTypes,',');
	for(s_f=0;s_f<s_lc.length;s_f++) {
	
		s_dlink[s_f]=s.linkHandler('.'+s_lc[s_f],'d');
		if(s_dlink[s_f]) {
		
			s.prop27=s.pageName?s.pageName:s.wd.location;
			s.linkTrackVars=s.apl(s.linkTrackVars,"prop27",",",2);
		}
	}
}
...
made when instructed to do so by your account manager.*/
    ...
    /*
    * Plugin: getVisitNum - version 3.0 - Return the user visitor number 
    */
...
/*
* Plugin: getTimeParting 3.4 - Used for Time Parting setting 
*/
