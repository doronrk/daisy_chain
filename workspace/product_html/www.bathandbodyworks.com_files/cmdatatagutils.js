/*
 * cmdatatagutils.js
 * $Id: cmdatatagutils.js,v 1.11.16.4 2014/03/11 15:54:25 dmulligan Exp $
 * $Revision: 1.11.16.4 $
 *
 * Version 4.2.0
 *
...
/*
 * Function for thematic page to create hidden fields for orsocode and orsotype
 */
function cmAddOrsoValues(productID){
    var formName = 'order-form' + productID;
    //Create an input type dynamically.
    var element = document.createElement("input");

    //Assign different attributes to the element.
    element.setAttribute("type", 'hidden');
    element.setAttribute("value", orsocode);
    element.setAttribute("name", 'orsoCode');

    var foo = document.getElementsByName(formName)[0];
    ...
    element1.setAttribute("value", orsotype);
    element1.setAttribute("name", 'orsoType');
...
function cmCreateProductviewTag(productID, productName, categoryID, totalReviewCount, avgRating, ratingsOnlyRV, buyAgainPerc,expressProduct,crossSell, searchString, searchResults,attributes) {
    ...
    var thematicPage = URL.split("/");
    //calling the cmAddOrsoValues function for thematic page
    if('familytheme' == thematicPage[3]){
        cmAddOrsoValues(productID);
    }
