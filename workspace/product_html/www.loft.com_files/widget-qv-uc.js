...
persistantCartCommands[0] = "/loft/catalog/basket.jsp";
...
function showBasket(action,params) {
	...
	{
		ucartLoadingHTML = 	'<div class="top ie6png"><!--  --></div>' +
									'<div class="body ie6png"><img class="loading" src="/webassets/loft/en_US/assets/images/img/img-cartAjaxLoading.gif" /></div>' +
	...
	pos[0] = pos[0] - ( 327 - $("#CartToggle").parent().width())+100;
	pos[1] = pos[1] + $("#CartToggle").parent().height();
	/* ------------------- */

	params = "ts=" + timestamp() + "&action=" + action + "&" + params;
	$.getScript("/webassets/loft/en_US/js/jScrollPane.js");
	lib.layer.create(persistantCartContainerId, {
		...
		callback : function() {
			...
			if(typeof sIFR !== 'undefined') { cart_sIFRtidy(); }
			/* --------------- */
			...
			$(persistantCartContainerId).hover(function() {
					...
					tCart = setTimeout(function() { hideBasket() }, 1000);
			});
			/* --------------- */
		}
	});
...
function addToCart() {
	...
	params = params + "&sizeInput=" + $(" input[name=sizeInput]").val()
			+ "&sizeColorInput=" + $(" input[name=sizeColorInput]").val();
...
function updateHeader(amt) {
	$("#CartToggle .total").html("(<strong>" + amt + "</strong>)");
};
...
function addProductFromQuickView(params) {
	...
	params = params + "&sizeInput=" + $(quickViewContainerId + " input[name=sizeInput]").val()
			+ "&sizeColorInput=" + $(quickViewContainerId + " input[name=sizeColorInput]").val();
...
var qvInterval;
function setupQuickViewButtons(flag) {
...
function cartHide() {
	$('.js-cartToggle').hover(function() {
		clearTimeout(tCart);
		if($(persistantCartContainerId).size() == 0) { showBasket('show',''); }
	}, function() {
		tCart = setTimeout(function() { hideBasket() }, 500);
	})
	.kbhover();
};
