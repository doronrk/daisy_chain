...
function populateCommonVars(sessionID,userID,placement0Name,placement1Name,contextPath,productPageType,requestInternational) {
	...
	R3_COMMON.addSegment('LOFT', 'LOFT');
	...
	RR.jsonCallback = function(){
		...
		//#############################################*/
			...
			switch (plcmnt0Name) {
				case 'item_page.loft1' :
				case 'item_page.loft_oos1' :
					if (showOrig) {
				...
				case 'cart_page.loft_empty' :
					...
					break;
				case 'search_page.loft1' :
					if (!showOrig) {
						createPlacement(RR.data.JSON.placements[0], searchTipsRR, isInternational);
					}
					break;
				case 'search_page.loft_noresults1' :
					if (!showOrig) {
						createPlacement(RR.data.JSON.placements[0], searchTipsRRnoRes, isInternational);
					}
					break;
			}	
		}
...
function createPlacement(RRData, placementId, isInternational) {
		...
		a.href = items[i].linkurl;
			...
			a.onclick = function () {
				...
				if (placementId.id == 'rrPlacement1') {
					switch (plcmnt1Name) {
						case 'item_page.loft2' :
							s.prop29 = s.eVar3 = 'Prod Bottom';
							break;
						case 'item_page.loft_oos2' :
				...
				if ((placementId.id == 'rrPlacement0') && ((plcmnt0Name == 'item_page.loft1') || (plcmnt0Name == 'item_page.loft_oos1'))){
					...
					// image
						...
						img.width = 102;
						img.height = 106;
						if (plcmnt0Name == 'search_page.loft1') {
							img.width = 193;
							img.height = 200;
						}
						if ((plcmnt0Name == 'search_page.loft_oos2') || (plcmnt0Name == 'search_page.loft_noresults1')) {
							img.width = 143;
							img.height = 151;
						}
						...
						overlayDiv.setAttribute('id', 'overlay'+i);
							...
							if ((placementId.id == 'rrPlacement0') && ((plcmnt0Name == 'item_page.loft1') || (plcmnt0Name == 'item_page.loft_oos1'))){
								infoDiv.className +=  'info';
									...
									if (placementId.id == 'rrPlacement0') {
											shortDesc = shortDesc.substr(0,35);
									}
							...
							//new code added by Rahul loftINC-1973	
					...
					if ((items[i].rating != '') && (items[i].rating != undefined)) {
						...
						img.height = 18;
						if ((placementId.id == 'rrPlacement0') && ((plcmnt0Name == 'item_page.loft1') || (plcmnt0Name == 'item_page.loft_oos1'))){
							img.src = '/webassets/loft/en_US/assets/images/bazaarvoice/stars_wht/rating-'+items[i].rating.substring(0,1)+'_'+items[i].rating.substring(2)+'.gif';
							img.className += " clearfix";
							infoDiv.appendChild(img);
							//console.log('add to info');
						} else {
							img.src = '/webassets/loft/en_US/assets/images/bazaarvoice/rating-'+items[i].rating.substring(0,1)+'_'+items[i].rating.substring(2)+'.png';
							imgDiv.appendChild(img);
	...
	if (plcmnt0Name == 'cart_page.loft1') {
	// empty h4 for horizontal rule
		h4 = document.createElement('h4');
		h4.innerHTML = '<div id=\'bottomRRLine\'style=\'height: 30px;\'></div>';
	div.appendChild(h4);
	}
