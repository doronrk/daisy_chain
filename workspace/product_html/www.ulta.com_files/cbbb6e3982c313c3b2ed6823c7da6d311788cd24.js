...
var s_account= s_account || ""; //set by brighttag in future
...
s.linkInternalFilters="javascript:,ulta.com,172.18.0.196" //???
...
var dfaConfig = {
    CSID:				'1530693', // DFA Client Site ID
	SPOTID:             '1957541', // DFA Spotlight ID
	tEvar:              'eVar52', // Transfer variable, typically the "View Through" eVar.
	errorEvar:          'eVar53', // DFA error tracking (optional)
	timeoutEvent:       'event20', // Tracks timeouts/empty responses (optional)
	...
	maxDelay:           "850", // The maximum time to wait for DFA servers to respond, in milliseconds.
...
function s_doPlugins(s) 
{
	...
	if(!s.campaign)
		s.campaign=s.getQueryParam('cmpid') //Change "cmpid" to query params you use
	if(!s.campaign)
	...
	/*Email Parameters*/
	if(!s.eVar49)
		s.eVar49=s.getQueryParam('RID')
	if(!s.eVar51)
	s.eVar51=s.getQueryParam('CCID')		

	if(!s.eVar48)
		s.eVar48=s.getQueryParam('ET_LID')
		
	s.prop28=s.getPreviousValue(s.pageName,'gpv','');
	
	s.prop29=s.getPercentPageViewed();
	
		/* Set Previous Page for various reports */
	s.prevPage=s.getPreviousValue(s.pageName,'gpv','');
	/*Search Location*/
	if(s.events&&s.events.indexOf('event1')>-1)
		s.eVar6=s.prevPage;	
	/*Cross Sell Location */
	if(s.eVar4&&s.eVar4!='N/A'&&s.eVar4!='n/a')
	{
		s.linkTrackVars=s.apl(s.linkTrackVars,'eVar26,eVar2',',',1);
		s.eVar26=s.prevPage;
		s.eVar28='';
		s.eVar2='cross sell'
	}
	...
	/* Copy PurchaseID to evar */ 
	s.eVar16=s.purchaseID;
	
	/*Time Parting*/
//	s.eVar24=s.getTimeParting('d','-5');
//	s.eVar25=s.getTimeParting('h','-5','','',1);
//	s.eVar24=s.getValOnce(s.eVar24,'s_ev24',0);
//	s.eVar25=s.getValOnce(s.eVar25,'s_ev25',0);	
 ...
 * n - Number
 * I - Interval returned
 * m = value to begin putting into a range
 */
s.getRange = new Function("n","i","m","if(n == null || n== ''){retu"+
"rn '';}if((m && n!=0 && n<=m)||isNaN(n*1)){return n;}if(n != 0 ){r "+
"= (n/i)-1;r = Math.ceil(r);b = 1 +(r*i);c = i +(r*i);range=b+'-'+c;"+
"return range;}else {return 'zero';}");
...
s.visitorNamespace="ulta"
s.trackingServer="web.ulta.com"
s.trackingServerSecure="sweb.ulta.com"
s.dc=122
...
+"000000000):tm.getTime();p.RAND+=Math.floor(tm.getTime()/10800000);x=0;while(x>=0){x=u.indexOf('[',x);if(x>=0){y=u.indexOf(']',x);if(y>x){z=u.substring(x+1,y);if(z.length>2&&z.substring(0,2)=='s."
...
+"sitorSampling,g=s.visitorSamplingGroup,k='s_vsn_'+s.un+(g?'_'+g:''),n=s.c_r(k),e=new Date,y=e.getYear();e.setYear(y+10+(y<1900?1900:0));if(v){v*=100;if(!n){if(!s.c_w(k,x,e))return 0;n=x}if(n000>"
...
+":tm.getTime(),sess='s'+Math.floor(tm.getTime()/10800000)+sed,y=tm.getYear(),vt=tm.getDate()+'/'+tm.getMonth()+'/'+(y<1900?y+1900:y)+' '+tm.getHours()+':'+tm.getMinutes()+':'+tm.getSeconds()+' '+"
