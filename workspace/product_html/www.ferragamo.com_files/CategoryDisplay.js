...
getDefaultItem : function(productId){
	...
	**/
	...
	/**
	* VARA - VARINA
	* AddItem2VarinaShopCartAjax This function is used to add a single or multiple items to the shopping cart using an ajax call.
							If an array is passed for partNumber and quantity parameters, then multiple items can be added.	In this case, partNumber[i] corresponds to quantity[i]
							Else, partNumber and quantity parameters represent a single catalog entry.
	*
	* @param {Array|String} partNumber An array of partNumbers or a single partNumber of the item to add to the cart.
	* @param {Array|int} quantity An array of quantities corresponding to the partNumber array or a single quantity of the item to add to the cart.
	* @param {Object} customParams - Any additional parameters that needs to be passed during service invocation.
	* 
	* @author Alberto Samorì
	**/
	AddItem2VarinaShopCartAjax : function(partNumber, quantity, customerCode, salesAssistant, shopCode)
	{
		var params = [];
		params.storeId		= this.storeId;
		params.catalogId	= this.catalogId;
		...
		params.salesAssistant = salesAssistant;
		params.shopCode = shopCode;
		var ajaxShopCartService = "AjaxAddOrderVarinaItem";
		var nonAjaxShopCartService = "AjaxAddOrderVarinaItem_shopCart";
		
		if(dojo.isArray(partNumber) && dojo.isArray(quantity)){
			for(var i=0; i<partNumber.length; i++){
				if(!isPositiveInteger(quantity[i])){
					MessageHelper.displayErrorMessage(MessageHelper.messages['QUANTITY_INPUT_ERROR']);
					return;
				}
				params["partNumber_" + (i+1)] = partNumber[i];
				params["quantity_" + (i+1)]	= quantity[i];
			}
		}
		else{
	...
	/**
	* FIAMMA
	* AddItem2FiammaShopCartAjax This function is used to add a single or multiple items to the shopping cart using an ajax call.
							If an array is passed for partNumber and quantity parameters, then multiple items can be added.	In this case, partNumber[i] corresponds to quantity[i]
							Else, partNumber and quantity parameters represent a single catalog entry.
	*
	* @param {Array|String} partNumber An array of partNumbers or a single partNumber of the item to add to the cart.
	...
	* @author Alberto Samorì
	**/
	AddItem2FiammaShopCartAjax : function(partNumber, quantity, customerCode)
	{
		var params = [];
		params.storeId		= this.storeId;
		params.catalogId	= this.catalogId;
		...
		params.inventoryValidation = "true";
		params.customerCode = customerCode;
		var ajaxShopCartService = "AjaxAddOrderFiammaItem";
		var nonAjaxShopCartService = "AjaxAddOrderItem_shopCart";
