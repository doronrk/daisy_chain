...
ServicesDeclarationJS = {
	...
	redirectToStep1:function(){
		document.location.href = "OrderShippingBillingView?storeId=" + ServicesDeclarationJS.storeId + "&catalogId=" + ServicesDeclarationJS.catalogId + "&langId=" + ServicesDeclarationJS.langId;
	}
}
	...
	* @constructor
	 ...
	 */
	wc.service.declare({
		id: "AjaxSetCurrencyPreference",
		actionId: "AjaxSetCurrencyPreference",
		url: getAbsoluteURL() + "AjaxSetCurrencyPreference",
		formId: ""
    /**
     * display a success message
     * @param (object) serviceResponse The service response object, which is the
     ...
     */
		,successHandler: function(serviceResponse) {
			var ean = new RegExp('[\\?&amp;]ean=([^&amp;#]*)').exec(serviceResponse.URL);
			var configuration = new RegExp('[\\?&amp;]configuration=([^&amp;#]*)').exec(serviceResponse.URL);
			
			shoppingActionsJS.AddItem2ShopCartAjaxMy(ean[1],1, configuration[1]);
		}
     /**
     * display an error message
     * @param (object) serviceResponse The service response object, which is the
   ...
   */
	...
	/**
	 * Add a varina item to a shopping cart in Ajax mode. A message is displayed after
	 * the service call.
	 * 
	 * @author Alberto Samorì
	 */
	wc.service.declare({
		id: "AjaxAddOrderVarinaItem",
		actionId: "AjaxAddOrderVarinaItem",
		url: getAbsoluteURL() + "AjaxOrderVarinaChangeServiceItemAdd",
		formId: ""
     ...
     */
		...
		,successHandler: function(serviceResponse) {
			...
			MessageHelper.hideAndClearMessage();
			//Nascosto il messaggio relativo alla conferma di aggiunta del prodotto al carrello
			//MessageHelper.displayStatusMessage(MessageHelper.messages["SHOPCART_ADDED"]);
			cursor_clear();
			if(categoryDisplayJS){
	...
	/**
	 * Add a fiamma item to a shopping cart in Ajax mode. A message is displayed after
	 * the service call.
	 * 
	 * @author Alberto Samorì
	 */
	wc.service.declare({
		id: "AjaxAddOrderFiammaItem",
		actionId: "AjaxAddOrderFiammaItem",
		url: getAbsoluteURL() + "AjaxOrderFiammaChangeServiceItemAdd",
		formId: ""
	...
	/* Questo successHandler non era presente: senza la pagina rimane in wait*/
		,successHandler: function(serviceResponse) {
			//MessageHelper.hideAndClearMessage();
     ...
     */
		,successHandler: function(serviceResponse) {
			...
			{
				// Verifico se NON sono nell'area "My Shop" tramite la persenza di un hidden in pagina 
				if ( !document.getElementById('disableRedirectAfterCartItemDelete') || !document.getElementById('disableRedirectAfterCartItemDelete').value=='true'  ) {
		...
		,failureHandler: function(serviceResponse) {
			...
			if (serviceResponse.errorMessage) {
				MessageHelper.displayErrorMessage(serviceResponse.errorMessage);
				 ...
				 if (serviceResponse.errorMessageKey) {
					if(MessageHelper.messages[serviceResponse.errorMessageKey])
     ...
     */
		,failureHandler: function(serviceResponse) {
			...
			var counter = "0";
			var errorMessageKey = "";
			var strResponse = serviceResponse.errorMessageKey;
			
			var showMessages = true;
			
			//Se trovo traccia del delimitaroe specifico di KR e non trovo quello specifico di BancaSella
			if (strResponse != null && strResponse.indexOf("##") < 0  && strResponse.indexOf("$|$") > 0 ){
				var strResponseArr = strResponse.split("$|$");
				var respCode = strResponseArr[0];
				var respDescr = strResponseArr[1];
				alert(respDescr);
			}
			else {
				var strResponseArr = strResponse.split("##");
				...
				if(respCode != null && respCode != ""){
					// Se il codice è uno di quelli di Cybersource
					if (respCode.indexOf('US_') == 0 ){
						// Disabilito la dialog con il messaggio di errore standard
						showMessages = false;
						
						if(respCode == 'US_101'||respCode == 'US_102'){
							alert(document.getElementById('cardAuthFailReason').value
							+document.getElementById('CSResponse_1').value);
						}
						else if(respCode == 'US_151'||respCode == 'US_152'||respCode == 'US_207'||respCode == 'US_250'||respCode == 'US_234'){
							alert(document.getElementById('cardAuthFailReason').value
							+document.getElementById('CSResponse_2').value);
						}
						else if(respCode == 'US_200'||respCode == 'US_201'||respCode == 'US_203'||respCode == 'US_205'||respCode == 'US_208'||respCode == 'US_221'||respCode == 'US_230'||respCode == 'US_231'||respCode == 'US_232'||respCode == 'US_233'){
							alert(document.getElementById('cardAuthFailReason').value
							+document.getElementById('CSResponse_3').value);
						}
						else if(respCode == 'US_202'){
							alert(document.getElementById('cardAuthFailReason').value
							+document.getElementById('CSResponse_4').value);
						}
						else if(respCode == 'US_204'||respCode == 'US_210'){
							alert(document.getElementById('cardAuthFailReason').value
							+document.getElementById('CSResponse_5').value);
						}
						else if(respCode == 'US_209'){
							alert(document.getElementById('cardAuthFailReason').value
							+document.getElementById('CSResponse_6').value);
						}
						else if(respCode.indexOf('US_211') != -1){
							counter = respCode.substring(7);
							alert(document.getElementById('cardAuthFailReason').value
							+document.getElementById('CSResponse_7').value);
						}
						else if(respCode == 'US_240'){
							alert(document.getElementById('cardAuthFailReason').value
							+document.getElementById('CSResponse_8').value);
						}
						else if(respCode == 'US_150'){
							alert(document.getElementById('cardAuthFailReason').value
							+document.getElementById('CSResponse_9').value);
						}else if(respCode.indexOf('US_231') != -1){
							counter = respCode.substring(7);
							alert(document.getElementById('cardAuthGenericMesg').value);
						}else{
							if(document.getElementById(errorMessageKey) != null){
								if(!(document.getElementById('VbVResultCode') != null && document.getElementById('VbVResultCode').value == respCode)){
									alert(document.getElementById(errorMessageKey).value+document.getElementById('cardAuthCheckDetail').value);
								}
							}else{
								alert(document.getElementById('cardAuthGenericMesg').value);
							}
				...
				//For redirecting to the Card issuer Bank's site in case of VbV transaction
				if(document.getElementById('VbVResultCode') != null && document.getElementById('VbVResultCode').value == respCode){
					var vbvRisp = strResponseArr[1];
					var merchantId = strResponseArr[2];
					var transKey = strResponseArr[3];
					//var fiscalCode = strResponseArr[4];
					
					document.getElementById("vbvRisp").value = vbvRisp;					
					document.getElementById("merchantId").value = merchantId;
					document.getElementById("transKey").value = transKey;
					//document.getElementById("FGfiscalCode").value = fiscalCode;
					
					// Disabilito la dialog con il messaggio di errore standard
					showMessages = false;
					
					document.vbvDataForm.submit();
				
				}else{
					if(showMessages) // add timeout before redirect
						window.setTimeout("ServicesDeclarationJS.redirectToStep1()", 3000);
					else
						ServicesDeclarationJS.redirectToStep1();
				}
			}
				
			if (showMessages) {
				// higher priority to "in page" messages
				var inPageMessage = MessageHelper.messages[serviceResponse.errorMessageKey];
				if(inPageMessage != null && inPageMessage != undefined){
					MessageHelper.displayErrorMessage(inPageMessage);
    ...
    */
     ...
     */
		,successHandler: function(serviceResponse) {
			/*start custom*/
			CheckoutHelperJS.saveOrderItemsList('-1',serviceResponse.addressId[0]);
			/*end custom*/
			...
			/*start custom*/
			//if (serviceResponse.addressId!=null && serviceResponse.addressId.length > 0 )
			//	document.location.href = "OrderShippingBillingView?storeId=" + SBServicesDeclarationJS.storeId + "&catalogId=" + SBServicesDeclarationJS.catalogId + "&langId=" + SBServicesDeclarationJS.langId + "&shippingSelectedAddressId="+serviceResponse.addressId[0];
			/*end custom*/
	...
	wc.service.declare({
		id: "FrgAjaxUpdateAddressForPerson",
		actionId: "FrgAjaxUpdateAddressForPerson",
		url: getAbsoluteURL() + "AjaxPersonChangeServiceAddressUpdate",
		formId: ""
     ...
     */
		,successHandler: function(serviceResponse) {
			...
			//if (debug || !$jq("#frgChooseExisting").is(':checked')) {
				AddressHelper.saveShopCartAddress("AjaxAddAddressForPerson", "shippingAddressCreateEditFormDiv_1");
			//}
