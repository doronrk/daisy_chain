	...
	if (app) {
		...
		app.Product = function(response) {
			...
			var getAddToCartBtn = function(thisProduct) {
				var addToCartBtn = jQuery(thisProduct.containerId+" .addtocartbutton:last").click(function(e) {
					...
					if (model.productSet || thisProduct.selectedOptions.Quantity > 0) {
						...
						thisProduct.selectedOptions.SelectedCategory = jQuery("#SelectedCategory").val();
			...
			var getTabs 		= function(containerId) {
				...
				tabsDiv.tabs({
					select: function() {
			         	...
			         	// No print button with reviews:
			         	var i = tabsDiv.tabs('option','selected');
			         	
			         	product_checkTab(tabsDiv);
			...
			var getMiscLinks 	= function(thisProduct) {
				...
				var disablelinks = function() {
					if ((model.master || model.variant) && thisProduct.selectedVar == null) {
						...
						//jQuery(thisProduct.disableA2CButton);
					}
				}
				...
				jQuery(thisProduct).bind("AddtoCartEnabled", {}, function(e, source){
					...
					//LYONS: disabled, was double triggering the enabling of the button
					//jQuery(thisProduct.enableA2CButton);
				});
				...
				jQuery(thisProduct.containerId+" .sendtofriend").click(function(e) {					
                    ...
                    // Safari disabled the selectboxes forever
					// when another dialog is shown above the quickview
					// dialog.
					// So after closing the size chart also close the quick
					// view and directly open it again to the select boxes
					// work again.
                    // https://www2.hosted-projects.com/bugzilla/bootbarn/show_bug.cgi?id=174
					if(jQuery.browser.safari) {
						jQuery( "#dialogcontainer" ).bind( "dialogclose", function(event, ui) {
			...
			var getRatingSection = function(containerId) {
				var tapTopPosition = jQuery(containerId+" #pdpTabsDiv").offset().top;
				jQuery(containerId+" #pdpReadReview").click(function(e) {
					jQuery(containerId+" #pdpTabsDiv").tabs("select", "pdpReviewsTab");
					jQuery('html, body').animate({scrollTop:tapTopPosition}, 'slow');
				} );
				
				
				
				/*// When #pdpReadReview is clicked
				jQuery(containerId+" #pdpReadReview").click(function(){
					// Scroll down to 'tapTopPosition'
			...
			var createAvMessage = function(thisProduct, val) {
				...
				//var shippingDays = thisProduct.getShippingDays();
				...
				//LYONS: avLevels was modified in availabilityjson.isml
				var availabilityThreshhold = 1;
				if (avLevels["availabilityThreshhold"] != "null") {
					availabilityThreshhold = avLevels["availabilityThreshhold"];
				}
				var onHandQuantity = avLevels["onHandQuantity"];
				var vendorQuantity = avLevels["vendorQuantity"];
				var siteName = avLevels["siteName"];
				var vendorName = avLevels["vendorName"];
				var vendorText = avLevels["vendorText"];
				var siteLinkHref = avLevels["siteLinkHref"];
				var vendorLinkHref = avLevels["vendorLinkHref"];
				var inStockMsg = "";
				var vendorShipsFrom = "Ships in 3-5 business days from " + vendorName;
				if (avLevels["vendorShipsFrom"] != "null") {
					vendorShipsFrom = avLevels["vendorShipsFrom"];
				}
				...
				if (avStatus === app.constants.AVAIL_STATUS_BACKORDER ||
					...
					if (val > ats && ats > 0)
					{
						avMessage = avMessage + "" + jQuery.format(app.resources["QTY_"+avStatus], ats);
					}
					...
					avMessage = "";
					if (inStockLevel > 0) {
						avMessage = avMessage + "" + jQuery.format(app.resources["QTY_"+app.constants.AVAIL_STATUS_IN_STOCK], inStockLevel);
					}
					if (backOrderLevel > 0) {
						avMessage = avMessage + "" + jQuery.format(app.resources["QTY_"+app.constants.AVAIL_STATUS_BACKORDER], backOrderLevel);
						// uncomment the following line to display availability message with back order information
						// avMessage = avMessage + getInStockDateMsg(thisProduct);
					}
					if (preOrderLevel > 0) {
						avMessage = avMessage + "" + jQuery.format(app.resources["QTY_"+app.constants.AVAIL_STATUS_PREORDER], preOrderLevel);
						// uncomment the following line to display availability message with back order information
						// avMessage = avMessage + getInStockDateMsg(thisProduct);
					} 
				}
				...
				//console.log(avLevels);
				if(onHandQuantity==null || vendorQuantity==null){
					return avMessage;
				}else if(onHandQuantity>0){
					if (onHandQuantity <= availabilityThreshhold) {
						inStockMsg = "Only " + onHandQuantity + " left. "
					}
					return inStockMsg + getShippingMessage(avLevels);
					//return "Ships from <a href='#' id='availToolTipLink' >"+siteName+"</a><div id='availText' style='display:none;'>"+siteText+"</div>";
				}else if((onHandQuantity == null || onHandQuantity <= 0) && vendorQuantity > 0){
					return "<a href='#' id='availToolTipLink' >"+vendorShipsFrom+"</a><div id='availText' style='display:none;'>"+vendorText+"</div>";     
				}else{
					//should have no inventory...
				}
			...
			var setAvailabilityMsg = function(msg) {
				...
				jQuery(myContainerId+" .availability .value").html(msg);
				//jQuery(".availability .label").show();
				// Overwrite availability message with custom info
				// function defined in product.isml
				if( jQuery(".availability .value").html() == "${Resource.msg('global.allnotavailable','locale',null)}") {
					var inventoryMessage = "${Resource.msg('global.lowinventory','locale',null)}";
					jQuery(".availability").html('<span class="value unavailable" style="font-weight: bold;">' + inventoryMessage + '</span>');
				}
				//LYONS: hide .label
				else if(jQuery(".availability .value").text().match("^Ships from:")){//regex "^Ships from"
					jQuery(".availability .label").hide();
				}
				
				//LYONS: setup tooltip if it exists
				app.tooltip({id: "availToolTipLink", options: {
					showURL:false,
					bodyHandler: function() {
						var theText = jQuery("#availText").html();
						return $("<div style='border:1px solid #bbb;background-color:#fff;padding:6px;'>"+theText+"</div>");
					}
				}});
			}
			...
			var getSizeChart = function() {
				jQuery(".attributecontentlink").click(function(e){
					...
					// https://www2.hosted-projects.com/bugzilla/bootbarn/show_bug.cgi?id=174
					if(jQuery.browser.safari) {
						jQuery( "#sizeChartDialog" ).bind( "dialogclose", function(event, ui) {
							jQuery("#QuickViewDialog").dialog('close');
							window.setTimeout(function () {
								jQuery("#QuickViewDialog").dialog('open');
							}, "1000");
			...
			return {
				...
				enableA2CButton: function() {
					...
					jQuery(this.containerId+" .addtocartbutton:last img.enabled").show();
					jQuery(this.containerId+" .addtocartbutton:last img.disabled").hide();
					
					
					jQuery(this.containerId+" .addtocartbutton").attr("title" , "");
					jQuery(this.containerId+" .addtocartbutton").css("cursor", "pointer");

					//LYONS: for findToCart
					jQuery(".addtocartbutton").triggerHandler("FindInStoreEnabled",this);
				},
				...
				disableA2CButton: function() {
					...
					jQuery(this.containerId+" .addtocartbutton:last img.enabled").hide();
					jQuery(this.containerId+" .addtocartbutton:last img.disabled").show();
					jQuery(this.containerId+" .addtocartbutton").css("cursor", "default");

					//LYONS: for findToCart
					jQuery(".addtocartbutton").trigger("FindInStoreDisabled");
				},
				...
				showSelectedVarAttrVal: function(varId, val) {
					...
					/*jQuery("li[class='emptyswatch selected']").find('span.selectedvarval').html('Click a button below');*/
						if (varId === id) {
							jQuery(this).find('span.selectedvarval').html(val);
						}
						/*else {
							jQuery(this).find('span.selectedvarval').html('Click a button below');
						}*/
					});
				},
					...
					jQuery.each(vals, function(){
						...
						if (this.val === selectedVal && this.images) {
							if (this.images.small.length > 0) {
								...
								jQuery(".largeproductthumbnails").html("");
								if(selectedVal != "") {
									jQuery(that.containerId+" .productimage").html("").append(jQuery("<img/>").attr("src", thisVal.images.medium[0].url).attr("alt", thisVal.images.medium[0].alt).attr("title", thisVal.images.medium[0].title));
									jQuery(".largeproductimage").html("").append(jQuery("<img/>").attr("src", thisVal.images.large[0].url ).attr("alt", thisVal.images.large[0].alt).attr("title", thisVal.images.large[0].title));
								}
							}
							// make sure to show number of images based on the smallest of large or small as these have to have 1-1 correspondence.
							if (this.images.small !== undefined) {
								var noOfImages = this.images.medium.length >= this.images.swatch.length ? this.images.swatch.length : this.images.medium.length;
								foo = this.images;
								...
								if ((this.images.small.length > 1 || that.isSubProduct()) && !that.isBonusProduct()) {
									jQuery.each(this.images.small, function(){
										...
										// hover for thumbnails on pdp: change product image (large+medium)
										jQuery(that.containerId+" .productthumbnails:last").append(jQuery("<img/>").attr("src", this.url).attr("alt", this.alt).attr("title", this.title).hover(function(e){
											jQuery(that.containerId+" .productimage").html("").append(jQuery("<img/>").attr("src", thisVal.images.medium[imageInd].url).attr("alt", thisVal.images.medium[imageInd].alt).attr("title", thisVal.images.medium[imageInd].title));
											jQuery(".largeproductimage").html("").append(jQuery("<img/>").attr("src", thisVal.images.large[imageInd].url).attr("alt", thisVal.images.large[imageInd].alt).attr("title", thisVal.images.large[imageInd].title));
											jQuery(".productthumbnails img").removeClass("active");
											jQuery(jQuery(".productthumbnails img")[imageInd]).addClass("active");
											jQuery(".largeproductthumbnails img").removeClass("active");
											jQuery(jQuery(".largeproductthumbnails img")[imageInd]).addClass("active");
										}));
										// on click for thumbnails on pdp: open overlay
										jQuery(that.containerId+" .productthumbnails img:last").click(function(){
											app.openImageOverlay();
										}); 
										// on click for thumbnails on zoom view: change product image (large+medium)
										jQuery(".largeproductthumbnails").append(jQuery("<img/>").attr("src", this.url).attr("alt", this.alt).attr("title", this.title).hover(function(e){
											jQuery(that.containerId+" .productimage").html("").append(jQuery("<img/>").attr("src", thisVal.images.medium[imageInd].url).attr("alt", thisVal.images.medium[imageInd].alt).attr("title", thisVal.images.medium[imageInd].title));
											jQuery(".largeproductimage").html("").append(jQuery("<img/>").attr("src", thisVal.images.large[imageInd].url).attr("alt", thisVal.images.large[imageInd].alt).attr("title", thisVal.images.large[imageInd].title));
											jQuery(".productthumbnails img").removeClass("active");
											jQuery(jQuery(".productthumbnails img")[imageInd]).addClass("active");
				...
				*/
				resetVariations: function() {
					...
					jQuery(this.containerId + " .variationattributes .swatches").each(function(){
						...
						//srw
						jQuery(this).find("li.emptyswatch").each(function(){
							// find A variation with this val
							if (that.isVariation({id:dataa, val:jQuery(this).attr("title")})) {
								// found at least 1 so keep it enabled
								jQuery(this).parent().removeClass("selected");
				...
				* */
						...
						if (swatchId === attrId) {
							...
							jQuery(this).find("a.swatchanchor").each(function(){
								...
								if (filteredVariants.length > 0) {
									...
									// no variant found with this value combination so disable it, excepted colors should always be clickable
									if (swatchId != "color") {
									...
									parentLi.removeClass("selected"); //xxx
									//parentLi.showSelectedVarAttrVal(attrId, "Click a button below");
								}
							});
						}
					});
				...
				refreshView: function() {
					...
					if (!isLoadingVar && this.selectedVar != null) {
						...
						// update availability
							...
							setAvailabilityMsg(createAvMessage(thisProduct, 1));
							/*
							if( getATS( model.variations.variants ) == 0 ) {
								console.log(model.variations.variants);
					...
					if (nonSelectedVars.length > 0) {
						...
						if( getATS( model.variations.variants ) == 0 && !model.inStock) {
							setAvailabilityMsg(createAvMessage(thisProduct, 1));
						} else {
							availMsg = jQuery.format(app.resources["MISSING_VAL_1"] + app.resources["MISSING_VAL_2"], tooltipStr);
							setAvailabilityMsg(availMsg);
						}
						
						jQuery(thisProduct.containerId+" .addtocartbutton:last").attr("title", availMsg.replace("<span class='val'>","").replace("</span>",""));
					}					
				},
				...
				showUpdatedPrice: function(sale, standard) {  
					...
					if (standardPrice > 0 && standardPrice > salePrice) {
						...
						priceHtml = (salePrice > 0 || this.isPromoPrice()) ? '<div class="mainproductprice"><div class="promoprice"> ' + formattedPrices.salePrice + ' </div>' : '<div class="mainproductprice"><div class="promoprice"> N/A </div>';						
						// show both prices xxx = & +=
						priceHtml += '<div class="standardprice"> ' + formattedPrices.standardPrice + ' </div></div>';
					} else {
						// in case it is a promotional price then we do not care if it is 0
						...
						jQuery(thisProduct.containerId + " .variationattributes .swatches").each(function(){
							...
							var varEventHandler = function(e){									
								...
								if (thisObj.parent().hasClass("unselectable")) {
									...
									// deselection  modified by Anjum... Now swatches do not toggle for selection and deselection
...
//									thisProduct.varAttrSelected(e);
									...
									//deselect all other swatches by clicking on a new color
									if (pdpVarId == "color") {
										jQuery(thisProduct.containerId + " .variationattributes .swatches").each(function(){
											var thisSwatch 	= jQuery(this);
											var varId 	= thisSwatch.data("data"); // data is id which is set via app.hiddenData onload
											if (varId != "color") {
												thisProduct.showSelectedVarAttrVal(varId, "Click a button below");
												thisProduct.selectedVarAttribs[varId] = null;
											}
										});
										
										thisSwatch.parent().find(".selected").removeClass("selected");//xxx
									}
							...
							if (pdpVarId === "color") {
								...
								.hover(function(e){
									...
									//modified by Anjum : if hover on color, nothing should be changed. 
									//AN: anything should only be changed on the click of mouse or touch on ipad
									// next 3 lines are commented to acheive this functionality. 
...
// show/hide recommendation tooltip and correct vertical offset
function recommendationTooltip(pid, show) {
	var offset = jQuery("#" + pid + " .pdpTooltip").height();
	jQuery("#" + pid + " .pdpTooltip").css('bottom', offset - 20);
	if( show ) {
		jQuery("#" + pid + " .pdpTooltip").show();
	} else {
		jQuery("#" + pid + " .pdpTooltip").hide();
	}
			
	
}

function techfeatureTooltip(feature, show) {
	
	if (show && feature=="addtocart") app.ProductCache.refreshView();
	var offset = jQuery("#techTooltip_" + feature ).height();
	jQuery("#techTooltip_" + feature ).css('margin-top', -(offset-130));
	if( show ) {
		//jQuery("#techTooltip_" + feature ).show();
		var el = document.getElementById("techTooltip_" + feature);
		el.style.display = "inline";
		//el.style.left = "inline";


	} else {
		//jQuery("#techTooltip_" + feature ).hide();
		var el = document.getElementById("techTooltip_" + feature);
...
function beforeCutoffTime(cutoffTime)
{
	var beforeCutoff = false; 
	var currentDate = new Date();
	//alert("utc hours: " + currentDate.getUTCHours() + " current hours: " + currentDate.getHours());
	if (currentDate.getHours() < new Number(cutoffTime)) {
		beforeCutoff = true;
	}
...
function getShippingMessage(avLevels)
{
	var currentDate = new Date();
    var today = currentDate.getDay();
	var afterShippingCutoff = "Ships next business day";
	if (avLevels["shippingCutoffTextAfter"] != "null") {
		afterShippingCutoff = avLevels["shippingCutoffTextAfter"];
	}
	
	var beforeShippingCutoff = "Ships today if ordered by 12pm PT/3pm ET";
	if (avLevels["shippingCutoffTextBefore"] != "null") {
		beforeShippingCutoff = avLevels["shippingCutoffTextBefore"];
	}
	var cutoffTime = avLevels["shippingCutoffTime"]; 
	var shipToday = false;
	var shippingMsg = ""; 
	var siteText = avLevels["siteText"];

	switch (today) {
	case 0:
		if (avLevels["shipOnSunday"] == "true") {
			shipToday = true;
		};
		break;
	case 1:
		if (avLevels["shipOnMonday"] == "true") {
			shipToday = true;
		};
		break;
	case 2:
		if (avLevels["shipOnTuesday"] == "true") {
			shipToday = true;
		};
		break;
	case 3:
		if (avLevels["shipOnWednesday"] == "true") {
			shipToday = true;
		};
		break;
	case 4:
		if (avLevels["shipOnThursday"] == "true") {
			shipToday = true;
		};
		break;
	case 5:
		if (avLevels["shipOnFriday"] == "true") {
			shipToday = true;
		};
		break;
	case 6:
		if (avLevels["shipOnSaturday"] == "true") {
			shipToday = true;
		};
		break;
	}
	
	if (beforeCutoffTime(cutoffTime) && shipToday) {
		shippingMsg = " <a href='#' id='availToolTipLink' >"+beforeShippingCutoff + " </a><div id='availText' style='display:none;'>"+siteText+"</div>";
	}
	else {												
		shippingMsg = " <a href='#' id='availToolTipLink' >"+afterShippingCutoff + " </a><div id='availText' style='display:none;'>"+siteText+"</div>";						
	}
