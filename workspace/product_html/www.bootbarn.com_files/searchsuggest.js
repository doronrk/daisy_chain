	...
	if (app) {
		...
		app.searchsuggest = {
			...
			init : function(formId, fieldId, fieldDefault, resultsId, url) {
				...
				app.searchsuggest.acSearchForm = jQuery(app.searchsuggest.acFormId);
				...
				jQuery.getJSON(reqUrl, function(json) {
					...
					if(ansLength > 0) {
						...
						for(i=0; i < ansLength; i++) {
							newData += "<div class=\"unselected\"><span class=\"suggestionterm\">" + json.suggestions[i].suggestion + "</span>";
			...
			repositionResultsDiv : function()
			{
				...
				var inPos = app.searchsuggest.acSearchForm.offset();
				var inTop = inPos.top;
				var inLeft = inPos.left;
				
				// get the field size
				var inHeight = app.searchsuggest.acSearchField.height();
				var inWidth = app.searchsuggest.acSearchForm.width();
				...
				app.searchsuggest.acResultsDiv.css("top", inTop + inHeight + 5);
				app.searchsuggest.acResultsDiv.css("width", inWidth); // to tweak
