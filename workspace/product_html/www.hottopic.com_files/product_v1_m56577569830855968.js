...
var addItem = function(addToWishList) {
	...
	//----- Prepare data -----
	var dataString = $("form[name='frm_ht_product_add_to_bag_wishlist']").serialize();
	dataString += "&qty=1";
	var selectedPrdCd = "";
	var currPrdIdSkuId = "";
	...
	$.ajax({
		url: "/hottopic/htStore/ajax/addedToBagWishList.jsp",
		type: "POST",
		data: dataString,
		dataType: "json",
		async: false,
		...
		success: function(msg) {
			try {
			    // Check for shopping bag and skip the notification
			    if(window.location.href.indexOf("ShoppingCart.jsp") > 0) {
				...
				if(domContainer == "iframe")
					itemTemplate = $(lineItemDom, window.parent.document).html();
				else
				...
				$.each(msg, function(idx, data) {
				    // Do not show Roundup Foundation SKU in the confirmation popup
                    if(data.prodCd == roundUpPrdCd)
                        return true;
				...
				if($("p#justAddedMsg", dataRootDom) || $("p#justAddedMsg", window.parent.document)) {
					var tMsg;
					if(domContainer == "iframe")
						tMsg = $("p#justAddedMsg", window.parent.document).html();
					else
						tMsg = $("p#justAddedMsg", dataRootDom).html();

					tMsg = utils.replaceAll(tMsg, "justAddedTitle", firstItemTitle);
					tMsg = utils.replaceAll(tMsg, "cartTotalItems", totalItems);

					if(domContainer == "iframe")
						$("p#justAddedMsg", window.parent.document).html(tMsg);
					else
						$("p#justAddedMsg", dataRootDom).html(tMsg);

					var tmp
					if(domContainer == "iframe")
						tmp = $("p#justAddedMsg", window.parent.document).html().indexOf(" items");
					else
						tmp = $("p#justAddedMsg", dataRootDom).html().indexOf(" items");

					if(tmp > 0) {
						if(domContainer == "iframe") {
							var space = $("p#justAddedMsg", window.parent.document).html().slice(0,tmp).lastIndexOf(" ");
							var correctedMsg = $("p#justAddedMsg", window.parent.document).html().slice(0,space) + " " + totalItems + $("p#justAddedMsg", window.parent.document).html().slice(tmp);
							$("p#justAddedMsg", window.parent.document).html(correctedMsg);
						} else {
							var space = $("p#justAddedMsg", dataRootDom).html().slice(0,tmp).lastIndexOf(" ");
							var correctedMsg = $("p#justAddedMsg", dataRootDom).html().slice(0,space) + " " + totalItems + $("p#justAddedMsg", dataRootDom).html().slice(tmp);
				...
				if(allItemTotal > 0) {
					if(domContainer == "iframe")
						$("div.j_sub span.b", window.parent.document).html("$"+allItemTotal);
					else
				...
				if(domContainer == "iframe") {
					$(targetRootDom, window.parent.document).html(allItems.join(""));
				...
				if(!addToWishList) {
					if(domContainer == "iframe") {
						var $span = $('div#sticky-nav div#top-bag a', window.parent.document).find('span');
						$('div#sticky-nav div#top-bag a', window.parent.document).html("");
						$('div#sticky-nav div#top-bag a', window.parent.document).append($span);
						$('div#sticky-nav div#top-bag a', window.parent.document).append(" [ " + totalItems + " ITEMS ] $"+allItemTotal);
					} else {
						var $span = $('div#sticky-nav div#top-bag a').find('span');
						$('div#sticky-nav div#top-bag a').html("");
						$('div#sticky-nav div#top-bag a').append($span);
						$('div#sticky-nav div#top-bag a').append(" [ " + totalItems + " ITEMS ] $"+allItemTotal);
					}
				}
                ...
                if(domContainer == "iframe") {
                    ...
                    window.setTimeout(function() {$(slideDownContent).fadeOut(200)}, slideUpTimeout);
                    $.colorbox.close();
...
function loadAndInitControls() {
	...
	$(document.body).delegate('.alternates a.alt','hover',function(event){
			...
			$(this).siblings(".alt").fadeTo(50, 0.4);
