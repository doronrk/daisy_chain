...
function hideQuickView(img_name) {
	...
	function addQV2CartAjax(form) {
		...
		if(form.AjaxOrderItemAddUrl != undefined) {
			wc.service.getServiceById("AjaxOrderItemAdd").url = form.AjaxOrderItemAddUrl.value;
		}
		...
		categoryDisplayJS.addQV2CartAjax(params);

	}

function redirectToR2net(url)
{	
	var ringSizeElem = document.getElementById('ring-size');
		
	var ringSize = 	ringSizeElem.options[ringSizeElem.selectedIndex].value;	
	location.href = url+'?ringSize='+ringSize;	
}
