...
function BrowserObj(){
	...
	this.ff3 = (navigator.userAgent.indexOf('Firefox/3.') > -1) ? true : false;
	this.ff5 = (navigator.userAgent.indexOf('Firefox/5.') > -1) ? true : false;
	...
	this.ie8 = (this.ie && info.indexOf('msie 8')!=-1) ? true : false;
	this.ie9 = (this.ie && info.indexOf('msie 9')!=-1) ? true : false;
...
var logFiner = false;
var logFinest = false;
// START Global FSM transitions
// Implementation of Search transition for use on global pages
function startSearch(lat, lng, address) {
	if (logFiner) console.log("Enter: startSearch");
	fsmRedirectToStoreLocator(lat, lng, address, false);
}

// Implementation of ShowSuggestions transition for use on global pages
function suggestions(lat, lng, address) {
	if (logFiner) console.log("Enter: suggestions");
	fsmRedirectToStoreLocator(lat, lng, address, true);
}

// Implementation of HandleErrors transition for use on global pages
function HandleErrors(error) {
	alert(error);
}
// END Global FSM transitions

// START Global FSM actions
var globalCurrentState = "Inactive";
var geocodingResults = null; 
function changeState(nextState, event) {
	// Set current state to next state. Do not invoke changeState handler for this.
	globalCurrentState = nextState;
}
function getCurrentState() {
	if (typeof storeMap !== 'undefined')
		return storeMap.currentState;
	return globalCurrentState;
}
function handleErrors(msg) {
	if (logFiner) console.log("ENTER: handleErrors");
	if (logFiner) console.log(msg);
}
function fsmRedirectToStoreLocator(lat, lng, address, showSuggestions) {
	...
	if (typeof storeMap == 'undefined')
	{	
	window.location.href = "http://" + document.domain + "/webapp/wcs/stores/servlet/StoreLocatorView?langId=-1&storeId=10101&catalogId=10001&latitude=" + lat + "&longitude=" + lng + "&current=" + current + "&address=" + address;
	}
	else
	{
		var path = "http://" + document.domain + "/webapp/wcs/stores/servlet/StoreLocatorView?langId=-1&storeId=10101&catalogId=10001&latitude=" + lat + "&longitude=" + lng + "&current=" + current + "&address=" + address;	
		submitStoresReq(path,{'filterStoreName':storeMap.filterStoreName, 'filterStoreId':storeMap.filterStoreId});
	}
	}

function submitStoresReq(path,filter_params) {
    ...
    for(var key in filter_params) {
            ...
            hiddenFld.setAttribute("name", key);
            hiddenFld.setAttribute("value", filter_params[key]);
            frm.appendChild(hiddenFld);
    }
...
function resetMap() {
	if (typeof storeMap !== 'undefined')
		storeMap.zoom = storeMap.DEFAULT_ZOOM;
}

function geocode(address, redirect, suggestion) {
	if (logFiner) console.log("ENTER: geocode");	
	geocoder = new google.maps.Geocoder();
	if (suggestion) {
		if (logFiner) console.log("DEBUG: sugestion: " + suggestion);
		address = suggestion.address;
	}
	if (logFiner) console.log("DEBUG: address: " + address);
	geocoder.geocode({'address': address}, function(results, status) {
		geocodingResults = results;
		if (typeof storeMap !== 'undefined')
			storeMap.handleEvent({ "type": "geocodeCallback" });
		if (status == google.maps.GeocoderStatus.OK) {
			...
			for (var i = 0; i < results.length; i++) {
				var duplicate = false;
				if (results.length == 0) {					
					// Next state is PROCESS_ERRORS
					// Transition is HandleErrors
					handleErrors("The address search you entered returned zero city or state locations.");
				}
			...
			if (redirect) {
				if (results.length == 1) {
					// The single result may be either a U.S. state or some other single location
					// Next state is SEARCHING
					// Transition is Search
					startSearch(lat, lng, address);
				} else if (suggestion) {
					// The single result may be either a U.S. state or some other single location
					// Next state is SEARCHING
					// Transition is Search
					startSearch(suggestion.lat, suggestion.lng, suggestion.address);
				} else {
					// Next state is SUGGESTIONS_PAGE
					// Transition is ShowSuggestions
					suggestions(lat, lng, address);
				}
			}
		} else {
			// Geocoding error
			// Next state is PROCESS_ERRORS
			// Transition is HandleErrors
			handleErrors("A problem occurred retrieving the location for your search: " + status);
		}
	});
}
...
function loadScript() {
  if (!scriptLoaded) {
	  ...
	  script.src = '//maps.googleapis.com/maps/api/js?client=gme-sterlingjewelers&v=3.16&sensor=false&libraries=geometry,places&region=US&callback=showFindStore';
...
// Find A Store modal
// Only used on global pages, not when on store locator page
function fsmFindStores(selectedSuggestion, search) {
	if (logFiner) console.log("ENTER: fsmFindStores");
	if (logFiner) console.log("DEBUG: selectedSuggestion: " + selectedSuggestion);
	...
	if (search) {
		address = search; // to support state landing page
	} else if(address == undefined || address == "" || address == "Enter a location" || address == null) {
	...
	if (selectedSuggestion >= 0) {
		...
		suggestion = storeMap.suggestions[selectedSuggestion];
		address = suggestion.address;
	...
	// This code is for filtering between main store and outlet stores
	var storeCheckBox = document.getElementById("store-checkbox");
	var outletCheckBox = document.getElementById("outlet-checkbox");
	var filterStoreId = "";
	var filterStoreName = "";
	if(storeCheckBox != null && outletCheckBox != null) {
		if(storeCheckBox.checked && outletCheckBox.checked) {
			//filterStoreId = "StoreId='"+storeCheckBox.value+"' or StoreId='"+outletCheckBox.value+"'";
			filterStoreId = WCParamJS.searchStore;
			filterStoreName = WCParamJS.storeOutletStoreName;
		} else if(storeCheckBox.checked) {
			filterStoreId = "StoreId='"+storeCheckBox.value+"'";
			filterStoreName = WCParamJS.storeName;
		} else if(outletCheckBox.checked) {
			filterStoreId = "StoreId='"+outletCheckBox.value+"'";
			filterStoreName = WCParamJS.outletStoreName;
		}
	}
    
	// Set the filterStoreId and filterStoreName to storeMap object.
	if (typeof storeMap != 'undefined' && storeMap.currentState != "ShipToStore") {
		storeMap.filterStoreId = filterStoreId;
		storeMap.filterStoreName = filterStoreName;
	}	
	
	// TRANSITION: GEOCODE
	changeState("Geocoding", "findStores");	
	geocode(address, true, suggestion);
	
	// To set a store locator view link to a storeLink hidden field for sending link in an email.  
	setTimeout(function(){
		var link = "http://" + document.domain + "/webapp/wcs/stores/servlet/StoreLocatorView?langId=-1&storeId=10101&catalogId=10001&latitude=" + response.lat + "&longitude=" + response.lng + "&address=" + address;
...
function findStores(selectedSuggestion, radius, subSequentSearch) {
	if (logFiner) console.log("ENTER: findStores");
	fsmFindStores(selectedSuggestion);
}
//END Global FSM actions
