...
{
	validEmailRegExp= /[A-z0-9!#$%&'*+\/=?^_`{|}~-]+(?:\.[A-z0-9!#$%&'*+\/=?^_`{|}~-]+)*@(?:[A-z0-9](?:[A-z0-9-]*[A-z0-9])?\.)+[A-z0-9](?:[A-z0-9-]*[A-z0-9])?/g
	var bValidEmail = true;
	if (oFieldValue == undefined || oFieldValue == null) return false;
	if (oFieldValue.match(validEmailRegExp) != oFieldValue) {
		bValidEmail = false;
	}
 ...
 * dates that are split into month, day, and year.
 ******************************************************/
function IsValidDateInCreditApp(szMonth, szDay, szYear, szInputFieldName, szErrorColor)
{
	var bValidDate = true;
	var iValuesProvided = 0;
 ...
 ******************************************************/
function changeFieldColor(szFieldName, szColor, fontWeight)
{
	var weight = "normal";
	if(fontWeight != null)
	{
