...
(function( $ ){
  ...
  var methods = {
	...
	*/
    attachButton : function (parentNode, dialogTitle, divID, onButton, offButton, url, width, height, showInCenter,family) {
		//var divObj = $("div:first", parentNode);

		if(family !='true'){
		   var divObj = $(".pqve_btn_thumb_off", parentNode);
		}else{
			var divObj = $(".qveThumbnail")
		}
		...
		divObj.unbind("click").bind("click", function(){
			var obj = $(this);
			(typeof uagent === 'function' && uagent()=="mobile") ? obj.ml_popup(dialogTitle, "", url, "iframe", width, height):obj.ml_popup(dialogTitle, "", url, "iframe", width, height, (screen.height > 600) && showInCenter);
		});
	},
	/**
...
function applyGC(){
	jQuery("div .qveThumbnail").unbind("mouseenter").bind("mouseenter", function(){
		var obj = jQuery(this);
		obj.qve("attachButton", this, obj.attr("dialogTitle"), obj.attr("catPK"), obj.attr("buttonOn"), obj.attr("buttonOff"), obj.attr("url"), obj.attr("windowWidth"), obj.attr("windowHeight"),(obj.attr("showInCenter")!=null),obj.attr("familypk"));
	});
...
jQuery(document).ready(function() {applyGC();});
