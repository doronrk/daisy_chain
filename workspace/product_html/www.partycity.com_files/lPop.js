...
function closePopup(){
 ...
 function AX_Pop_Create(objpop){
	...
	if(objpop.pTP=="close_on_outside_click_reload"){reloadIt=true;pBG.onclick=closePopup;}
 ...
 function PC_Pop(pH,pW,pT,pL,pS,pID,pST,pTP,pSC,showWaitMsg){
	this.pHeight=pH;
	this.pWidth=pW;
	this.pTop=pT;
	this.pLeft=pL;
	...
	this.pSC=pSC;
	this.pTP=pTP;
	if(typeof showWaitMsg != 'undefined'){
		showLoadMsg = showWaitMsg;
	}
	AX_Pop_Create(this);
 }
...
function wf(){
	if(typeof uagent === 'function' && uagent()=="mobile")dnsp=true;
	AX_Win_Prop();
	if(py>0){
		for(i=1;i<=py;i++){
				if(dnsp){
