...
function DetailImageSwatches(iProductPk, oOptionType, iPreSelectedOptionPk, sDefaultLargeImage, sImagePath) {
  ...
  DetailImageSwatches.prototype.swatchChanged = function(iOptionPk) {
    ...
    if (iOptionPk > 0) {
      ...
      this.getLargeImage().src = this.isUrlWithScheme(oOption.sLargeImage) ? oOption.sLargeImage : (this.sImagePath + oOption.sLargeImage);
    ...
    //
      ...
      this.getLargeImage().src = this.isUrlWithScheme(this.sDefaultLargeImage) ? this.sDefaultLargeImage : (this.sImagePath + this.sDefaultLargeImage);
    }

    jQuery && jQuery(this.getLargeImage()).trigger("swatchChanged");
  ...
  DetailImageSwatches.prototype.swatchEnter = function(iOptionPk) {
    ...
    if (this.iSelectedSwatch != iOptionPk && this.iHoveredSwatch != iOptionPk) {
      ...
      // Update large image.
      oLargeImg.src = this.isUrlWithScheme(oOption.sLargeImage) ? oOption.sLargeImage : (this.sImagePath + oOption.sLargeImage);
  ...
  /**
   * Checks if an URL has scheme part inside or not.
   * @param {String} sUrl URL to be checked
   * @returns {Boolean} true if URL has the scheme part inside, false otherwise
   */
  DetailImageSwatches.prototype.isUrlWithScheme = function(sUrl) {
    return (sUrl && sUrl.indexOf('/') > 0 && sUrl.substring(0, sUrl.indexOf('/')).indexOf(':') > 0);
  }
