JY = {
    ...
    common: {
        init: function() {
            ...
            $('div#modal-quickshop').on('click', 'ul#qs-alt-views a', function (e) {
                e.preventDefault();
                var link = $(this);
            ...
            $('a#lnkTwHeader').prop('href', 'https://twitter.com/share?text=' + encodeURI($('meta[name=description]').prop('content')));
            $('a#lnkFbHeader').prop('href', 'http://www.facebook.com/sharer.php?s=100&p[url]=' + encodeURI(location.href) + '&p[summary]=' + $('meta[name=description]').prop('content') + 'p[title]=' + $('title').text());
            var ptImage = 'http%3A%2F%2Fjourneys.com%2Fassets%2Fimg%2Flogo-jy-main.png';
            if ( $('img#imgFull').length > 0 ){ ptImage = encodeURI($('img#imgFull').prop('src')); }
            $('a#lnkPtHeader').prop('href', '//pinterest.com/pin/create/button/?url=' + encodeURI(location.href) + '&media=' + ptImage + '&description='  + encodeURI($('meta[name=description]').prop('content')));
        },
        finalize: function() {
    ...
    jy_body_product: {
        ...
        jy_body_product: function() {
            ...
            $('select#selSizes').on('change', function () {
            	var chosen = $(this).find('option:selected');
            	if ($('div#no-size').is(':visible')) { $('div#no-size').hide(); }
            	if (chosen.prop('disabled') == true) {
            		var top = $('select#selSizes').offset().top - $('div#description-wrap').offset().top + 63;
            		$('div#no-size').slideDown();
            		$('div#no-size').css('bottom', top + 'px');
            	} else {
					JY.jy_body_product.chosenSize = chosen.val();
            	}
            });
            ...
            $('a#btn-check-store').click(function(e) {
                e.preventDefault();
                var sku = $('select#selCheckSize option:selected').val();
                if (sku === undefined) {
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            if (!data.found) {
                                ...
                                if (window.saleTrack) { UTILS.AddStoreTracker(data); } //Log DCStorm
                            } else {
    ...
    jy_gift_cards: {
        ...
        addToCart: function(selVal) {
            ...
            $.ajax({
                data: 'oid=' + selVal
                    + '&jyemail=' + $('input#txtJYRecEmail').val() + '&jyname=' + $('input#txtJYRecName').val()
                    + '&kzemail=' + $('input#txtKZRecEmail').val() + '&kzname=' + $('input#txtKZRecName').val()
                    + '&shemail=' + $('input#txtSHRecEmail').val() + '&shname=' + $('input#txtSHRecName').val(),
                url: '/ajx/getminicart.aspx',
                dataType: 'json',
                success: function(data) {
        ...
        jy_gift_cards: function() {
            $('input#rbJYVirtual').click(function () {
                $("select#selJourneys").val("");
                $("select#selJourneys").hide(); $("select#selJourneysVirtual").show();
                $("#dvJYForm").slideDown();
            });
            $('input#rbJYPhysical').click(function () {
                $("select#selJourneysVirtual").val("");
                $("select#selJourneysVirtual").hide(); $("select#selJourneys").show();
                $("#dvJYForm").slideUp();
            });
            $('input#rbKZVirtual').click(function () {
                $("select#selKidz").val("");
                $("select#selKidz").hide(); $("select#selKidzVirtual").show();
                $("#dvKZForm").slideDown();
            });
            $('input#rbKZPhysical').click(function () {
                $("select#selKidzVirtual").val("");
                $("select#selKidzVirtual").hide(); $("select#selKidz").show();
                $("#dvKZForm").slideUp();
            });
            $('input#rbSHVirtual').click(function () {
                $("select#selShi").val("");
                $("select#selShi").hide(); $("select#selShiVirtual").show();
                $("#dvSHForm").slideDown();
            });
            $('input#rbSHPhysical').click(function () {
                $("select#selShiVirtual").val("");
                $("select#selShiVirtual").hide(); $("select#selShi").show();
                $("#dvSHForm").slideUp();
            });
            ...
            $('a#btn-add-card').click(function(e) {
                ...
                $('div.error').hide();
                if ($('select#selJourneysVirtual option:selected').val() != '') {
                    if ($('input#txtJYRecName').val().replace(' ', '') == '') {
                        errors++;
                        $('div#rec-jy-name-error').slideDown();
                    }
                    if ($('input#txtJYRecEmail').val().replace(' ', '') == '') {
                        errors++;
                        $('div#rec-jy-email-error').slideDown();
                    }
                    if ($('input#txtJYRecEmail2').val().replace(' ', '') == '' || $('input#txtJYRecEmail2').val() != $('input#txtJYRecEmail').val()) {
                        errors++;
                        $('div#rec-jy-email2-error').slideDown();
                    }
                    if (errors <= 0) { cards += $('select#selJourneysVirtual').val() + ','; }
                }
                if ($('select#selKidzVirtual option:selected').val() != '') {
                    if ($('input#txtKZRecName').val().replace(' ', '') == '') {
                        errors++;
                        $('div#rec-kz-name-error').slideDown();
                    }
                    if ($('input#txtKZRecEmail').val().replace(' ', '') == '') {
                        errors++;
                        $('div#rec-kz-email-error').slideDown();
                    }
                    if ($('input#txtKZRecEmail2').val().replace(' ', '') == '' || $('input#txtKZRecEmail2').val() != $('input#txtKZRecEmail').val()) {
                        errors++;
                        $('div#rec-kz-email2-error').slideDown();
                    }
                    if (errors <= 0) { cards += $('select#selKidzVirtual').val() + ','; }
                }
                if ($('select#selShiVirtual option:selected').val() != '') {
                    if ($('input#txtSHRecName').val().replace(' ', '') == '') {
                        errors++;
                        $('div#rec-sh-name-error').slideDown();
                    }
                    if ($('input#txtSHRecEmail').val().replace(' ', '') == '') {
                        errors++;
                        $('div#rec-sh-email-error').slideDown();
                    }
                    if ($('input#txtSHRecEmail2').val().replace(' ', '') == '' || $('input#txtSHRecEmail2').val() != $('input#txtSHRecEmail').val()) {
                        errors++;
                        $('div#rec-sh-email2-error').slideDown();
                    }
                    if (errors <= 0) { cards += $('select#selShiVirtual').val() + ','; }
                }
                ...
                if (errors<=0) JY.jy_gift_cards.addToCart(cards);
            });
        },
        finalize: function() {
    ...
    jy_contact: {
        init: function(){
            ...
            $('a#lnk-popup').on('click', function(e){
                e.preventDefault();
                $('div#modal-serious').dialog('open');
            });
            $('a#btn-close-modal').click(function(e) {
                e.preventDefault();
                $('div#modal-serious').dialog('close');
            });
            $('body').keydown(function(e){
                if (e.keyCode == 27){
                    $('div#modal-serious').dialog('close');
    ...
    jy_cart: {
        ...
        jy_cart: function() {
            ...
            $('div#promoApply strong').on('click', function () {
                $(this).slideUp(150, function(){
                    $('div#promoWrap').slideDown();
                });
            });
            ...
            if ($('#pPromoCodeErr').text() != 0) {
                $('div#promoWrap').slideDown();
            } else {
                $('div#promoWrap').slideUp();
            };
            ...
            $('div#modal-piggy').dialog({
                autoOpen: false,
                modal: true,
                position: ['center', 'center'],
                draggable: false,
                resizable: false,
                dialogClass: 'modal-piggy-wrap'
            });
            $('a#lnkVPmodal').click(function (e) {
                e.preventDefault();
                $('div#modal-piggy').dialog('open');
                $('div#modal-piggy').css('height', $('div#modal-piggy div.modal-inner').height() + 40 + 'px');
            });
            $('a#btn-close-vp').click(function (e) {
                e.preventDefault();
                $('div#vpChildList').empty();
                $('div#vpPaymentList').empty();
                $('div#piggyLogin').show();
                $('div#piggyParent').hide();
                $('div#modal-piggy').dialog('close');
            });
            $('a#btnVpSignIn').on('click', function (e) {
                e.preventDefault();
                var $link = $(this);
                var $loading = $('img#imgOinkLoad');
                if ($link.hasClass('disabled')){ return false; }
                $loading.hide();
                $link.addClass('disabled');
                $loading.show();
                $('div#vpLoginError').empty().hide();
                var loginObj = { 'action': 'login', 'email': $('input#txtVpUser').val(), 'password': $('input#txtVpPassword').val() };
                $.ajax({
                    url: '/ajx/oink.aspx',
                    dataType: 'json',
                    type: 'POST',
                    ...
                    success: function (data) {
                        if (data.Success) {
                            if (data.UserType === 'Child') {
                                ...
                                var childRadios = '', paymentRadios = '', children = data.Children, payments = data.Payments;
                                $('div#piggyLogin').hide();
                                $('div#piggyParent').show();
                                if (children.length < 1) {
                                    $('div#vpLoginError').html('Sorry, there are currently no children added to your account, log in to <a href="http://oink.com" target="_blank">oink.com</a> to update your account').show();
                                    $('a#btnConfirmVP').addClass('disabled');
                                    $('div#modal-piggy').css('height', $('div#modal-piggy div.modal-inner').height() + 40 + 'px');
                                } else {
                                    for (var i = 0; i < children.length; i++) {
                                        childRadios += '<label for="radChild-' + i + '" class="radio"><input type="radio" id="radChild-' + i + '" class="radio" name="vpChild" value="' + children[i].Identifier + '">' + children[i].Name + '</label>';
                                    }
                                    if (payments.length > 0) {
                                        for (var i = 0; i < payments.length; i++) {
                                            paymentRadios += '<label for="radPayment-' + i + '" class="radio"><input type="radio" id="radPayment-' + i + '" class="radio" name="vpPayment" value="' + payments[i].Identifier + '">' + payments[i].Name + '</label>';
                                        }
                                        $('div#vpChildList').empty().append(childRadios);
                                        $('div#vpPaymentList').empty().append(paymentRadios);
                                        $link.removeClass('disabled');
                                        $loading.hide();
                                        $('div#modal-piggy').css('height', $('div#modal-piggy div.modal-inner').height() + 40 + 'px');
                                    }else{
                                        $('div#vpLoginError').html('Sorry, there is currently no payment method added to your account please log into your <a href="http://oink.com" target="_blank">oink.com</a> account to add a payment method').show();
                                        $('a#btnConfirmVP').addClass('disabled');
                                        $('div#modal-piggy').css('height', $('div#modal-piggy div.modal-inner').height() + 40 + 'px');
                                    }
                                }
                            ...
                            $('div#vpLoginError').html(data.Message).show();
                            $link.removeClass('disabled');
                            $('div#modal-piggy').css('height', $('div#modal-piggy div.modal-inner').height() + 40 + 'px');
                            $loading.hide();
                        }
                    ...
                    error: function (a, b, c) {
                        console.log('error logging in to Oink: ', a, b, c);
                    }
                });
            });
            $('a#btnConfirmVP').on('click', function (e) {
                e.preventDefault();
                var confirmObj;
                $('div#vpConfirmError').empty().hide();
                if ($(this).hasClass('disabled')) {
                    return false;
                } else {
                    confirmObj = { 'action': 'parent', 'ChildID': $('div#vpChildList input[name="vpChild"]:checked').val(), 'PaymentID': $('div#vpPaymentList input[name="vpPayment"]:checked').val() };
                    $.ajax({
                        url: '/ajx/oink.aspx',
                        dataType: 'json',
                        type: 'POST',
                        ...
                        success: function (data) {
                            if (data.Success) {
                                ...
                                $('div#vpConfirmError').text(data.Message).show();
                            }
                        },
                        error: function (a, b, c) {
                            console.log('error confirming Oink Order: ', a, b, c);
                        }
                    });
                }
            });
        ...
        init: function() {
            ...
            $('div#form-payment-opts input[type="checkbox"]').each(function () {
                ...
                if ($cb.is(':checked')){
                    ...
                    if ($cb.prop('id') === 'cbCreditCard' && $('input#txtAddress1').val().length > 0) {
                        //$('div#form-payment-info label[for="chk-use-ship"]').click();
...
UTILS = {
    ...
    homeTabs: function() { 
        $('nav#home-tab-nav a').click(function(e) {
            ...
            $('article#' + showMe).show().addClass('active-tab');
            $('html,body').animate({ scrollTop: $('header#hdr-home-tabs').offset().top - 180 }, 200, 'easeInQuad');
            if ($(e.target).data('tab') === "tab-brand-video" || "tab-brand-slider") {
                $('#featured-brand-wrap ul').resize();
            }
    ...
    addToCart: function(qs) {
        ...
        if (qs) {
            ...
            sku = $('select#selSizes option:selected').val();
        }
    ...
    AddStoreTracker: function (stores) {
        //this fires the DCStorm Tracker
        saleTrack.addSaleItem({
            itemcount: 1,
            itemvalue: 0,
            m1: 'Check Store Availability',
            m2: stores.sku,
            m3: '',
            m4: '',
            m5: stores.zip,
            m6: stores.radius,
            m7: stores.count,
            m8: stores.inStock, //count In stock
            m9: stores.count - stores.inStock //count OOS
        });
        saleTrack.st = 'CheckStoreAvail';
        saleTrack.logSale(0);
