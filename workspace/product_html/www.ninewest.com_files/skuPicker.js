...
//---------------------
	...
	this.updateFields = function (e) {
		...
		omnSelectionReport(that.currentSKU,that.skuPicker);
   	}
		...
		if ($(that.skuPicker.idPrefix+'displayNameAndPrice')) {
			...
			if (colorChip.showStandardPrice) {
				...
				if(colorChip.availability!='In Stock') {
					...
					setLivePersonVar('True',1); // send SizeError to LivePerson
				} else {
					...
					setLivePersonVar('False',2); // send SizeError to LivePerson
				}				
				...
				if(colorChip.availability!='In Stock') {
					...
					setLivePersonVar('True',3); // send SizeError to LivePerson
				} else {
					$(that.skuPicker.idPrefix+'availProdSkuText').style.color = '';
					$(that.skuPicker.idPrefix+'nameShipDate').style.color = '';
					if($(that.skuPicker.idPrefix+'descShipDate')) {
				...
				setLivePersonVar('False',4); // send SizeError to LivePerson
			}
		}
	}
	...
	this.setMessage = function (force) {
			if(that.selectedColor != -1 && (!that.skuPicker.hasWidth || that.selectedWidth != -1) && (!that.skuPicker.hasSize || that.selectedSize != -1)) {
				...
				if(force) 
				{
					...
					setLivePersonVar('True',5); // send SizeError to LivePerson
				}
				...
				*/
				...
				if(force)
				{
					...
					setLivePersonVar('True',6); // send SizeError to LivePerson
				}
			}
	}
	this.addTo = function () {
...
function miniCartLogic(parent) {
	...
	this.ajaxSuccess = function(transport) {
		...
		// send cartTotal to LivePerson
		setLivePersonCartTotal(parseInt(jQuery('#miniSubTotal').text().replace(/\D/g,''))/100);
	}
...
function omnAjaxReport(){
	try{
		if($('trackingDataOMN')) {
			...
			try{jQuery('.AjaxOmnitureData').remove();}catch(e){}
		}
	} catch (e){}
}
function omnSelectionReport(sku,currentPicker){
	try{
		if(sku.variantColor == currentPicker.selectionStates.selectedColor && (!currentPicker.hasWidth || sku.variantWidth == currentPicker.selectionStates.selectedWidth) && (!currentPicker.hasSize || sku.variantSize == currentPicker.selectionStates.selectedSize)){
			if(currentPicker.prdNamee == '') {
				currentPicker.prdNamee = _metadata.products[0].product.split(':').first();
			}
			if(currentPicker.defaultAvailabilityy == 2) {
				currentPicker.defaultAvailabilityy = _metadata.products[0].product.split(':').last();
			}
			
			var prdName = currentPicker.prdNamee;
			prdName += ':'+$(currentPicker.idPrefix+sku.variantColor).title;
			if(currentPicker.hasWidth){prdName += ':'+$(currentPicker.idPrefix+sku.variantWidth).title;}
			if(currentPicker.hasSize){prdName += ':'+$(currentPicker.idPrefix+sku.variantSize).title;}
			prdName += ':'+currentPicker.defaultAvailabilityy;
...
// send SizeError to LivePerson
function setLivePersonVar(val,mark) {
	try {
		lpSendData('page','SizeError',val);
	}
	catch(e){}
}
// send cartTotal to LivePerson
function setLivePersonCartTotal(val) {
	try {
		lpSendData('page','cartTotal',val);
	}
	catch(e){}
}