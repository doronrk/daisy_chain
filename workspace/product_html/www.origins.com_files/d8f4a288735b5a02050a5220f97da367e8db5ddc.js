...
a.loadTimeout=500;a.marketingCloudServer=a.audienceManagerServer="dpm.demdex.net"}Visitor.getInstance=function(k){var a,f=window.s_c_il,i;if(f)for(i=0;i<f.length;i++)if((a=f[i])&&"Visitor"==a._c&&a.namespace==k)return a;return new Visitor(k)};
   ...
   // s.account="esteeoriginsna,esteeglobal"
    ...
    var prod="www.origins.com";
    var reHostName = new RegExp(omnitureHostName, 'g');
    
    if ( !prod.match(reHostName) ){
    ...
    function s_doPlugins(s) {
        ...
        s.eVar21 = s.getTimeParting('f', '-4');       
	  
	    // top navigation
	    // 'vX' names the cookie; if using eVar1, use 'v1'
      //if (s.c_r('v24') === 'true') {
        if (s.c_r('v24') != 'noclick' && s.c_r('v24') !=''){
    	    // will set eVarX to the value in the cookie, which should be the link name
	        var thisClick= s.c_r('v24');
            //console.log("last click was true",thisClick)
            
	        // update to use the correct event for navigation clicks
	        //s.events = s.apl(s.events, 'event8', ',', 2);
            if (typeof jQuery != 'undefined'){
               jQuery(window).trigger("GNAV:CLICK", [thisClick]); 
            }
            s.c_w('v24', 'noclick');
	   }
    } 
...
// either in s_code or using a BrightTag event handler    
s.setupLinkTracking = function($) {
	$(function() {
		$('#navigation').on('click', 'a', function() {
            //console.log("Clicked");
			if (s.checkLink($(this).attr('href'))) {
				// update 'vX' to reference the right eVar
