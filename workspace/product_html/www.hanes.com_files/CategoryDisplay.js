...
getDefaultItem : function(productId){
  ...
  *
  **/
  Add2ShopCartAjax : function(entitledItemId,quantity,isPopup,customParams, customerParams2)
  {
    var entitledItemJSON;
    ...
    var catalogEntryId = this.getCatalogEntryId();
    if(catalogEntryId!=null){
      this.AddItem2ShopCartAjax(catalogEntryId , quantity,customParams, customerParams2["field2"]);
      this.baseItemAddedToCart=true;
      requestSubmitted = false;
      document.getElementById('SuccessMessageText').innerHTML = MessageHelper.messages['SHOPCART_ADDED'];
    } else{
      MessageHelper.displayErrorMessage(MessageHelper.messages['ERR_RESOLVING_SKU']);
      this.baseItemAddedToCart=false;
    }

    var paramsArray = customerParams2["field2"].split("||");
    var categoryId = paramsArray[0];

    callTealiumAddToCartTag(categoryId, 'quick');
  ...
  *
  **/
  AddItem2ShopCartAjax : function(catEntryIdentifier, quantity, customParams, viewType)
  {
    var params = [];
    params.storeId    = this.storeId;
    params.catalogId  = this.catalogId;
    ...
    // Setting CatEntry & Quanity for display in QuickView ShopCart popup
    this.setCatEntryQuantityJustAdded(catEntryIdentifier,quantity);
...
widgetHTML = widgetHTML
  ...
  **/
  ...
  if(this.merchandisingAssociationItems[0].catEntry_Type=='ProductBean'){
    this.AddItem2ShopCartAjax(baseItemId,baseItemQuantity,customParams, viewType);
    if(this.baseItemAddedToCart){
      showPopup(this.merchandisingAssociationItems[0].catEntry_Identifier,function(e){return e;},'marchandisingAssociationDisplay');
    }
  }else if (this.merchandisingAssociationItems[0].catEntry_Type=='ItemBean' || this.merchandisingAssociationItems[0].catEntry_Type=='PackageBean' || this.merchandisingAssociationItems[0].catEntry_Type=='BundleBean'){
...
setOrderId : function(orderId)
{
  ...
  **/
  ...
  setCatEntryQuantityJustAdded:function(catalogEntryId, quantity){
    this.catEntryQuantityJustAdded = {};
    if(!dojo.isArray(catalogEntryId) && !dojo.isArray(quantity)){
      this.catEntryQuantityJustAdded.catEntryId=catalogEntryId;
      this.catEntryQuantityJustAdded.quantity=quantity;
    }
  },
