...
cy.control._cyGetElementSrc = function(res)
{
	...
	if (cy.CUSTOMERCODE=='7989898747')
	{
 	 	var callout = cy.control.DXoptionCallOut();
...
var cyDxSrc=document.createElement("img");
    cyDxSrc.setAttribute("id","cydx");
   // cyDxSrc.type = "text/javascript";
    document.getElementsByTagName("head")[0].appendChild(cyDxSrc);

String.prototype.cyDxHash=function()
{
    var hash = 0;
...
function cyDxFLCheck()
{

	
	var protocol=window.location.protocol;
	cy.control._cyConvertCYPropertyNamesToUpperCase();
	cyDxFL=(cy.FUNNELLEVEL==undefined) ? cy.control.defaults.FunnelLevel : cy.FUNNELLEVEL;
	cyDxTX=cy.control._cyGetSessionId();
	cyDxSKU=(cy.CUSTOM1==undefined) ? cy.control.defaults.Custom1 : cy.CUSTOM1;
	cyDxPR=(cy.VALUE==undefined) ? cy.control.defaults.Value : cy.VALUE;
	if(cyDxFL>='7')
	{
		if(_cyGetCookie("_cy_dx")==-1)
		{
			cyDxSrc.src=protocol+'//tags.w55c.net/rs?id=6e7f1d9c3d1042969e968ada6503e9ab&t=checkout&tx='+cyDxTX+'&sku='+cyDxSKU+'&price='+cyDxPR;	
		}
		else
		{
			cyDxSrc.src=protocol+'//tags.w55c.net/rs?id=a21952e02da14eb19c35c1fa0c6e6f6b&t=checkout&tx='+cyDxTX+'&sku='+cyDxSKU+'&price='+cyDxPR;
		}
	}
...
{
	cy.control._cyConvertCYPropertyNamesToUpperCase();
	var domain=window.document.domain.toUpperCase();
	var page=window.location.pathname.toUpperCase();
	var title=window.document.title.toUpperCase();
	var fl=(cy.FUNNELLEVEL==undefined) ? cy.control.defaults.FunnelLevel : cy.FUNNELLEVEL;
	cyDxTX=cy.control._cyGetSessionId();
	cyDxSKU=(cy.CUSTOM1==undefined) ? cy.control.defaults.Custom1 : cy.CUSTOM1;
	cyDxPR=(cy.VALUE==undefined) ? cy.control.defaults.Value : cy.VALUE;
	var protocol=window.location.protocol;
	
	if (page.search('PRODUCT')>0)
	{
		dxserve='//tags.w55c.net/rs?id=d6437199d0a54783886523a0721d796c&t=checkout&tx='+cyDxTX+'&sku='+cyDxSKU+'&price='+cyDxPR;
	}
	else if (title.search('CART')>0 || title=='CART' || fl=='3')
	{
		dxserve='//tags.w55c.net/rs?id=9064acd1a66f4252beaae7edbac95d6e&t=cart&sku='+cyDxSKU;
	}
	else
	{
		dxserve='//tags.w55c.net/rs?id=419f4f1662b6408a970e2af9abf42e99&t=checkout&tx='+cyDxTX+'&sku='+cyDxSKU+'&price='+cyDxPR;
	}
	
	cyDxSrc.src=protocol+dxserve;
	
}

function cyDxDesc(seed,dropcookie)
{
var dxlink=''
var protocol=window.location.protocol;
cyDxFL=(cy.FUNNELLEVEL==undefined) ? cy.control.defaults.FunnelLevel : cy.FUNNELLEVEL;
...
cyDxPR=(cy.VALUE==undefined) ? cy.control.defaults.Value : cy.VALUE;
	...
	{
		decision=-1; //PCTRL
		dxlink='//tags.w55c.net/rs?id=20dd2ff6156545b8a20a4a093f0271a8&t=homepage';
		        	
        	cyDxSrc.src=protocol+dxlink;
		
	}
	else if (seed >=0 && seed <= 0.2)
	{
		decision=0;
        	dxlink='//tags.w55c.net/rs?id=e8ae6e1daab44185a45fbe2c9a46d284&t=checkout&tx='+cyDxTX+'&sku='+cyDxSKU+'&price='+cyDxPR;
        	
        	cyDxSrc.src=protocol+dxlink; 
        }
	else if (seed>0.2 && seed<=0.6) //EO
	{
	...
	cy.Custom11=decision;
	cy.Custom12=seed;
	
	if(dropcookie)
	{
		_cyCreateClientCookie("_cy_dx",seed,2419200);
	}
...
function cyDX(){
	...
	if(cyQSCheck("cyEmail")){
		cy.UserId=unescape(cyQSCheck("cyEmail"));
		cy.FunnelLevel='1';
		cyDxTX=cy.control._cyGetSessionId();
		cyDxSKU=(cy.CUSTOM1==undefined) ? cy.control.defaults.Custom1 : cy.CUSTOM1;
		cyDxPR=(cy.VALUE==undefined) ? cy.control.defaults.Value : cy.VALUE;
		dxserve='tags.w55c.net/rs?id=e6c9e122feef4df0aaf3c94a3523e986&t=checkout&tx='+cyDxTX+'&sku='+cyDxSKU+'&price='+cyDxPR;
	}
	
	cy.control._cyConvertCYPropertyNamesToUpperCase();
	cyDxVal=(cy.USERID==undefined || cy.USERID=='') ? -1 : cy.USERID.toUpperCase().cyDxHash();
	cyDxSes=cy.control._cyGetSessionId().toUpperCase().cyDxHash();	

	//alert(cyDxVal);
	//alert(cyDxSes);
	
	if(_cyGetCookie("_cy_dx"))
	{
		cyDxDesc(_cyGetCookie("_cy_dx"),false);
	}
	else if(cyDxSes > -1 && cyDxVal < 0)
	{
		if (cyDxSes <= 0.1)
		{
			cyDxDesc(-1,true);
...
function cyQSCheck(cyQN){
	
	cyQS = window.location.search.substring(1);
	cyQSS = cyQS.split("&");
	for (i=0;i<cyQSS.length;i++)
	{
		cyQR = cyQSS[i].split("=");
		if (cyQR[0] == cyQN)
		{
...
cy.control.DXoptionCallOut = function()
{
    //by default, do nothing

    //=====================optioncallout mergepoint
	cyDX();

    //====================end optioncallout mergepoint
 }
