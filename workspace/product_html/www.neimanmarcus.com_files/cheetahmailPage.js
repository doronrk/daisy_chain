...
//This 	cheetahMailDomainName is set in the property - jsVarProperties of the
// BrandSpecs.properties , so you can add as many var as you want to access it in the JSP
...
function hs_gtCk(hs_cn) {
	...
	for (var i = 0; i < hs_ca.length; i++) {
		var hs_tcn = hs_ca[i];
		while (hs_tcn.charAt(0) == ' ')
			hs_tcn = hs_tcn.substring(1, hs_tcn.length);
		if (hs_tcn.indexOf(hs_tcne) == 0)
			return hs_tcn.substring(hs_tcne.length, hs_tcn.length);
	}
	return 0;
}
...
if (hs_gtCk('hs_basket') != "")
	hs_ES = hs_gtCk('hs_basket').replace(/\|\|/g, '').replace(/\|/g, '~')
			.replace(/~~/g, '~')
			+ hs_ES;
...
function hs_extDom(hs_fullURL, addWWW) {
	...
	if (fp != -1) {
		...
		if (fp == -1)
			hs_retDom = "www." + hs_retDom;
	}
	return hs_retDom;
}
function hs_addDefPg(hses) {
	if (hs_defPg == "donothing")
		return hses;
	hs_fS = hses.indexOf("://");
	if (hs_fS != -1) {
		hs_fS += 3;
...
function hs_flipPixel(lObj) {
	if (h_d.hs_pix_es && h_d.hs_pix_es.src && lObj.href && lObj.linkIndex >= 0) {
		...
		if (hs_trkDom == hs_chkDom) {
			...
			if (hs_trkDom != hs_chkDom)
				hs_trkLnk = hs_repl(hs_trkLnk, hs_trkDom, hs_chkDom);
			...
			hs_lnkStr = hs_mkHSD("28", hs_aff, "5", hs_n5, "31", hs_ev31,
					hs_levi, hs_trkLnk, hs_levii, lObj.linkIndex);
			...
			hs_lnkStr += hs_dlev + "~" + hs_ext + "~" + hs_dlnm + "~"
					+ hs_trkLnk + "~" + hs_dlpgnm + "~" + hs_n5 + "~";
		} else {
			hs_lnkStr = hs_mkHSD("28", hs_aff, "5", hs_n5, "31", hs_ev31,
					hs_levi, hs_esc(hs_trkLnk), hs_levii, lObj.linkIndex);
			if (hs_trkLnk.indexOf('http') >= 0 && hs_trkLnk.indexOf('http') < 2)

				hs_lnkStr += hs_levo + "~" + hs_esc(hs_trkLnk) + "~"
						+ hs_levopn + "~" + hs_n5 + "~"
						+ hs_gEv(lObj.href.toLowerCase(), "lohs");
		}
		hs_lnkStr += "29~~28~" + hs_aff;
		if (hs_add20 == 1) {
			hs_lnkStr = "20~~" + hs_lnkStr;
		}
		h_d.hs_pix_es.src = hs_coll + "RND="
				+ Math.floor(Math.random() * 10000) + "&event=" + hs_lnkStr;
...
function hs_linkFix(passedArg) {
	...
	if (h_w.hs_strt) {
		...
		if (h_d.hs_pix_es && h_w.sc_timePage && h_w.sc_timePage == 1) {
			h_d.hs_pix_es.src = hs_coll + "event=29~~28~" + hs_aff + "~32~"
					+ hs_td + "~121~" + hs_n5;
		}
	}
	if (h_w.origOnload) {
...
h_d.write("<i" + "mg" + " name='hs_pix_es' src='" + hs_coll + hs_CB + "event="
		+ hs_ES + "' border=0 height=1 width=1>");
h_w.linksChecked = 0;
