...
sidecar.app = {
	...
	detectTransactions: function() {
		...
		if (sidecar.transactions.add === undefined || !sidecar.transactions.add) {
			/* if (!this.empty('JSON', true)) { sidecar.sjax({ data: { service: "debug_transaction", transaction: JSON.stringify(sidecar.transactions) }, success: "sidecar.trans.noAction" }); } */
...
sidecar.config = {
	site_id: 134,
	site_name: "Fansedge",
	site_key: "87b1e0581acc803303fb175d77ec9dba",
	logging: true,
	jxhrdata: {
		url: "http://membrain.getsidecar.com/trackstar/"
	},
	cookie_domain: '.fansedge.com',
	search_url: 'http://www.fansedge.com/search/',
	search_param: '',
	cart: {},
	hosts: [],
	required_includes: [
