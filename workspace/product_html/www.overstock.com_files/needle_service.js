...
N.nps='2';else
N.nps='3';}
N.scheme='https';np.lz='overstock';if(np.productPrice&&np.productPrice.match(/^n_/)){np.n_productPrice=np.productPrice.substring(2,np.productPrice.length);delete np.productPrice;}
np.proactive='off';if(window.s&&s.eVar25&&s.eVar25.match(/PROD_CHAT_NEED2$/)){np.proactive='on';}else if(window.s&&s.eVar24&&s.eVar24.match(/PROD_CHAT_NEED2$/)){np.proactive='on';}
if(window.s&&s.eVar25){var tg=s.eVar25.match(/PROD_CHAT_NEED$|PROD_CHAT_NEED2$|PROD_CHAT_NEED3$|PROD_CHAT_NEED4$/);if(tg){np.testGroup=tg.toString();}}else if(window.s&&s.eVar24){var tg=s.eVar24.match(/PROD_CHAT_NEED$|PROD_CHAT_NEED2$|PROD_CHAT_NEED3$|PROD_CHAT_NEED4$/);if(tg){np.testGroup=tg.toString();}}
if(window.s&&s.eVar6){if(s.eVar6==="Home & Garden"){np.skillGroup='home';}
else if(s.eVar6==="Jewelry & Watches"){np.skillGroup='jewelry';}}
N.analyticsCall=function(a,v,e){var s=null;if(typeof s_account=="undefined"){N.PageUpdate("A","&call="+a+"&error=1");return;}
if(typeof s_gi=="function"&&typeof s_account=="string"){s=s_gi(s_account);}
if(typeof s.tl!="function"){N.PageUpdate('A',"&call="+a+"&error=2");return;}
if(s==null){N.PageUpdate("A","&call="+a+"&error=1");return;}
if(a=="qc"){s.linkTrackEvents='event45';s.events='event45';s.linkTrackVars='events';}else if(a=='tg'){s.prop19=v;s.linkTrackVars='prop19';}else if(a=='bc'){s.linkTrackEvents='event81';s.events='event81';s.linkTrackVars='events';}else{N.PageUpdate('A',"&call="+a+"&error=3");return;}
try{s.tl(true,'o','Needle Chat');N.PageUpdate('A',"&call="+a);}catch(err){N.PageUpdate('A',"&call="+a+'&error=4');}};if(!N.loaded&&N.nps<3&&N.getCookie("needleopt")==null){if("onfocusin"in document){N.oldOnFocus=document.onfocusin;N.oldOnBlur=document.onfocusout;document.onfocusin=N.onFocus;document.onfocusout=N.onBlur;}else{N.oldOnFocus=window.onfocus;N.oldOnBlur=window.onblur;window.onfocus=N.onFocus;window.onblur=N.onBlur;}
