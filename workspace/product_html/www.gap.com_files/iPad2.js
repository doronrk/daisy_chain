	...
	//$("zoomToolPic").src = "/gid/assets/common/en/iPadTouchToZoom.gif";
	$("zoomToolPic").setStyle({ "background":"url(/gid/assets/common/en/iPadTouchToZoom.gif) center center no-repeat" });
...
//	console.info("Begin: invoke ProductZoom.mouseOverHandler");
				...
				//$("zoomToolPic").style.background = "none";
				//$("zoomToolPic").src = "/gid/assets/common/en/iPadTouchToZoomiOS5.gif";
				$("zoomToolPic").setStyle({ "background":"url(/gid/assets/common/en/iPadTouchToZoom.gif) center center no-repeat" });
...
// console.info("iPadZoomOverride");
		...
		var previewIsTrue  = ['www.plol.wip.gidapps.com','www.plol.app.gidapps.com'];
		var topValue = "0px";
		var topValue2 = "0px";
		if (zoomIsTrue.include(location.pathname)){
			...
			if (iPadNavSpace.indexOf("iPad") != -1) { // if iPad, override the zoom
				...
				var zoomEventTrigger  = function() {
					...
					//$("zoomToolPic").src = "/gid/assets/common/en/iPadTouchToZoomOut.gif";
					$("zoomToolPic").setStyle({ "background":"url(/gid/assets/common/en/iPadTouchToZoomOut.gif) center center no-repeat" });
	...
	var sideNavLineHeight = function() {
		...
		var sideNavIsTrue = ['/browse/division.do','/browse/category.do','/browse/categorySearch.do','/browse/subDivision.do','/browse/product.do'];
		
		if (sideNavIsTrue.include(location.pathname)){
			var iPadNavSpace = navigator.userAgent;
			if (iPadNavSpace.indexOf("iPad") != -1) {
				//$('sideNav').style.lineHeight="30px";
				var sideNavContainers = $("sideNav").select("[id^=categoryNav]");
				for (i = 0; i < sideNavContainers.length; i++) {
					sideNavContainers[i].setStyle({ lineHeight:'30px' });
	...
	var iPadQuicklookOverride = function() {   
		var sideNavIsTrue = ['/browse/category.do','/browse/categorySearch.do','/browse/search.do','/browse/product.do']; // only on category, product (for cross sells) and search pages
		if (sideNavIsTrue.include(location.pathname)){
	...
	var saveToHomeScreenImg = new Element('img', { 'style': 'display:block;position:absolute;top:1px;left:1px;z-index:456;', 'class' : 'screenpop', 'id' : 'saveToHomeScreenImg', 'width' : '321', 'height' : '139', 'alt' : 'get gap on your home screen, click the "forward" button and then "add to home screen"' });
				
	var saveToHomeScreen = function() {
		var expireDate = new Date();
		expireDate.setDate(expireDate.getDate() + 30);
		gidLib.setCookieVar("iPadHomeSave","iPadHomeScreenPL","true",expireDate);
		if (location.pathname.indexOf("home.do") != -1){
			var iPadNavSpace = navigator.userAgent;
			if (iPadNavSpace.indexOf("iPad") != -1) { // if iPad, run the code
				document.title = "Piperlime.com"; // be sure your brand is here
				// use window.devicePixelRatio to detect retina display, assumes 1 if prop does not exist
				var pixelRatio = !!window.devicePixelRatio ? window.devicePixelRatio : 1; 
				// be sure your band's img path is here
				if (pixelRatio > 1) { // this is a retina device
					saveToHomeScreenImg.src = "/gid/assets/common/en/ipad_bubble_retina.png";
				} else {
					saveToHomeScreenImg.src = "/gid/assets/common/en/ipad_bubble.png";
				}
				$("mainContent").insert(saveToHomeScreenImg);
			}
		}
		//console.info("saveToHomeScreen() has run");
		var myTimeout = setTimeout("closeSaveToHomeScreen()", 30000);
		Event.observe(saveToHomeScreenImg, "click", closeSaveToHomeScreen);
	}
	
	var closeSaveToHomeScreen = function() {
		saveToHomeScreenImg.style.display = "none";
	}
	...
	var iPadInit = function() {	
		...
		if (gidLib.getCookieVar("iPadHomeSave","iPadHomeScreenPL") == "") {
			Event.observe(window, "load", saveToHomeScreen);
		}
