...
//    if (uagent.search(smartDeviceList) > -1) {
    if (DetectMobileQuick()) {
        ...
        if (homePageMatch != null && homePageMatch.length > 0) {
            window.location = "/Mobile/Default.aspx?originalReferrer=" + referrer + (currentPageQueryStrings != null && currentPageQueryStrings.length > 0 ? "&" + currentPageQueryStrings : "");
        }
        else if (resultsPageMatch != null && resultsPageMatch.length > 0) {
            window.location = "/Mobile/Results.aspx?originalReferrer=" + referrer + (currentPageQueryStrings != null && currentPageQueryStrings.length > 0 ? "&" + currentPageQueryStrings : "");
        }
        else if (productPageMatch != null && productPageMatch.length > 0) {
            window.location = "/Mobile/ProductDetail.aspx?originalReferrer=" + referrer + (currentPageQueryStrings != null && currentPageQueryStrings.length > 0 ? "&" + currentPageQueryStrings : "");
        }
        else if (loginPageMatch != null && productPageMatch.length > 0) {
            window.location = "/Mobile/Profile/login.aspx?originalReferrer=" + referrer + (currentPageQueryStrings != null && currentPageQueryStrings.length > 0 ? "&" + currentPageQueryStrings : "");
        }
        else if (cartPageMatch != null && cartPageMatch.length > 0) {
            window.location = "/Mobile/Cart.aspx?originalReferrer=" + referrer + (currentPageQueryStrings != null && currentPageQueryStrings.length > 0 ? "&" + currentPageQueryStrings : "");
        }
    }
    else {
