...
function isPassLen(v){ return( v.length < 8 ); }
...
function isPwdValid(v){return( !(validateRegExp(v, "^(?=.*[A-Z])(?=.*[0-9])(?=.*[a-z])(?=.*[^A-Za-z0-9]).{8,32}$") || (v.length == 0) ) );}
function isEmptySpace(v){if(v.match(/^.*[^\s{1,}]\s.*/)){return true;}return false;}
...
errMap[13] = {fn:isPassLen, msg:"Password must be 8 Characters long.", error:true};
...
errMap[49] ={fn:isPwdValid, msg:"Please enter a valid password: It must contain one Upper Case,one Lower Case, one Number and atleast one Special Character", error: true};
errMap[50] ={fn:isEmptySpace, msg:"Space detected: Password must not contain any space", error: true};
