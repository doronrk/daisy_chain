...
var productVariables = {
    ...
    launchCopy: 'Product added to the cart is not guaranteed until the checkout process is completed.',
    ...
    excludedMessage: '<span class="message" id="excludedMessage"> Excluded From Discount <span title="Item is excluded from discount. View details."><a class="info_icon" data-tooltip="The dollar value of this item will count toward meeting a required minimum purchase total necessary to receive a discount, but the price of this item itself will not be discounted. Only other eligible items in your cart will be discounted. (Example: You can purchase this product to reach the order threshold to receive a discount, but you will only receive the discount on other eligible items in your cart.)"></a></span></span>',
    freeShippingMessage: '<span class="message" id="freeShippingMessage" title="Item is eligible for FREE shipping"> Ships Free! <span title="Item is eligible for free shipping. View details."><a title="Item is eligible for free shipping. View details." class="info_icon" data-tooltip="We offer FREE SHIPPING on thousands of items every day! Look for the &quot;Free Shipping&quot; indicator on all eligible merchandise. Shipping will automatically be deducted at checkout. *Please note: This offer is valid only on eligible items, and does not apply to in-store orders. This offer is limited to standard delivery within the 48 contiguous US states and APO/FPO addresses. Excludes bulk orders and drop ships. Entire order must ship to a single address. Customer is responsible for shipping costs on returned merchandise. May not be used toward purchase of Gift Cards or team orders. Promotion may be modified or terminated at any time. Certain restrictions and exclusions may apply."></a></span></span>',
    shipRestrictionMessage: '<span class="message" id="shipRestrictionMessage"> International shipping restrictions may apply <span title="There are shipping restrctions on this item. View details."><a class="info_icon" data-tooltip="International shipping restrictions apply. International customers will be informed during check-out if restrictions apply to your country. "></a></span></span>',
    ...
    try {
        ...
        if (imageSetArray.length >= 1 && imageSetArray != "") {
            $.each(imageSetArray, function (i, img) {
                ...
                $('div[data-info=product_images] #selected_item ul').append('<li><div class="alt_view"><img src="http://images.footlocker.com/is/' + sku_nbr + '/zoom/' + productVariables.productName + '" class="reg_image d" alt="' + $("#product_images").attr("title") + '" border="0" /><img class="zoom_image" data-zoomsrc="' + scene7url + '/' + img.split(";")[0] + '" alt="' + $("#product_images").attr("title") + '" border="0" /></div><div class="pdp_sprite product_shadow"></div></li>');
            ...
            $.extend(productVariables.inlineZoomSettings, {
                'altText': $("#product_images").attr("title")
            });
            ...
            if (imageSetArray.length > 1) {
                if ($('div[data-info=product_images] .slide_buttons').length == 0) {
                    $('div[data-info=product_images]').append('<div class="slide_buttons" style="display: block;">' +
                        '<a href="#" class="sl_previous left-arrow pdp_sprite" title="View Previous Image" tabindex="560"></a>' +
                        '<a href="#" class="sl_next right-arrow pdp_sprite" title="View Next Image" tabindex="565"></a>' +
                    '</div>');
            ...
            var productName = $("#model_name").html();

            $('#selected_item').show();
            $('#zoominit').show();
            productVariables.productSpotlight.resizeSpotlight();
            if ($('.fullscreen-maximize').length === 0) {
                var clickZoomImage = $('#product_images .alt_view');
                $('.shoe_images').prepend('<a href="#full_screen" id="open_pushdown" class="full_toggle pdp_sprite fullscreen-maximize" title="View Full Screen" tabindex="545"></a><a class="screenreader zoomBtn" tabindex="550" href="javascript:void(0);" title="Zoom In">Zoom</a>');
            };
            $('#open_pushdown').fullScreen({ data: obj, altText: $("#product_images").attr("title") });
            $('#open_pushdown').on('click', cmConversionEventViewLargerImage);
        } else {
            console.log('NON-SCENE7')
            $('div[data-info=product_images] #selected_item ul').html('<li><img src="' + scene7url.replace(/\/is\/image\//, '/pi/') + '/' + sku_nbr + '/' + productVariables.mainImageSize[0] + '/' + productVariables.productName + '" class="reg_image" alt="' + productVariables.productName + '" border="0" /><div class="pdp_sprite product_shadow"></div></li>');
...
var productFunctions = {
    ...
    initHotLaunch: function () {
        ...
        if (productVariables.timeToHL < 0 || !productFunctions.checkHotLaunchItem($("#pdp_selectedSKU").val())) {
            ...
            if (productLaunchStyles.indexOf($("#pdp_selectedSKU").val()) !== -1) {
                if (productFunctions.checkHotSkuQtyOverMax($("#pdp_selectedSKU").val(), 1)) {
    ...
    checkHotSkuQtyOverMax: function (sku, quantity) {
        if (productLaunchStyles.indexOf(sku) && productVariables.hot_launch_max_per_order > 0 && productFunctions.checkHowManyHLSKU(sku) + parseInt(quantity) > productVariables.hot_launch_max_per_order) {
            return true;
        }
        return false;
    },
    ...
    changeSku: function (sku, selected) {
        ...
        $('.info_icon').each(function () {
            Tipped.create(this, $(this).attr('data-tooltip'), { 'showOn': productVariables.tooltipAction, 'closeButton': productVariables.TOUCH_DEVICE, maxWidth: 350, title: true });
        });
        $('#pdp_selectedSKU').val(sku_nbr).change();
        productFunctions.initHotLaunch();
        if (!selected) {
            sku_nbr = tempSku;
        }
    ...
    updateImages: function (sku) {
        if (typeof (sku) === 'string') {
            ...
            if (productVariables.styleData[sku_nbr][productFunctions.getStyleValue('scene7Enabled')]) {
                ...
                $('div[data-info=product_images] #hover_item ul li').html('<img src="' + scene7url.replace(/image\//, '') + '/' + sku + '/' + productVariables.mainImageSize[0] + '/" class="reg_image a" data-imageType="scene7" data-pisrc="' + scene7url.replace(/\/is\/image\//, '/pi/') + sku + '/' + productVariables.mainImageSize[0] + '/' + productVariables.productName + '" alt="' + productVariables.productName + '" border="0" /><div class="pdp_sprite product_shadow"></div>');
                ...
                $('#zoominit').hide();
                $('div[data-info=product_images] #hover_item ul li').html('<img src="' + scene7url.replace(/\/is\/image\//, '/pi/') + sku + '/' + productVariables.mainImageSize[0] + '/' + productName + '" class="reg_image" alt="' + productVariables.productName + '" border="0" /><div class="pdp_sprite product_shadow"></div>');
            }
            ...
            if (productVariables.styleData[sku_nbr][productFunctions.getStyleValue('scene7Enabled')]) {
                ...
                $('div[data-info=product_images] #selected_item ul li').html('<img src="' + scene7url.replace(/\/is\/image\//, '/pi/') + sku_nbr + '/' + productVariables.mainImageSize[0] + '/' + productVariables.productName + '" class="reg_image" alt="' + productVariables.productName + '" border="0" /><div class="pdp_sprite product_shadow"></div>');
    ...
    updateSizes: function () {
        if (model.HASSIZES) {
            $('div[data-info=product_sizes]').html($('<a />', { 'href': '#sizes', 'title': 'Select a Size'}).html('<span class="indicator pdp_sprite"></span><span class="arrow pdp_sprite"></span>')).attr({ 'id': 'select_size' });
            ...
            if (productVariables.showOutOfStock) {
                $.each(productVariables.modelData.AVAILABLE_SIZES, function (m, modelSize) {
                    $('div[data-info=product_sizes] #sizes select').append('<option value="' + $.trim(modelSize) + '" title="size ' + $.trim(modelSize) + '"data-modelsize="' + $.trim(modelSize).replace(/[^0-9a-z-]/gi, '_') + '" disabled data-sfs="false" class="disabled" tabindex="">' + $.trim(modelSize) + '</option>');
                });
                $.each(productVariables.styleData[sku_nbr][productFunctions.getStyleValue('availableSizes')], function (s, size) {
                    ...
                    if (size[4].toLowerCase() == 'n' && size[5].toLowerCase() == 'y') {
                        $('div[data-info=product_sizes] #sizes select option[data-modelsize=' + $.trim(size[0]).replace(/[^0-9a-z-]/gi, '_') + ']').append(productVariables.SFSMessage).attr('data-sfs', 'true').attr('title', 'Size ' + $.trim(size[0]) + productVariables.SFSMessage.replace(/ -/g, '').toLowerCase() + '');
                    }
                    if (size[6] !== 'N') {
                        $('div[data-info=product_sizes] #sizes select option[data-modelsize=' + $.trim(size[0]).replace(/[^0-9a-z-]/gi, '_') + ']').attr('data-backordered', size[3]);
                    }
                });
            } else {
                $.each(productVariables.styleData[sku_nbr][productFunctions.getStyleValue('availableSizes')], function (s, size) {
            ...
            if (productVariables.fitGuarentee != '') {
                $('div[data-info=product_sizes] #sizes').append($('<a />', { 'data-info': 'fit_guarentee', 'title': 'View 100% Fit Guarantee', 'href': productVariables.fitGuarentee, 'tabindex': '826' }));
            }
            $('div[data-info=product_sizes] #sizes').append($('<div />', { 'id': 'size_footer' }));
        } else {
            productFunctions.selectSize('');
        }
        $('div#sizes a.screenreader').removeClass('selected');
        $('div#sizes a.screenreader').removeAttr('data-href');
    },
    ...
    selectSize: function (size) {
        ...
        if (size) {
            ...
            $('#pdp_selectedSize').val($(this).val()).change();
        }
        ...
        if (productVariables.styleData[sku_nbr][productFunctions.getStyleValue('metadata')]['IS_STRIPERPEDIA']) {
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $('.pdp_description').append('<div id="articles"><h2 class="heading">Articles<span class="highlight"></span></h2></div>');
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (fitIcon !== 'truesize.gif' && !productVariables.showTrueSize) {
                        $('div[data-info=fit_info]').html('<div class="fitInfoTitle"><span class="fit-notice-left-shoe pdp_sprite"></span><span class="title" title="' + data[fitIcon][4] + '"></span><span class="fit-notice-right-shoe pdp_sprite"></span></div><div class="fitInfoContent"><p>' + data[fitIcon][4] + '</p></div>');
                    } else {
                        $('div[data-info=product_sizes]').attr('data-showfit', 'false');
                    }
                    if (fitIcon != '' && fitIcon != null) {
                        $('div[data-info=fit_icon]').html('<h2 class="heading" tabindex="663">Fit Guide<span class="highlight"></span></h2><img alt="Length runs ' + data[fitIcon][1] + '. Width runs ' + data[fitIcon][2] + '." tabindex="664" src="' + productVariables.fitInfoImgBaseURL + fitIcon + '" border="0" /><div><span class="fit_description">' + data[fitIcon][3] + '<span></div><div class="fit_info_wrappers"><div><span data-label="fit_length" class="label"></span><span data-value="fit_length" class="value">' + data[fitIcon][1] + '<span></div><div><span data-label="fit_width" class="label"></span><span data-value="fit_width" class="value">' + data[fitIcon][2] + '<span></div></div>');
    ...
    getOtherStyles: function () {
        ...
        $.each(productVariables.styleData, function (s, style) {
            ...
            if (s == selectedSku) {
                ...
                //selectedThumb = '<a href="javascript:void(0);" title="'+style[productFunctions.getStyleValue('attributes')]+'" data-sku="'+s+'"><img src="'+scene7url.replace(/image\//,'').replace(/\/is\//,'/pi/')+s+'/'+productVariables.otherStylesOptions.imageSize[0]+'/'+productVariables.productName+'" data-pisrc="'+scene7url.replace(/image\//,'').replace(/\/is\//, '/pi/')+s+'/'+productVariables.otherStylesOptions.imageSize[0]+'/'+productVariables.productName+'" border="0" /></a>';
        ...
        if ($('div[data-info=product_styles] .slide_content ul li').length > 1) {
            $('div[data-info=product_styles]').append('<div class="slide_buttons" style="display: block;">' +
                '<a href="#" class="sl_previous left-arrow pdp_sprite disable"  tabindex="599" title="View Previous Page of Alternate Styles"></a>' +
                '<a href="#" class="sl_next right-arrow pdp_sprite" tabindex="649" title="View Next Page of Alternate Styles"></a>' +
            '</div>');
        }
        $('[data-imageType="scene7"]').error(productFunctions.scene7ErrorLoad);
        $.extend(productVariables.stylesSpotlightSettings, {
            'transition': 'slide', 'endStop': true, 'rotate': false, 'onSnap': function () {
    ...
    getVideos: function () {
        if (productFunctions.getStyleValue('hasVideos')) {
            ...
            $.extend(productVariables.pdpVideoSettings, {
                'sku': sku_nbr, 'domain': '', 'initPlay': 'true', 'swfpath': '/ns/flash/video-player/uat/', 'onPlay': function () {
        ...
        $BV.ui("rr", "show_reviews", {
            ...
            onEvent: function (json) {
                ...
                if (json.eventSource == "Display") {
                    ...
                    /*ADDITION OF TITLE ATTRIBUTES TO PDP*/
                    $('.BVRRRatingSummaryLinkWriteFirst a').attr('title', 'Write a Review');

                    var bv_rating = $('#BVRRQuickTakeSectionID .BVRRRatingNumber').text();
                    var bv_ratingRange = $('#BVRRQuickTakeSectionID .BVRRRatingRangeNumber').text();

                    $('#BVRRQuickTakeSectionID .BVImgOrSprite').attr('title', bv_rating + " out of " + bv_ratingRange + " Rating");
                    $('#BVRRQuickTakeSectionID .BVImgOrSprite').attr('alt', bv_rating + " out of " + bv_ratingRange + " Rating");

                    $('#BVRRSummaryContainer .BVImgOrSprite').attr('title', bv_rating + " out of " + bv_ratingRange + " Rating");
                    $('#BVRRSummaryContainer .BVImgOrSprite').attr('alt', bv_rating + " out of " + bv_ratingRange + " Rating");

                    $(".BVRRNextPage a").attr('title', 'Go To Next Page');
                    $(".BVRRPreviousPage a").attr('title', 'Go To Previous Page');

                    $(".BVRRSocialBookmarkingSharingLinkFacebook img").attr('title', 'Share Review on Facebook');
                    $(".BVRRSocialBookmarkingSharingLinkDigg img").attr('title', 'Digg this Review');

                    $(".BVDI_FVPositive .BVDILink").attr('title', 'Review was Helpful');
                    $(".BVDI_FVNegative .BVDILink").attr('title', 'Review was Unhelpful');
                    $(".BVDI_FVReportLink .BVDILink").attr('title', 'Report Review');

                    $("#BVQASearchFormTextInputID").attr("title", "Search for a question");
                    $("#BVQASearchFormSubmitButtonID").attr("title", "Submit Search");

                    //$(".BVRRSortSelectWidget #BVRRDisplayContentSelectBVFrameID").before("<label for='BVRRDisplayContentSelectBVFrameID' style='font-size:0px; position:absolute;'>Sort By</label>");

                    /*END ADDITION OF TITLE ATTRIBUTES TO PDP*/

                    $(".BVRRFooter .BVRRPageNumber").each(function () {
                        $(this).children('a').attr('title', "Go to page " + $(this).text());
                    });

                    $(".BVRRDisplayContentBody .BVRRDisplayContentReview .BVRRRatingNormalImage .BVImgOrSprite").each(function () {
                        $(this).attr('title', $(this).attr("title") + " Rating");
                    });
            ...
            doShowContent: function () {
                ...
                //bvGoToTab('reviews');
                productFunctions.swapTabs('reviews');
                return false;
            }
        });
    ...
    swapTabs: function (tab) {
        ...
        if (productVariables.pdpStyle.indexOf('jump') !== -1) {
            $('html, body').stop();
            $('html, body').animate({ scrollTop: ($('div[data-tabcontent=' + tab + ']').offset().top - $('.pdp_header').height() - productVariables.adjust) + 'px' }, 700)
        }
        $(window).resize();
        $(window).scroll();
    },
    ...
    loadHash: function () {
        ...
        if (location.hash.toLowerCase().replace(/#/, '').indexOf('tab-') === -1) {
            productFunctions.swapTabs('shoe_image');
        }
    }
};
...
$(document).ready(function (e) {
    ...
    $("form#product_form").submit(function (e) {
        e.preventDefault();
        if (productVariables.hasOwnProperty('haltAdd') && productVariables.haltAdd == true) {
            return false;
        }
        addToCart();
    ...
    if (searchParams.hasOwnProperty('size') && searchParams.size !== ''){
        $('#product_sizes').val(decodeURIComponent(searchParams.size)).change(); //<- Set the desired size option
    }
...
function disableToCartButton() {
    ...
    $("form#product_form input[type=submit]").addClass('processing');
    $("form#product_form input[type=submit]").val('Processing...');
    productVariables.haltAdd = true;
...
function enableToCartButton() {
    ...
    /*
       This function is marginally useless because of the minicart code calling this immediately after it
       executes, allowing users to rapid-fire the add to cart...so it does nothing.
    */
}

function enableAddToCartButton() {
    //Moved the meat of the enable function to here 
    $("form#product_form input[type=submit]").removeClass('processing');
    $("form#product_form input[type=submit]").val('Add To Cart');
    productVariables.haltAdd = false;
}
...
//create Recently Viewed Product Name list
var recentName = $('.title[data-info="product_title"] h1').text();
if (recentName.length > 0) {
    if ($.cookie('read', 'RECENTNAMELIST').length <= 0) {
        var valueName = recentName;
        createCookie('RECENTNAMELIST', valueName);
    } else {
        var nameList = $.cookie('read', 'RECENTNAMELIST');
        var valueName = recentName + ', ' + nameList;
        createCookie('RECENTNAMELIST', valueName);
    }
}