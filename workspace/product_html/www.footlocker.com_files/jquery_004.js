...
function YouTube() {
	...
	this.init = function(id) {
		var yt = ('<iframe width="'+this._defaults.width+'" height="'+this._defaults.height+'" src="http://www.youtube.com/embed/'+this._defaults.id+'?rel=0&wmode=transparent" frameborder="0" allowfullscreen></iframe>');
		if(/msie/.test(navigator.userAgent.toLowerCase())) {
			yt = ('<object width="'+this._defaults.width+'" height="'+this._defaults.height+'"><param name="movie" value="http://www.youtube.com/v/'+this._defaults.id+'?version=3&amp;hl=en_US&amp;rel=0"></param><param name="allowFullScreen" value="true"></param><param name="wmode" value="transparent"></param><param name="allowscriptaccess" value="always"></param><embed src="http://www.youtube.com/v/'+this._defaults.id+'?version=3&amp;hl=en_US&amp;rel=0" type="application/x-shockwave-flash" width="'+this._defaults.width+'" height="'+this._defaults.height+'" wmode="transparent" allowscriptaccess="always" allowfullscreen="true"></embed></object>');
		}
		if(typeof this.callback == "function") this.callback(); else {};
...
(function($) { // Hide scope, no $ conflict
	...
	var settings = {
		...
		nameList: 'RECENTNAMELIST', //name of Recently Viewed Product Name list
		excludedSkus :[],
		sku: '', //Product Number,
	...
	var methods = {
		init : function(options) {
		    ...
		    return this.each(function() {
		    	...
		    	var otherCookieData = $.cookie('read', settings.nameList);
				var code = '';
				if(cookieData == "") {
					...
					var nameListArray = otherCookieData.split(",");
					for(var p = 0; p < productArray.length; p++) {
						if(p < settings.maxProducts){
							...
							if(settings.excludedSkus.indexOf(product[0]) === -1) {
							    code = code + '<li><a href="' + settings.domain + '/product/model:' + product[1] + '/sku:' + product[0] + '" class="recentViewed" data-recentsku="' + product[0] + '"><img alt="' + nameListArray[p] + '" src="' + settings.imageDomain + '/pi/' + product[0] + '/' + settings.imageSize[0] + '/" border="0" /></a></li>';
							}
						} else {
	...
	var methods = {
		...
		showFull : function(view) {
			...
			if($('#full_screen .main_view img.main_img').length == 0) {
			    ...
			    $('#full_screen .main_view img.main_img').attr({ 'src': settings.scene7url + view + "?wid=" + settings.currentZoom * 500 + "&hei=" + settings.currentZoom * 500, 'data-view': view, 'alt': settings.altText });
			}
			try {
			...
			$('#full_screen .main_view > img.main_img').load(function() {
				...
				if($('#full_screen .main_view img.loading_img').length == 0) {
				    ...
				    $('#full_screen .main_view img.loading_img').attr({ 'src': settings.scene7url + view + "?wid=" + settings.currentZoom * 500 + "&hei=" + settings.currentZoom * 500, 'data-view': view, 'alt': settings.altText });
				}
		...
		initFull : function() {
			$('#full_screen').remove();
			$('body').prepend('<div id="full_screen"><div class="full_screen_header"><span class="inner_header"><h2 title="Foot Locker Approved Logo"><span class="logo"></span></h2><a href="#full_screen" class="close_btn" title="Close Full Screen View"></a><span class="controls"><span id="zoomIn" title="Zoom In"></span><span class="zoom_label"></span><span id="zoomOut" title="Zoom Out"></span></span></span></div><span class="main_view"></span><span class="alt_views" title="View Alternate Image"></span></div>');
			$('#full_screen').css({
				overflow :'hidden'
			});
	...
	var methods = {
			...
			return this.each(function(){
				...
				if($element.data("target").children('img.zoom_image').length !== 0) {
					$element.data("target").children('img.zoom_image').css({
						'position':'absolute',
						'z-index':'1'
					}).attr('alt', settings.altText).wrap('<div class="mouse_follow"></div>').wrap('<div class="zoom_images" style="position:absolute;top:0;left:0;"></div>').hide();
		...
		initZoom : function(target) {
			...
			if(settings.type == 'followmouse' || settings.type == 'magnify') {
				...
				target.children('.mouse_follow').children('.zoom_images').children('img.zoom_image').attr('src', target.children('.mouse_follow').children('.zoom_images').children('img.zoom_image').attr('data-zoomsrc') + '?hei='+settings.zoomSize+'&wid='+settings.zoomSize).show();
			}
			...
			if(settings.type == 'drag') {
				...
				target.children('.mouse_follow').children('.zoom_images').children('img.zoom_image').attr('src', target.children('.mouse_follow').children('.zoom_images').children('img.zoom_image').attr('data-zoomsrc') + '?hei='+settings.targetHeight+'&wid='+settings.targetWidth).show();
				...
				target.children('.mouse_follow').children('.zoom_images').children('img.zoom_image').load(function() {
					if($(this).parent('.zoom_images').children('img.loading_img').length == 0) {
						...
						}).css({'position':'absolute','left':'0','top':'0','z-index':'2','height':settings.targetHeight,'width':settings.targetWidth}).appendTo($(this).parent('.zoom_images'));
					} else {
		...
		loading : function(target, e) {
			...
			target.children('.mouse_follow').children('.zoom_images').children('img.loading_img').animate({
				'height' : settings.currentZoom * settings.targetHeight,
				'width' : settings.currentZoom * settings.targetWidth
			},200);
			...
			if(settings.currentZoom != 1) {
				...
				left = parseInt(  (target.width() / 2) - (xPercent * (settings.currentZoom * settings.targetWidth))  );
				top = parseInt(  (target.height() / 2) - (yPercent * (settings.currentZoom * settings.targetHeight))  );
			}
		...
		updateZoomLevel : function(e) {
			...
			var height = settings.currentZoom * settings.targetHeight;
			var width = settings.currentZoom * settings.targetWidth;
			$(this).children('.mouse_follow').children('.zoom_images').children('img.zoom_image').css({
				'height':height,
				'width':width
			});
			$(this).children('.mouse_follow').children('.zoom_images').children('img.zoom_image').attr('src', $(this).children('.mouse_follow').children('.zoom_images').children('img.zoom_image').attr('data-zoomsrc') + '?hei='+height+'&wid='+width).show();
